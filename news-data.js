/* ============================================================
   SMR News data — single source for SMR-news.html
   Auto-updated by scripts/track-news.mjs (GitHub Actions); changes land via PR.
   Schema: date, title, summary, cat(인허가|계약|투자|기술|정책),
           type(General|PWR|BWR|SFR|HTGR|FHR|MSR|Micro), dev, region, source, url, k(internal dedup key)
   Optional: summaryLong (2~3문장 — 홈 피처드 카드에서 summary 대신 표시)
   ============================================================ */
window.SMR_NEWS = [
  {
    "date": "2026-06-14",
    "title": "우즈베키스탄 소형모듈원자로 수출 프로젝트 기공",
    "summary": "소형모듈원자로의 첫 수출 프로젝트가 우즈베키스탄에서 기공식을 개최했다.",
    "summaryLong": "소형모듈원자로(SMR)의 초규모 국제 수출 건설 프로젝트가 우즈베키스탄에서 정식 기공식을 거행했다. 이는 러시아 등 기술 보유국이 추진하는 소형모듈원자로의 국제 배치 전략의 첫 본격적 사례로, 중앙아시아 지역의 에너지 안보와 탈탄소화 수요에 대응하는 것으로 평가된다. 해당 프로젝트의 진행은 소형모듈원자로 기술의 글로벌 경쟁력을 입증하고, 향후 유사 지역 프로젝트 수주 확대에 긍정적 신호를 제공할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiS0FVX3lxTE9fcDNHZFcxa185emVBS2xhNE96aG85UUIzU21HV01BN1BvRmdxRHJYeDFmR3FfRHE5encyRVVsQkhNRHdKeEpyS0FMMA?oc=5",
    "k": "first export small modular nuclear plant breaks ground in uzbekistan",
    "id": "a8db5ba8"
  },
  {
    "date": "2026-06-14",
    "title": "SMR 원전의 경주(Gyeongju) 유치가 국익",
    "summary": "경주(Gyeongju)로의 소형모듈원자로 유치가 국가 이익이라는 주장의 기고문.",
    "summaryLong": "매일신문 기고에서 김석기 필자는 소형모듈원자로(SMR)의 경주(Gyeongju) 유치가 국가 이익임을 주장했다. 경주(Gyeongju)는 한국 원전 기술의 중심지로서 기존 원전 기반시설과 인프라를 활용하여 SMR 국책사업 추진에 최적의 후보지라는 입장을 제시했다. 이는 한국의 SMR 상용화 및 글로벌 경쟁력 강화와 관련된 지역 정책 논의의 일부이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiYkFVX3lxTFBsVGxRR0dxQTcyc3h1NFVzS0pnblV3LTdyREN2THprT293Z0pTTHh6MHVVUVpiUzYxTmE5bW1FdWdXblpBSGRpVjBOWDlfYkhsQTc3SFpnaUJsMVlFV2RDRnhB?oc=5",
    "k": "기고 김석기 smr 원전의 경주 유치가 국익",
    "id": "6f501414"
  },
  {
    "date": "2026-06-14",
    "title": "SMR 국책사업 부지로 경주(Gyeongju)가 최적지",
    "summary": "경주(Gyeongju)가 소형모듈원자로 국책사업 부지의 최적지라는 보도.",
    "summaryLong": "경북매일은 소형모듈원자로(SMR) 국책사업 부지로 경주(Gyeongju)가 최적지라고 보도했다. 경주(Gyeongju)는 기존 원전 운영 경험, 전문 인력, 관련 기반시설 등 SMR 실증 및 상용화에 필요한 조건을 갖춘 지역으로 평가된다. 이는 한국 정부의 SMR 국책 프로그램 추진 과정에서 부지 선정 관련 논의의 일부이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiWkFVX3lxTE1WNGJXaklaVUVLSTlLbks2R2NXRHBjTWRNa1RicTRRWUtEa0xDVzZPczRfcmFQWDNVVkRHSjB4ZmdnTUVKc3hLVGE1ZVViT1ZlaFkyYXRMMGZBZw?oc=5",
    "k": "smr 국책사업 부지는 경주가 최적지다",
    "id": "2e877dc7"
  },
  {
    "date": "2026-06-13",
    "title": "미국(US) NRC 위원장, SMR 상용화 규제 장벽 제거 의지 표명",
    "summary": "미국 원자력규제위원회(NRC) 위원장이 소형모듈원자로(SMR) 상용화를 위해 규제 걸림돌을 없애겠다고 선언했다.",
    "summaryLong": "미국 원자력규제위원회(NRC) 위원장이 SMR의 상용화를 가속화하기 위해 현행 규제 체계의 장벽을 제거하겠다는 의지를 표명했다. 이는 미국 정부가 추진 중인 SMR 개발 지원 정책의 일환으로, 규제 당국의 명확한 입장 표시가 업계의 투자와 개발 확대에 신호를 주는 의미를 갖는다. SMR의 안전성 검증과 설계인증 등 규제 절차를 합리화하려는 움직임으로 해석되며, 미국의 SMR 상용화 로드맵 실현에 중요한 진전이다.",
    "cat": "기술",
    "type": "General",
    "dev": "NRC",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZEFVX3lxTE5McXU4U2xoTG5xY0t4WEJVT0N3b2dvQjBwWkhubDBVZXozd1hpRHJoRldxVFpwYmltbnRnRmlWMERDbERpTjY3TzNUSHNZRmdHVFhfMUo1Qks2NTk5SDJSRFNBR0c?oc=5",
    "k": "nrc 위원장 smr 상용화 규제 걸림돌 없앨 것",
    "id": "6396f58f"
  },
  {
    "date": "2026-06-13",
    "title": "스웨덴의 선도적 SMR 전략, 글로벌 시장 영향력 확대",
    "summary": "스웨덴의 소형모듈원자로(SMR) 개발 추진이 글로벌 시장에 미칠 영향을 분석한 보도이다.",
    "summaryLong": "스웨덴이 소형모듈원자로(SMR) 개발과 배치에서 앞서가고 있으며, 이러한 선도적 입장이 글로벌 시장을 크게 변화시킬 수 있다는 분석이 제시되었다. 스웨덴의 SMR 추진 전략과 실적은 다른 국가들의 SMR 정책 수립에 영향을 미칠 것으로 예상된다. 이는 유럽 내 저탄소 에너지 전환 가속화의 신호탄으로 평가되고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "EU",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZEFVX3lxTE1PeG4yTUhRU19ZRGNRQlN3RkRCaFY2R0U3TDlGRENDak1lcXM2SGs2WC1uWXItcHlubHZ3WFJqYWI2UVhDNlMwOVp5cTJubmpvRFFNYzZEeG1aV3lPVHh4ZEYwUUY?oc=5",
    "k": "앞서가는 스웨덴 smr 시계 글로벌 시장 뒤흔들까",
    "id": "7f0f8588"
  },
  {
    "date": "2026-06-12",
    "title": "칼럼: '탈원전'에서 '소형모듈원자로(SMR)'로의 정책 전환",
    "summary": "한국의 탈원전 정책에서 소형모듈원자로 중심 정책으로의 전환을 다룬 칼럼이다.",
    "summaryLong": "한국의 에너지 정책이 탈원전에서 소형모듈원자로(SMR) 중심으로 전환하고 있다. 이는 국제적 원전 부흥 추세와 탄소중립 목표 달성의 필요성에 따른 정책 방향 변화를 반영한다. 소형모듈원자로는 모듈식 설계로 다양한 산업 분야 적용이 가능하며 안전성이 향상되었다는 평가를 받고 있다. 정책 전환에 따라 국내 원전 산업의 새로운 성장 기회가 열릴 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZEFVX3lxTE9raVF2WGtSbHBpVTNsRFBaN3Y3cTFVdmhSUUJ6N1BlLWNzVXRJR1NZOTBoY2N0cUUwLVlnWTY1Wmo2b3pEaWV5dUxJWHFNWHdVdnlId1ZiOGRIbXhqcVYyMU5kSHE?oc=5",
    "k": "김화진 칼럼 탈원전 에서 smr 로",
    "id": "49ffa86b"
  },
  {
    "date": "2026-06-12",
    "title": "국내 SMR(소형모듈원자로) 건설주, 하반기 미국 투자 수혜주로 주목",
    "summary": "국내 소형모듈원자로 관련 건설 및 기업들이 하반기 미국의 SMR 투자 확대로 인한 수혜주로 부상할 것으로 예상된다.",
    "summaryLong": "미국의 소형모듈원자로 투자 확대로 국내 SMR 관련 건설사 및 산업 기업들의 수익 창출 기회가 증가하고 있다. 특히 하반기 미국의 정부 지원 및 민간 투자가 본격화하면서 한국 기업들의 부품 공급, 건설 참여 등이 확대될 것으로 전망된다. 이는 한국의 원전 산업 경쟁력을 국제 시장에서 입증할 수 있는 기회가 될 것으로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "smr 건설주 하반기 대미투자 수혜주로 뜨나",
    "id": "3878431b"
  },
  {
    "date": "2026-06-12",
    "title": "일본, 미국 SMR(소형모듈원자로) 프로젝트에 약 100조원 투자 계획",
    "summary": "일본이 미국의 소형모듈원자로 프로젝트에 약 100조원 규모의 투자를 추진하고 있다.",
    "summaryLong": "일본이 미국의 소형모듈원자로 개발 및 상용화 프로젝트에 약 100조원대 규모의 대규모 투자를 계획 중이다. 이는 일본의 에너지 안보 강화 및 미국과의 기술 협력을 통한 원전 산업 참여 확대를 의미한다. 이러한 투자는 미국 내 SMR 건설 가속화에 주요 자금 역할을 할 것으로 예상되며, 양국 간 에너지 협력을 심화시킬 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "JP",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMingFBVV95cUxOWWhCZ2hVSXVHVGpiTUdtcGhRYUdyRlZrQ2lSTmljTS1oNWJqbUJRdXY0el94TG52UDRrdkk0ZmtwaFRNYjhTTnl1V3hVNk9VZy1EZ3dYS3JNQkRmQnFINnI2a1p2eGEtOTV4SlJsX2FhYkdVSzNNZHU0ZnB6LVZEWURYMTdFbGVyTTBBZ2VMUUdXRUs4T043UW5JSHlLQQ?oc=5",
    "k": "smr에 100조원 투자 러트닉 상무 미일 세계 smr 선도할 것",
    "id": "aa8b69d8"
  },
  {
    "date": "2026-06-12",
    "title": "미국 상무장관, 일본 자금으로 SMR(소형모듈원자로) 투자해 2050년 원전 발전용량 4배 확대 계획 추진",
    "summary": "미국 상무장관이 일본의 자금을 활용한 소형모듈원자로 투자를 통해 2050년 원전 발전용량 4배 확대 계획을 지원한다고 발표했다.",
    "summaryLong": "미국 상무장관은 일본의 투자 자금을 활용한 소형모듈원자로 프로젝트를 통해 2050년까지 미국의 원전 발전용량을 현재의 4배 수준으로 확대할 계획을 공식 발표했다. 이는 미국의 탄소 중립 목표 달성과 에너지 안보 강화를 위한 핵심 전략이다. 일본의 대규모 투자는 이 야심찬 계획의 재정적 근간을 제공할 것으로 평가되며, 양국의 전략적 에너지 협력을 보여주는 사례이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMic0FVX3lxTE9pMGZrVmRFRmpUN3NHZmJEXzVrdG9Td21vczVDajFmcURJZ014LTZDdkhYNTN5dG1MVVV4TVF0N1lyZmZPd0F5dFBLRkkxN0tDVnl6S2ZOaUU2UWNUeVp4ZFNFei01bEJzSWJmMUpLYUUxWVE?oc=5",
    "k": "미국 상무장관 일본 자금으로 smr에 투자 2050년 원전 4배 확대 계획 지원",
    "id": "2d7518a5"
  },
  {
    "date": "2026-06-12",
    "title": "트럼프 정부, 일본 투자금으로 미국에 SMR(소형모듈원자로) 건설 추진",
    "summary": "트럼프 정부가 일본의 투자 자금을 활용하여 미국에 소형모듈원자로 건설을 추진하겠다는 입장을 밝혔다.",
    "summaryLong": "트럼프 정부는 일본의 대규모 투자 자금을 활용하여 미국 내 소형모듈원자로 건설 프로젝트를 추진할 계획을 표명했다. 이는 일본의 에너지 투자와 미국의 원전 정책이 결합된 국제 협력 사례이다. 트럼프 정부는 이를 통해 미국의 에너지 독립성 강화와 제조업 기반 확대를 동시에 달성하려는 전략으로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMigAFBVV95cUxPX1hkVENaaW5kOEx1dzZHbDN5eW40N0RoWkdJMl9ZSlhzbVEzbDJ5Y1hFTVBpUFVXNDJZd0ZTZ2JBd1QyTUFreFNQMmhQb1k0S3NNaTNKQ0FDVTEyZkJwYWp1RlNlS01JN2h6UG43djNBVUN1WU5pOVdKU0tVYlNxUg?oc=5",
    "k": "트럼프 투자금으로 미국에 원전 짓는다 소형원자로 주도",
    "id": "92ffb53b"
  },
  {
    "date": "2026-06-12",
    "title": "미국, 상업용 SMR(소형모듈원자로) 추진선 생태계 구축으로 국내 산업에 기회",
    "summary": "미국이 상업용 소형모듈원자로 추진선 산업 생태계 구축을 본격화하면서 국내 철강·조선·원전 산업에 새로운 수출 기회를 제공하고 있다.",
    "summaryLong": "미국이 상업용 소형모듈원자로 및 추진선(원자로 탑재 선박) 산업 생태계 구축을 가속화하고 있다. 이러한 시장 형성은 한국의 철강, 조선, 원자력 산업에 부품 공급, 시스템 통합, 건설 참여 등 다양한 수출 기회를 제공할 것으로 예상된다. 한국의 세계 최고 수준의 조선 및 원전 기술이 국제 시장에서 경쟁력을 발휘할 수 있는 전기가 될 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiakFVX3lxTE5GV2RicWI0TTV1dWVjeHlPQnpkWFh2UWdfbjZVajV0dEFCaktXOFhKVzE0OWpodE1VREtNM2FDejBHNzRRUmE4QzVUZmpidU9NQmN1enotR0ZwckdCaXlYVEp0R21NVkxwM0E?oc=5",
    "k": "상업용 smr 추진선 생태계 구축 국내 철강 조선 원전 산업에 새로운 기회",
    "id": "c8e7a4b5"
  },
  {
    "date": "2026-06-12",
    "title": "일본의 약 105조원 투자로 미국 SMR(소형모듈원자로) 건설, 트럼프 '세계 주도' 선언",
    "summary": "일본이 약 105조원 규모의 투자금으로 미국의 소형모듈원자로 원전 건설을 추진하고 있으며, 트럼프 대통령이 SMR의 세계 주도 입장을 표명했다.",
    "summaryLong": "일본이 미국의 소형모듈원자로 개발 및 건설 사업에 약 105조원의 대규모 투자를 진행하고 있다. 트럼프 대통령은 이러한 투자와 함께 미국이 SMR 분야에서 세계적 주도권을 확보할 것이라는 의지를 표명했다. 이는 미국의 에너지 정책 강화와 동맹국의 경제 협력이 결합된 전략적 파트너십으로 평가되며, 글로벌 원자력 시장의 판도 변화를 예고하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiVkFVX3lxTE81dTBKMjBMaHlHWlcwQUZlcndSUEd0MmZicHNNT3AtU1JJTEZ0WDlxcHB4X0NhRGdBcnZud3MxZEdxRjRjWW1naG5IM0ZVcnltZ3h4NjZR?oc=5",
    "k": "투자금 105조원으로 원전 짓는다 트럼프 smr 세계 주도",
    "id": "ee76621d"
  },
  {
    "date": "2026-06-12",
    "title": "스웨덴 Studsvik, SMR 국가 지원 신청",
    "summary": "스웨덴 원자력 기술 서비스 회사 Studsvik이 남부 지역 소형모듈원자로(SMR) 최대 1,400 MW 규모의 신규 원자력 발전 사업에 대한 정부 지원을 신청했다.",
    "summaryLong": "스웨덴의 원자력 기술 서비스 제공업체 Studsvik이 스웨덴 정부에 남부 지역 소형모듈원자로(SMR) 기반 신규 원자력 발전 사업(최대 1,400 MW 규모)에 대한 국가 보조금을 신청했다. 이는 스웨덴이 재생에너지에 더해 소형모듈원자로를 통해 탄소중립 목표 달성과 에너지 안보를 강화하려는 정책 방향을 반영한 것이다. Studsvik의 신청은 북유럽의 SMR 프로젝트 추진 확대를 시사한다.",
    "cat": "정책",
    "type": "General",
    "dev": "Studsvik",
    "region": "EU",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/studsvik-seeks-state-support-for-smrs-in-sweden",
    "k": "studsvik seeks state support for smrs in sweden",
    "id": "5d869fab"
  },
  {
    "date": "2026-06-12",
    "title": "한국·카자흐스탄, 차세대 SMR 협력 확대 및 원전 인재 양성 추진",
    "summary": "한국과 카자흐스탄이 차세대 소형모듈원자로(SMR) 개발 협력을 본격화하고 원자력 인재 양성 프로그램도 함께 추진하기로 했다.",
    "summaryLong": "한국과 카자흐스탄이 소형모듈원자로(SMR) 기술 개발 및 상용화 분야에서의 협력을 본격화하기로 합의했다. 양국은 차세대 SMR 기술 교류와 공동 연구 추진뿐만 아니라, 원자력 분야 전문 인재 양성 프로그램 구축도 함께 추진할 예정이다. 이는 중앙아시아의 에너지 안보 강화와 한국의 원자력 기술 수출 다변화 전략의 일환으로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMibkFVX3lxTE16Tm55X3FVNkM3dm1Lck9PbmktZVpxcThISlctWk1QdW5MRnJ6UHlLQ0ZYUkIwV1hHMnFINWtjaVdyZldNQUFLQkt0N0E3S3lueXBCeURyUHd3X1hRYmpXLWs3cHFBSEpnRnJGZDhR?oc=5",
    "k": "한국 카자흐스탄 차세대 smr 협력 본격화 원전 인재 양성도 추진",
    "id": "1de1e2c5"
  },
  {
    "date": "2026-06-12",
    "title": "미국, 일본 투자로 SMR 건설 구체화…'글로벌 시장 주도' 목표",
    "summary": "미국이 일본의 투자 자금을 활용하여 소형모듈원자로(SMR) 건설 사업을 구체화하고 있으며, 양국이 세계 SMR 시장 주도를 목표로 협력하고 있다.",
    "summaryLong": "미국이 일본의 투자 자금을 통해 소형모듈원자로(SMR) 건설 프로젝트를 본격적으로 추진하고 있다. 미국과 일본은 이 협력을 통해 글로벌 SMR 시장에서의 주도권 확보 및 국제 경쟁력 강화를 목표로 하고 있다. 양국의 기술·자금 협력은 SMR의 상용화 시대 도래와 국제 원자력 시장 재편을 예고한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiXkFVX3lxTE1KdHpub1hzWF9qeFAxZG5FMnpnRDhLS3hSTFVmaFFuYzVtc09zRmsweHFSWGxLS1pOZG9nNTQ3aGRESGJSMHVvZzZhS2t5bGk5VmdYdnRMdjVJdlFVSlE?oc=5",
    "k": "투자금으로 smr 건설 구체화 세계시장 주도해 양국 기여",
    "id": "43f93b32"
  },
  {
    "date": "2026-06-12",
    "title": "일본, 미국 SMR 프로젝트에 650억 달러 투자",
    "summary": "일본이 미국의 소형모듈원자로 프로젝트에 대규모 투자 계획을 발표.",
    "summaryLong": "일본이 미국의 소형모듈원자로(SMR) 프로젝트에 650억 달러 규모의 투자 계획을 공개했다. 이는 양국 간 에너지 협력을 강화하고 저탄소 전원 확보를 목표로 하는 전략적 투자로, 글로벌 SMR 시장에서 일본의 역할 확대를 의미한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMihAFBVV95cUxOLUR2WEVkb1VWcklweUNwWlA2cGd6SGFVY3RMZTByeFRFSnJrbmJEWTlNQzVzS1hHNzFIOEZWMnB3ZXBqR2RFZy00RVowa0VfYk9VcDFjU3lOTG50aFFldzAxYUpXakp5czFKcThDNlVfZjZGWUtrcFNnQjQ5R2NUcDktWTI?oc=5",
    "k": "japan invests 65 billion in u s smr projects",
    "id": "04f4ce03"
  },
  {
    "date": "2026-06-12",
    "title": "미국과 일본, SMR 개발에 협력",
    "summary": "미국과 일본이 소형모듈원자로 개발에 협력 강화.",
    "summaryLong": "미국과 일본이 소형모듈원자로(SMR) 개발 분야에서 협력을 강화하기로 합의했다. 양국 간 파트너십을 통해 SMR 기술 개발, 안전성 확보, 시장 확대를 추진하며, 이는 향후 글로벌 에너지 안보와 탄소중립 달성에 기여할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMihAFBVV95cUxQeTk5RDlXTGI4X0dzLUVlSGlNZFhjVndTbnpVMXdBZ2NDS2pSNUlQZTJ5M1lWZ3VoQTNUX2g3VXhuN0xQdWc1WUpBUlFSTHFCbTMzSTBYRFBGYUZVeW4tOUdla2JyWF9Ob0dZYnZna1dtZVZXTVUxckJYa0RMa2RjczE5RUI?oc=5",
    "k": "u s and japan partner on smr development",
    "id": "49274fde"
  },
  {
    "date": "2026-06-12",
    "title": "미국과 일본, AI 전력 수요 대응으로 SMR 개발 협력",
    "summary": "미국과 일본이 인공지능(AI) 산업의 증가하는 전력 수요를 충당하기 위해 소형모듈원자로 개발에 협력.",
    "summaryLong": "미국과 일본이 인공지능(AI) 산업 확산으로 인한 증가하는 전력 수요에 대응하기 위해 소형모듈원자로(SMR) 개발 협력을 강화하고 있다. 양국은 SMR의 신속한 배치, 유연한 전력 공급, 탄소 저감 특성을 활용하여 차세대 에너지 솔루션을 제공하려 하며, 이는 글로벌 에너지 시장의 구조 변화를 반영한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMipAFBVV95cUxOUjdLUXdyQnhCSHZzSWlsOW81bXFockZSd09MVjd1Z3dhWElRZEhYcTlrZllBZnVPTy1jYTNpNWxlV2p5clFPdXR4Y3hyc3FCQ0pNQ1p5TXI2WkdrRUkxd0tYdFlndGlHZUpwQUtGalZhX01IMjlVRms1Rm9EaVZLbTJ4cTlMZ3N0Tmp6c2lqMDVyNGtvM3FFc3lBRTFvMXZaZHU4bg?oc=5",
    "k": "us japan join forces on small modular reactors amid ai power crunch",
    "id": "08fd15dc"
  },
  {
    "date": "2026-06-12",
    "title": "미국, SMR 추진 선박 개발 검토…한국 조선·원전 기회",
    "summary": "미국이 소형모듈원자로 추진 선박 개발을 검토 중으로, 한국 조선 및 원전 산업에 기회 제공.",
    "summaryLong": "미국이 소형모듈원자로(SMR)를 추진 시스템으로 하는 선박 개발을 검토하고 있다. 이는 해양 산업과 원자력 기술의 결합을 통한 새로운 시장 창출을 의미하며, 한국의 조선 기술과 원전 기술을 활용할 수 있는 기회를 제공할 수 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiXkFVX3lxTE1aRWlNR0lTNUZuYkZ3V0c3NFlRSzBkbEVWbnNGZlpiWmotZTRCWWhiMWFHQzhNM1NHNk5wU0NRWThaWHlUYnROSGNQUk5oaGpuOFBwWWo3WXVPQ2RXSVE?oc=5",
    "k": "smr 추진 선박 개발 검토 k 조선 원전 기회",
    "id": "2dd3ff4e"
  },
  {
    "date": "2026-06-12",
    "title": "일본, 대미투자 중 10조 엔을 소형모듈원자로 건설에 사용",
    "summary": "일본이 미국에 대한 투자 중 10조 엔 규모를 소형모듈원자로 건설에 배치할 계획.",
    "summaryLong": "일본이 미국에 대한 투자 계획 중 약 10조 엔(약 680억 달러에 해당) 규모를 소형모듈원자로(SMR) 건설에 투입하기로 결정했다. 이는 일본의 전략적 에너지 정책과 미국과의 경제·에너지 협력 강화를 보여주며, 글로벌 SMR 시장의 급속한 확대를 견인할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "대미투자 중 10조엔 소형모듈원자로 건설에 사용 방침",
    "id": "1c52f7e9"
  },
  {
    "date": "2026-06-11",
    "title": "미국 에너지부(DOE), 오클로(Oklo)의 오로라(Aurora) 원자로 예비안전분석(PDSA) 승인",
    "summary": "미국 에너지부(DOE)가 오클로(Oklo)의 오로라(Aurora) 원자로에 대한 예비안전분석(PDSA)을 승인했으며, 이는 아이다호 국립연구소 건설을 향한 인허가 과정의 중요한 이정표이다.",
    "summaryLong": "미국 에너지부(DOE)는 오클로(Oklo)의 오로라(Aurora) 소형모듈원자로에 대한 예비안전분석(PDSA) 승인을 완료했다. 이는 아이다호(Idaho) 국립연구소 내 건설 예정인 오로라 원자로의 인허가 과정에서 핵심적인 진전이다. 예비안전분석 승인은 설계 안전성이 확보되었음을 의미하며, 후속 상세설계 및 건설 인허가 단계로 나아갈 수 있는 기초를 마련했다. 이는 미국의 SMR 상용화 일정을 앞당기는 데 중요한 역할을 할 것으로 기대된다.",
    "cat": "인허가",
    "type": "SFR",
    "dev": "Oklo",
    "region": "US",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/doe-approval-milestone-for-oklo-reactor",
    "k": "doe approval milestone for oklo reactor",
    "id": "e9c89609"
  },
  {
    "date": "2026-06-11",
    "title": "트럼프 행정부 환경청(EPA) 청장, 소형모듈원자로(SMR) 확대 지지 표명",
    "summary": "미국 환경청(EPA) 청장 리 젤딘(Lee Zeldin)이 소형모듈원자로(SMR) 확대를 지지하면서 관련 기업들의 주가가 상승했다.",
    "summaryLong": "트럼프 행정부의 환경청(EPA) 청장 리 젤딘(Lee Zeldin)이 소형모듈원자로(SMR) 산업 확대를 지지한다는 입장을 표명했다. 이 발언으로 인해 소형모듈원자로(SMR) 관련 기업들의 주가가 상승하는 반응을 보였다. 이는 트럼프 행정부가 청정에너지 및 핵심 에너지 기술로서 소형모듈원자로(SMR)에 긍정적인 정책 환경을 제공할 것으로 예상되는 신호로 해석된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "smr nne oklo gain after trump s epa chief lee zeldin touts small modular reactor buildout",
    "id": "78738556"
  },
  {
    "date": "2026-06-11",
    "title": "한국 첫 혁신형 소형모듈원자로(i-SMR) 경주 유치 제안",
    "summary": "국내 첫 혁신형 소형모듈원자로(i-SMR) 구축 후보지로 경주 지역을 제안하는 기고문이 제시되었다.",
    "summaryLong": "한국의 첫 혁신형 소형모듈원자로(i-SMR) 건설 후보지로 경주를 제안하는 주장이 제기되었다. 경주는 기존 원자력 시설과 기반시설이 갖춰진 지역으로, 소형모듈원자로(i-SMR) 구축에 유리한 입지로 평가되고 있다. 이는 한국의 소형모듈원자로(i-SMR) 실증화 및 상용화 계획과 관련된 지역 선정 논의의 일환이다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiWEFVX3lxTE1IS0hoUmhURFJtNWxEbm9kTVJnTXNnQ1R6Y2YzWWNnZ1R2NENBRDRadnN6ajNNWFlkOWIxOW1QeDJFeEl6WVB1akp1SFl0bFJuOW1FTlJPYTA?oc=5",
    "k": "황대욱 특별기고 대한민국 첫 i smr 답은 경주다",
    "id": "72a92cd5"
  },
  {
    "date": "2026-06-11",
    "title": "용융염 원자로 탑재 선박 개념설계 승인",
    "summary": "로이드선급(Lloyd's Register)과 미국선급협회(American Bureau of Shipping)가 용융염 원자로 기반 선박 추진 시스템의 개념설계를 승인했다.",
    "summaryLong": "선박 분류 기관인 로이드선급(Lloyd's Register)과 미국선급협회(American Bureau of Shipping)가 용융염 원자로를 탑재한 선박의 추진 시스템 개념설계를 공식 승인했다. 용융염 원자로는 고온의 용융염을 냉각재로 사용하는 차세대 원자로 기술로, 선박 추진용 원자력 시스템으로의 적용 가능성을 보여준다. 이 승인은 원자력 추진선 개발의 중요한 진전을 의미하며, 향후 실증 및 상용화 추진의 근거가 될 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/nuclear-powered-ship-conceptual-designs-approved",
    "k": "nuclear powered ship conceptual designs approved",
    "id": "4885c07a"
  },
  {
    "date": "2026-06-11",
    "title": "네덜란드 오버레이설주(Overijssel), 소형모듈원자로(SMR) 건설 적지 식별 완료",
    "summary": "네덜란드의 오버레이설주(Overijssel)가 Tractebel과 함께 소형모듈원자로(SMR) 건설 적지를 파악하는 초기 연구를 완료했다.",
    "summaryLong": "네덜란드 오버레이설주(Overijssel)가 컨설팅사 Tractebel과 협력하여 소형모듈원자로(SMR) 건설을 위한 적지 식별 초기 연구를 마쳤다. 이 연구는 지역 내 지리적, 인프라, 환경 조건을 종합적으로 평가하여 소형모듈원자로(SMR) 구축에 적합한 입지를 선정하는 작업이었다. 해당 결과는 네덜란드의 에너지 전환 및 탄소중립 목표 달성을 위한 소형모듈원자로(SMR) 도입 계획의 구체화 단계로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "EU",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/study-identifies-potential-smr-sites-in-dutch-province",
    "k": "study identifies potential smr sites in dutch province",
    "id": "6567c110"
  },
  {
    "date": "2026-06-11",
    "title": "소형모듈원자로(SMR), 항만 전력 공급원으로서의 활용 검토",
    "summary": "소형모듈원자로(SMR)를 항만 시설의 전력 공급원으로 활용하는 방안이 연구되고 있다.",
    "summaryLong": "소형모듈원자로(SMR)를 항만 시설에 필요한 전력 공급의 새로운 에너지원으로 활용하는 방안에 대한 연구가 진행 중이다. 항만은 대규모 지속적인 전력 수요를 가진 산업 시설로, 소형모듈원자로(SMR)의 컴팩트한 설계와 높은 전력 출력이 적합한 응용 분야로 평가된다. 이는 항만의 탄소 중립화 및 에너지 자급률 향상을 목표로 하는 계획의 일부이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMibkFVX3lxTFBWb3RfcUdSUWRCOU9uNmZSVlNEUkJtby1uTXZKYkplQm52SGowd1dmYWt2aDBMU05IZGQ0SXNkTlUtbzBqeW1KaE9ONWZoMzhyN01ncTF1Y3doREh6ME5hTHcwOXhYZG5sWWd6blBB?oc=5",
    "k": "smr studied to power ports",
    "id": "f25fda11"
  },
  {
    "date": "2026-06-11",
    "title": "우즈베키스탄, 첫 소형모듈원자로 발전소 프로젝트 공식 착수",
    "summary": "우즈베키스탄이 첫 소형모듈원자로(SMR) 발전소 프로젝트를 공식적으로 착수했다.",
    "summaryLong": "우즈베키스탄이 자국의 첫 소형모듈원자로(SMR) 발전소 구축 프로젝트를 공식 착수하였다. 이는 중앙아시아 지역에서 소형모듈원자로(SMR) 도입을 추진하는 국가의 확대를 의미한다. 우즈베키스탄의 소형모듈원자로(SMR) 프로젝트는 지역 에너지 안보와 전력 공급 안정화를 목표로 하며, 향후 중앙아시아의 원자력 에너지 확산에 영향을 미칠 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMioAFBVV95cUxQZGVPYXlHMzNVVHZHMC1UaXZtQVFVWlA3Qkx4MUFsLXN6ZTE5MmtfdkRBR2pCWmRQMWRMMWEyQ2dLZFFWMkwzUExYdlNZdE0zTF9lak1Zc2pza0FuaDlxcTdrN3VhNlJLNmNJVFg2YVFmYU4tV0RhSndNTkstYkl5Z08wX0d2N2NFWDRpdzh5M0lLbURzank2QzU1ejlsWEdR?oc=5",
    "k": "uk media uzbekistan launches first small modular reactor npp project",
    "id": "dac2b89e"
  },
  {
    "date": "2026-06-11",
    "title": "HD현대, 소형모듈원자로(SMR)·원자력 추진선으로 '원자력 기업' 변신 추진",
    "summary": "HD현대가 소형모듈원자로(SMR)와 원자력 추진선 사업에 본격 진출하며 기업 포트폴리오 전환을 추진하고 있다.",
    "summaryLong": "조선해양 대기업 HD현대가 소형모듈원자로(SMR)와 원자력 추진선 사업 분야로 전략적 진출을 추진하면서 기업의 포트폴리오를 다각화하고 있다. 이는 해운·조선 산업의 탄소중립 화와 차세대 에너지 기술 확보 전략의 일환이다. HD현대의 원자력 사업 진출은 한국 조선업의 기술 고도화 및 글로벌 시장 경쟁력 강화를 의미하며, 향후 원자력 추진선 건조 사업의 성장 가능성을 시사한다.",
    "cat": "기술",
    "type": "General",
    "dev": "HD현대",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "hd현대의 원자력 기업 변신 smr 원자력 추진선 선점 나선다",
    "id": "b131c66e"
  },
  {
    "date": "2026-06-11",
    "title": "정부 에너지 정책담당자, 유럽연합(EU)과의 소형모듈원자로(SMR) 논의로 글로벌 정책 변화 반영",
    "summary": "정부 에너지 정책담당자 김용범이 유럽연합(EU)과의 소형모듈원자로(SMR) 협력 논의를 언급하며 변화된 글로벌 에너지 환경에 대응하는 정책 추진을 시사했다.",
    "summaryLong": "정부의 에너지 정책담당자 김용범이 유럽연합(EU)과의 소형모듈원자로(SMR) 협력 논의 진행을 언급했다. 이는 글로벌 차원의 소형모듈원자로(SMR) 도입 확대 추세에 한국이 적극 대응하고 있음을 보여준다. 한국 정부는 국제 협력을 통해 소형모듈원자로(SMR) 기술 표준화, 규제 협력, 시장 진출 등에 대한 정책을 추진할 것으로 예상되며, 이는 한국 소형모듈원자로(SMR) 산업의 글로벌 경쟁력 강화로 이어질 전망이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMigAFBVV95cUxNTGg3Y3kyT281NFdTR3JleVJJV3B2eVd0dEhVa24yTks0SnJycGxrQ0o4ZUVPdHYtQjlEN1RZd0dJVVhfTTVna05GR1pfMnVkdGRXWjlmUjZYR29kaTlqYTVmMk1zRGtkemFCR2wwYnNYUlA4Ynk1T0l6bjhQZHdTaw?oc=5",
    "k": "김용범 eu와 소형모듈원전 논의 변화된 글로벌환경 반영",
    "id": "2995c1e8"
  },
  {
    "date": "2026-06-11",
    "title": "미국, 상업용 원자력 추진선 시대 준비 본격화로 해상 소형모듈원자로(SMR) 생태계 구축",
    "summary": "미국이 상업용 원자력 추진선 산업화 준비를 본격화하며 해상 소형모듈원자로(SMR) 생태계 구축에 나서고 있다.",
    "summaryLong": "미국이 상업용 원자력 추진선 개발을 위한 본격적인 준비에 착수하고 있다. 이는 해양 운송 부문에 원자력 기술을 적용하려는 전략적 시도이다. 관련 산업 생태계 구축을 통해 소형모듈원자로(SMR) 기술의 새로운 활용 분야를 개척하고 있다. 이러한 움직임은 탄소 감축과 해양 운송의 친환경화를 동시에 추진하는 의미를 가진다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMic0FVX3lxTE5SRFp1OTIyeHpQcWJxM1Z2akFRWWZhOS1PWU50YUV6OHJwNE5XNDRjLUQ1bUNScjlnVXA2cVUyOERIbTlBQTZ6dU9LQU0yTzlLQ0NRYWJCOFlvYWIzVGVTdEoyelZ4WllkZ1hVR1pCZlVmajg?oc=5",
    "k": "상업용 원자력 추진선 시대 준비 본격화하는 미국 해상 smr 생태계 구축 나섰다",
    "id": "8bd32060"
  },
  {
    "date": "2026-06-11",
    "title": "원자력·조선·해양학회, 소형모듈원자로(SMR) 선박 개발 지지 공동선언",
    "summary": "원자력·조선·해양 분야 학회들이 소형모듈원자로(SMR) 기반 선박 개발을 지지하는 공동선언을 발표했다.",
    "summaryLong": "원자력, 조선, 해양학 분야의 주요 학회들이 소형모듈원자로(SMR) 기반 선박 개발 사업에 대한 지지 입장을 공식화했다. 이는 학계와 산업계가 원자력 추진선 개발의 중요성을 함께 인식하고 있음을 보여준다. 다학제적 협력을 통해 기술 개발과 실용화를 앞당기려는 의도로 해석된다. 공동선언은 정부 정책 수립과 산업 투자 유도에 긍정적 신호로 작용할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZkFVX3lxTE85ZDNrTXpUZThnV083MjNSVms2aGcxeDVXM3NPVDhEdFZwcnZKaXlGR1Y5UUJrOUdzQ1RvWjNnakYwY1YyYjFjQ3NzUEZxb1kzNy11cE9jUGxTZl9OSW1MdlBGd0ZVUQ?oc=5",
    "k": "원자력 조선 해양학회 smr 선박 개발 지지 공동선언",
    "id": "e356aa7b"
  },
  {
    "date": "2026-06-11",
    "title": "네덜란드 오버레이설(Overijssel), 소형모듈원자로(SMR) 예비 입지 후보에 대한 연구 결과 발표",
    "summary": "네덜란드 오버레이설이 소형모듈원자로(SMR) 입지 예비 후보지에 대한 연구 결과를 발표했다.",
    "summaryLong": "네덜란드의 오버레이설(Overijssel) 지역이 소형모듈원자로(SMR) 건설을 위한 예비 입지 후보지에 대한 조사 결과를 공개했다. 이는 유럽 지역에서 소형모듈원자로(SMR) 도입을 구체화하려는 움직임을 반영한다. 입지 선정 연구는 환경, 안전, 인프라 등 다양한 요소를 검토하는 과정이다. 향후 네덜란드의 소형모듈원자로(SMR) 실증 또는 상용화 사업 추진의 기초가 될 것으로 보인다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "EU",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZEFVX3lxTE5qb2RvQ2ozMGdPVVJYamtnZ3AtdWE4N251TlN0THdZQ3RqcVlqbHctckM4d0IybFdEdVpDUEVPakZhSS12Rk1XU0RlYV9vM3hZWU5saVNmdjgxc0JzTUlSbDdaSGw?oc=5",
    "k": "네덜란드 오버레이셀 smr 예비 후보지 연구 결과 발표",
    "id": "c844949d"
  },
  {
    "date": "2026-06-11",
    "title": "미국, 일본 자금으로 소형모듈원자로(SMR) 건설 추진…100조원 규모 투자 검토",
    "summary": "미국이 일본의 자금을 활용하여 소형모듈원자로(SMR) 건설을 추진하고 있으며, 약 100조원 규모의 투자가 검토 중이다.",
    "summaryLong": "미국이 일본의 자본을 활용하여 소형모듈원자로(SMR) 건설 사업을 진행하고 있다. 양국이 협력하여 약 100조원 규모의 대규모 투자를 검토하고 있는 상황이다. 이는 일본의 에너지 안보 강화와 미국의 원자력 기술 활용이라는 양국의 전략적 이해가 일치한 결과이다. 향후 구체적인 투자 규모 확정과 건설 일정 수립을 통해 미국 내 소형모듈원자로(SMR) 산업이 본격화될 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiXEFVX3lxTE1QQVczMnljSU1RQVBhRUVyRVpxUmNzc2psSHVEN3FaX1NabXN6eEJvTlRTT1JHNG14UTJNck83NFQ1ZEgySENrWUV5QXJDbng5V1RPQWtUcWxZbUNh?oc=5",
    "k": "일본 돈으로 원전 짓겠다 smr에 100조원 투자 검토",
    "id": "fb5cd40a"
  },
  {
    "date": "2026-06-10",
    "title": "홀텍(Holtec), 오이스터 크릭(Oyster Creek) 부지에 4기 소형모듈원자로(SMR) 검토",
    "summary": "홀텍(Holtec) 인터내셔널이 뉴저지주 오이스터 크릭(Oyster Creek) 폐지에 4기의 소형모듈원자로(SMR) 건설을 검토하고 있다.",
    "summaryLong": "미국의 소형모듈원자로(SMR) 개발사인 홀텍(Holtec) 인터내셔널이 뉴저지주에 위치한 폐지인 오이스터 크릭(Oyster Creek)에 4기의 소형모듈원자로(SMR) 건설을 추진 중이다. 이는 기존 폐지를 재활용하여 새로운 원자력 인프라를 구축하는 사례이다. 홀텍(Holtec)은 미국 내 여러 소형모듈원자로(SMR) 프로젝트를 추진하고 있는 주요 업체이다. 해당 프로젝트가 실현될 경우 미국의 원자력 발전 용량 확대와 지역 경제 활성화에 기여할 것으로 예상된다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "Holtec",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "holtec considers 4 nuclear reactors for oyster creek site in lacey",
    "id": "e42080ca"
  },
  {
    "date": "2026-06-10",
    "title": "BN-1200 고속로, 2027년 건설 착수 목표",
    "summary": "러시아의 나트륨냉각 고속중성자로(BN-1200)가 벨로야르스크에서 2027년 건설 착수를 목표로 진행 중이다.",
    "summaryLong": "러시아가 개발 중인 나트륨냉각 고속중성자로(BN-1200)는 벨로야르스크 지역에서 2027년의 건설 착수를 목표로 하고 있다. 나트륨냉각 고속중성자로(BN-1200)는 차세대 원자로 기술로서 높은 효율성과 안전성을 갖춘 설계이다. 러시아는 핵연료 재활용과 방사성 폐기물 감량이 가능한 고속로 기술 개발을 국가 정책으로 추진하고 있다. 2027년 착공이 실현될 경우 러시아의 원자력 산업 강화와 고속로 기술의 실증화에 중요한 이정표가 될 것으로 보인다.",
    "cat": "기술",
    "type": "SFR",
    "dev": "",
    "region": "EU",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/bn-1200-targeted-for-construction-start-in-2027",
    "k": "bn 1200 targeted for construction start in 2027",
    "id": "05137f83"
  },
  {
    "date": "2026-06-10",
    "title": "일본-카자흐스탄, 고속로 협력 확대",
    "summary": "일본원자력연구개발기구(JAEA)와 카자흐스탄 국가핵심센터가 고속로 핵심 안전 실험의 새로운 협력 단계에 합의했다.",
    "summaryLong": "일본원자력연구개발기구(JAEA)와 카자흐스탄의 국가핵심센터가 고속로 기술 협력을 확대하기로 합의했다. 양 기관은 고속로의 핵심 안전 실험 수행을 중심으로 협력 관계를 한 단계 강화하기로 결정했다. 이러한 협력은 국제적 차원에서 차세대 원자력 기술 개발을 촉진하려는 움직임이다. 고속로 기술의 안전성 검증과 국제적 실증을 통해 글로벌 원자력 산업의 발전을 도모하는 의미를 갖는다.",
    "cat": "기술",
    "type": "SFR",
    "dev": "",
    "region": "JP",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/japan-kazakhstan-extend-cooperation-on-fast-reactors",
    "k": "japan kazakhstan extend cooperation on fast reactors",
    "id": "501d642c"
  },
  {
    "date": "2026-06-10",
    "title": "딥 피션(Deep Fission), 기업공개(IPO) 앞두고 공모 규모 72% 축소",
    "summary": "소형모듈원자로(SMR) 개발사 딥 피션(Deep Fission)이 4,300만 달러 규모 기업공개(IPO)를 앞두고 공모 규모를 대폭 줄였다.",
    "summaryLong": "캐나다의 소형모듈원자로(SMR) 개발사인 딥 피션(Deep Fission)이 기업공개(IPO) 과정에서 공모 규모를 당초 계획 대비 72% 축소하여 4,300만 달러 규모로 조정했다. 이는 소형모듈원자로(SMR) 산업의 자금 조달 환경이 기대보다 어려운 상황을 반영한다. 딥 피션(Deep Fission)을 포함한 신생 원자력 기업들은 기술 실증과 상용화 단계 진입을 위해 충분한 자금이 필요한 상황이다. 공모 규모 축소는 투자자의 신중한 시장 심사와 함께 소형모듈원자로(SMR) 산업의 성숙도 및 수익성 검증이 남아 있음을 보여준다.",
    "cat": "투자",
    "type": "General",
    "dev": "Deep Fission",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "small modular nuclear reactor developer deep fission slashes deal size by 72 ahead of 43 million ipo",
    "id": "c51eb076"
  },
  {
    "date": "2026-06-10",
    "title": "의견: 소형모듈원자로(SMR)는 전력 위기의 직접 해결책은 아니지만 미래를 형성할 수 있다",
    "summary": "소형모듈원자로(SMR)의 장단점과 에너지 산업에서의 역할에 관한 전문가 의견 기사.",
    "summaryLong": "소형모듈원자로(SMR) 기술이 현재의 에너지 문제 해결책으로 기대를 모으고 있다. 본 의견 기사는 SMR의 기술적 잠재력과 현실적 한계를 균형있게 분석한다. SMR이 단기적으로 전력 위기를 완전히 해결하기는 어렵지만, 장기적으로 에너지 산업의 지속가능한 발전을 주도할 수 있다는 점을 강조한다. 향후 기술 혁신과 산업 성숙도 제고가 핵심 과제로 지적된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "opinion small modular reactors may not solve the power crisis but they could shape the future",
    "id": "029cfd82"
  },
  {
    "date": "2026-06-10",
    "title": "미국(USA)의 원전 르네상스로 경북 동해안에 소형모듈원자로(SMR)·수소산업 기회 확산",
    "summary": "미국의 원전 르네상스 확대가 국내 경북 동해안 지역의 소형모듈원자로(SMR) 및 수소산업 진출 기회를 제공하고 있다.",
    "summaryLong": "미국에서 원자력 에너지의 재부흥이 일어나면서 관련 산업 생태계가 확대되고 있다. 이러한 글로벌 추세는 한국의 경북 동해안 지역에 소형모듈원자로(SMR) 기술과 수소산업 진출의 새로운 기회를 제공하고 있다. 경북 동해안은 기존 원전 인프라와 산업기반을 활용해 SMR 제조 및 수소 생산 거점으로 발전할 가능성이 높다. 국가 차원의 전략적 지원과 지역 산업 육성이 경쟁력 확보의 핵심이 될 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMib0FVX3lxTFA0U1ZhSkZsUjhRZEpSQUE1R0syQjFhd0FJSFhwbElMNXdEdm5INGdoN0xMSFR4bG81Rlhpck9vY1psbUp5ai1XQ051MzJXU1oxc0YwSVlOQXI4RFd2Y3ZUN3FZVURmaTFrbmRYeVljdw?oc=5",
    "k": "원전 르네상스에 들썩이는 경북 동해안 smr 수소산업 새 기회",
    "id": "a7c614c8"
  },
  {
    "date": "2026-06-10",
    "title": "홀텍(Holtec), 퇴역 원전 부지에 소형모듈원자로(SMR) 4기 추진으로 인공지능 전력 공급",
    "summary": "홀텍(Holtec)이 퇴역 원전 부지를 재활용하여 소형모듈원자로(SMR) 4기를 건설하고 인공지능 데이터센터의 전력 수요를 충족시키려는 프로젝트를 추진하고 있다.",
    "summaryLong": "미국의 원전 기업 홀텍(Holtec)이 폐기된 원자력발전소 부지를 활용한 소형모듈원자로(SMR) 건설 사업을 추진 중이다. 이 프로젝트는 4기의 SMR을 통해 인공지능(AI) 데이터센터의 급증하는 전력 수요를 해결하는 것을 목표로 한다. 퇴역 원전 부지의 재활용은 기존 인프라 활용, 규제 승인 절차 간소화, 지역경제 활성화 등 여러 측면에서 경제성과 효율성을 높일 수 있다. 이러한 사업은 SMR의 실질적 상용화 첫 사례로서 산업 발전에 중요한 의미를 갖는다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "Holtec",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiiAFBVV95cUxQRU9UZ0V2ODRJMHVObnIwc0QxMzY1c1pLc043WXhzb2xmd0FWTTREWjFpeURweXZPOEFscDV2a2lyS0VaZHlLT005bzNyeVBzQ0xGSW5WYTd2U2VkQk1fQmJTeXZ4QmVrdVJXQ294aHVDV0JzWWlDSGk2am5vYW5UNm9BRm0yTFEw?oc=5",
    "k": "smr 4기 부지 재활용 승부수 홀텍 퇴역 원전으로 ai 전력 병목 뚫는다",
    "id": "47b2c4b1"
  },
  {
    "date": "2026-06-10",
    "title": "바다 위의 소형모듈원전(SMR), 해양 패권과 에너지 안보의 새 지평 열다",
    "summary": "소형모듈원자로(SMR)의 해양 수송·운영 기술이 해양 패권과 에너지 안보의 새로운 영역을 창출하고 있다.",
    "summaryLong": "소형모듈원자로(SMR) 기술이 해양 플랫폼, 선박 추진, 해양 기지 전력 공급 등으로 확산되면서 해양 에너지 안보의 새로운 패러다임이 형성되고 있다. 이는 해양 자원 개발, 극지 탐사, 도서 지역 전력 공급 등 다양한 해양 활동의 에너지 독립성을 강화한다. SMR의 해양 활용은 전 지구적 탄소중립 달성과 더불어 국가 에너지 자주권 강화의 전략적 수단으로 부각되고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZEFVX3lxTE5PQUdaQ2JxMGowVHhJMW5Sa01IdlhtZ0steWlOT0dtNGtWR1NGNnVkLU96VmJkclREU3pzN29UMEIwLW56MFM2Z041ZWlBN3pyaXM5R2VYZm83bEYtZlBXR2V0d0Q?oc=5",
    "k": "바다 위 소형모듈원전 smr 해양 패권과 에너지 안보의 새 지평을 열다",
    "id": "e2c7253a"
  },
  {
    "date": "2026-06-10",
    "title": "용융염원자로(MSR) SMR 적용 자동차운반선 개념설계, 로이드선급(Lloyd's Register) 기본승인 획득",
    "summary": "용융염원자로 기반 소형모듈원자로를 추진 동력으로 하는 자동차운반선 개념설계가 로이드선급으로부터 기본승인을 획득했다.",
    "summaryLong": "용융염원자로(MSR) 기술을 적용한 소형모듈원자로(SMR) 추진 자동차운반선의 개념설계가 국제선급 로이드선급(Lloyd's Register)으로부터 기본승인(AIP, Approval in Principle)을 얻었다. 이는 SMR의 해상 수송 및 에너지 공급 활용 가능성을 입증하는 중요한 마일스톤으로, 향후 상세설계 및 건조 단계로의 진전을 의미한다. 용융염 기반 SMR은 높은 냉각 효율과 안전성을 갖춘 첨단로 기술로 평가받고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiakFVX3lxTFBuZ0Z0d1FBT01DWVN5WFB3WDNLNXFNS0xVREloUHk4RnFDdS1wWjBOX1RqR1Myc3QzdVBGSDR0eXBPYWxCdUpETzNDMllhYVJsM2gwQmVmdXZ6TkRzSlZmOE5pOHNGTndIcXc?oc=5",
    "k": "용융염원자로 smr 적용 자동차운반선 개념설계 로이드선급 기본승인 획득",
    "id": "6d2b8afe"
  },
  {
    "date": "2026-06-10",
    "title": "SMR 초도호기 인허가 진행 중이나 전용 규제 체계 구축 지체",
    "summary": "SMR 초도호기 인허가 절차가 시작되었지만 SMR 전용 규제 체계 마련이 지연되고 있다.",
    "summaryLong": "한국(Korea) SMR 초도호기에 대한 인허가 심사가 본격 진행되고 있으나, 소형모듈원자로의 특성을 반영한 전용 안전규제 체계 구축이 아직 미흡한 상황이다. 기존 대형 원전 규제 체계와는 차별화된 SMR 맞춤형 규제 기준의 부재가 인허가 절차의 병목이 될 우려가 제기되고 있으며, 이는 규제 당국과 산업계 간 협력 강화의 필요성을 드러낸다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "smr 초도호기 인허가 돌입했는데 전용 규제 체계 구축은 하세월",
    "id": "17e01fd4"
  },
  {
    "date": "2026-06-10",
    "title": "경남도(Gyeongnam Province), 원전 투자펀드 첫 주인공 선정으로 글로벌 SMR 시장 선점 추진",
    "summary": "경남도가 원전 투자펀드의 첫 지원 대상을 선정하여 글로벌 SMR 시장 진출을 본격화한다.",
    "summaryLong": "경남도(Gyeongnam Province)가 설립한 원자력 투자펀드의 첫 주인공(지원 대상)을 선정하였으며, 이는 소형모듈원자로(SMR) 분야 기업의 성장을 금융으로 지원하여 글로벌 SMR 시장 선점을 추진하는 전략의 일환이다. 지역 기반 투자펀드를 통해 SMR 관련 기업의 기술 개발, 사업화, 국제 경쟁력 강화를 촉진하려는 의도로, 지역 경제 활성화와 국가 원자력 산업 경쟁력 제고를 동시에 목표로 하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "경남도 원전 투자펀드 첫 주인공 선정 글로벌 smr 시장 선점 시동",
    "id": "ca60c255"
  },
  {
    "date": "2026-06-10",
    "title": "한-EU(유럽연합) 경제안보·에너지 고위급 대화 출범, SMR 등 저탄소 협력 추진",
    "summary": "한국과 유럽연합(EU)이 경제안보 및 에너지 분야 고위급 대화를 시작하며 소형모듈원자로(SMR) 등 저탄소 에너지 협력을 추진하기로 했다.",
    "summaryLong": "한국과 유럽연합(EU)이 경제안보와 에너지 분야의 협력을 강화하기 위해 고위급 정책 대화를 공식 출범시켰다. 양측은 소형모듈원자로(SMR) 등의 첨단 저탄소 에너지 기술 협력을 주요 의제로 추진하기로 합의했다. 이는 글로벌 에너지 전환 시대에 한국과 EU가 SMR을 포함한 핵심 미래 기술 협력의 기틀을 마련한 의미있는 정책 합의이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiW0FVX3lxTE1Ld1YwclB6emwxZ3EyN0FQTmhKcW1pTGdiX1ZpN2ZKMm5oUVNmWU5FaG0wbC1KV2E2TEdGekJPQXBjbUdfeFBXQzJPQ0hUVkhJX21JalQzUEJ5M1U?oc=5",
    "k": "한 eu 경제안보 에너지 고위급 대화 출범 smr 등 저탄소 협력 추진",
    "id": "982a1f45"
  },
  {
    "date": "2026-06-10",
    "title": "첨단 소형모듈원자로(SMR) - 미국 에너지부(DOE)",
    "summary": "미국 에너지부(DOE)에서 제공하는 첨단 소형모듈원자로(SMR) 관련 정보 페이지다.",
    "summaryLong": "미국 에너지부(Department of Energy, DOE)는 첨단 소형모듈원자로(Advanced Small Modular Reactors, SMR) 분야에 대한 정책 정보 및 기술 자료를 공식 웹사이트를 통해 제공하고 있다. 이는 미국 연방 정부의 SMR 산업 육성 정책의 일환이며, 기술 정보 제공을 통해 산업 생태계 형성을 지원하는 것으로 볼 수 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "DOE",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMib0FVX3lxTFBPVUJRRUU2RXBmUU5TN2NVc21mVFVaZ0NscGpnVk84WWZHV2Mxd1I2VmpldkJCUEt5a2xpQ0s0VWtZeW5waDYwRXJ5NkZBNUZtcC1JWURVT0czNlVxbFkwZmNNdThHd1dNejVQZEtaMA?oc=5",
    "k": "advanced small modular reactors smrs",
    "id": "9e2216fc"
  },
  {
    "date": "2026-06-10",
    "title": "첨단 소형모듈원자로(SMR)의 4가지 핵심 이점 - 미국 에너지부",
    "summary": "미국 에너지부(DOE)가 첨단 소형모듈원자로(SMR)의 4가지 주요 이점을 설명하고 있다.",
    "summaryLong": "미국 에너지부(Department of Energy, DOE)는 첨단 소형모듈원자로(Advanced Small Modular Reactors, SMR)의 4가지 핵심 이점을 정리하여 공개했다. 이는 SMR 산업 홍보 및 정책 당위성 설명의 일환으로, 정부 차원에서 SMR 기술의 가치와 잠재력을 강조하는 것으로 해석된다. 미국 정부의 에너지 정책에서 SMR이 전략적 중요성을 갖고 있음을 보여준다.",
    "cat": "기술",
    "type": "General",
    "dev": "DOE",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiiAFBVV95cUxQMnljRWtldnlaa1J4SFBqSXY3RDgtSlZMdzJaX3R4OW1BYjV6NlR6VDNFZEZnb2NXMlkxUmM2emRKbTFqUzhFeTFoTk1QaWlucFUtWko0Z2J5NklwUUYzZlgtbF9tMDRYb2lncTJOM25GcHpxbEFRY3BkTTVpcTRYV20zWmtOTGdP?oc=5",
    "k": "4 key benefits of advanced small modular reactors",
    "id": "252a1f05"
  },
  {
    "date": "2026-06-10",
    "title": "UAE 국영 DP월드, 루마니아 항구에 SMR 추진",
    "summary": "UAE 국영 기업 DP월드(DP World)가 루마니아 항구에 소형모듈원자로(SMR) 프로젝트를 추진 중이다.",
    "summaryLong": "UAE 국영 항만운영사 DP월드(DP World)가 루마니아 항구 지역에서 소형모듈원자로(SMR) 프로젝트를 추진하고 있다. 이는 중동 국가의 원자력 기술 확산 노력과 해외 에너지 인프라 진출 전략의 일환으로 보인다. SMR을 항구 및 산업 지역의 에너지 공급원으로 활용하려는 국제적 추세를 반영하고 있으며, UAE의 원자력 산업 네트워크 확대를 의미한다.",
    "cat": "기술",
    "type": "General",
    "dev": "DP월드",
    "region": "EU",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiY0FVX3lxTFB5QTVBenhMbDQ0QTBFSzlXYmlzcUVTQlF1eDNZRlNqX2gyWXZ0VV8wRW1wS092RUhoVlRRcWZ2bllFRlgyY1JtRmdIN1YzTEhKSVo4VXBiQlJqV3laTkJSeEpnSQ?oc=5",
    "k": "uae 국영 dp월드 루마니아 항구에 smr 추진",
    "id": "49e84136"
  },
  {
    "date": "2026-06-09",
    "title": "루마니아 콘스탄차 항만, 소형모듈원자로(SMR) 도입 타당성 검토",
    "summary": "아랍에미리트(UAE)의 디피월드(DP World)가 루마니아 콘스탄차 항만의 에너지 수요 충족을 위해 소형모듈원자로(SMR) 기술 도입 가능성을 검토하고 있다.",
    "summaryLong": "글로벌 항만 운영 기업인 디피월드(DP World)는 루마니아의 주요 해운 거점인 콘스탄차 항만의 에너지 자립을 목표로 소형모듈원자로(SMR) 기술 도입을 검토 중이다. 현대적 항만은 크레인, 적하역 설비 등 대량의 전력을 요구하며, 이를 안정적으로 공급할 필요성이 높다. SMR은 소규모 포트폴리오에 맞춘 청정 전력 공급원으로서 항만 운영의 탈탄소화와 에너지 독립성 강화를 동시에 달성할 수 있다. 성공할 경우 항만 산업에서의 소형모듈원자로 활용 모델이 확대될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "DP World",
    "region": "EU",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/smrs-to-be-considered-at-romanian-port",
    "id": "fdd6a661"
  },
  {
    "date": "2026-06-09",
    "title": "SMR 회사 Oklo, 원자력 엔지니어링 전문기업 ArmecK 인수",
    "summary": "샘 올트먼(Sam Altman)이 지원하는 소형모듈원자로(SMR) 개발사 Oklo가 원자력 엔지니어링 전문기업 ArmecK를 인수했다.",
    "summaryLong": "소형모듈원자로(SMR) 개발 회사인 Oklo가 원자력 엔지니어링 전문기업 ArmecK를 인수했다. Oklo는 OpenAI CEO 샘 올트먼(Sam Altman)과 기타 투자자들의 지원을 받는 유망 SMR 스타트업으로, ArmecK 인수를 통해 설계·건설·운영 역량을 강화하려는 전략이다. 이는 SMR 산업의 기술 통합과 가치사슬 고도화를 보여주는 사례이다.",
    "cat": "기술",
    "type": "SFR",
    "dev": "Oklo",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiY0FVX3lxTE9FSzdfUFE1dERfUkM0NGJmYjMycmNyczByVzd6ZEFDZHdaUkYzMjZuQnJrV0tPcDdmRUJ3R3NiY1cwRHNVMkpTelJubk93Ym9ENGVMYlZaQ09FVGZZb0YtaWU0Yw?oc=5",
    "k": "샘 올트먼 smr 오클로 원자력 엔지니어링 전문기업 아르멕 인수",
    "id": "e57b1809"
  },
  {
    "date": "2026-06-09",
    "title": "서울대, 반도체·SMR 융합 연구단체 첫 출범 중국 추격 경고",
    "summary": "서울대학교가 반도체와 소형모듈원자로 분야를 통합하는 연구단체를 새로 구성하며 국가 경쟁력 강화에 나섰다.",
    "summaryLong": "서울대학교가 반도체와 소형모듈원자로(SMR) 연구를 융합하는 새로운 연구단체를 첫 출범시켰다. 이는 첨단 기술 분야에서 중국의 추격을 우려하며 한국의 과학 기술 경쟁력을 강화하려는 국내 학계의 움직임을 보여준다. SMR은 반도체 제조, 데이터센터, 산업열 등 차세대 산업 수요와의 연계 가능성이 높아, 미래 에너지 솔루션으로서의 중요성이 부각되고 있다. 대학의 기초 연구 강화는 한국의 SMR 산업 생태계 성숙과 기술 자주성 확보를 위한 초석이 될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "서울대",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "이대로면 중국에 다 따라잡힌다 서울대 반도체 smr 연구단체 첫 출범",
    "id": "e4d61d6c"
  },
  {
    "date": "2026-06-09",
    "title": "HD현대, SMR 추진 자동차운반선 영국선급 기본인증 획득",
    "summary": "현대중공업(HD현대)이 소형모듈원자로(SMR) 추진 자동차운반선에 대해 영국선급 협회(Lloyd's Register)로부터 설계 기본인증을 획득했다.",
    "summaryLong": "현대중공업(HD현대)이 소형모듈원자로(SMR) 기술을 탑재한 자동차운반선 개발 과정에서 국제 선급 기관인 영국선급 협회(Lloyd's Register)로부터 설계 기본인증(기본설계인증)을 획득했다. 이는 원자력 추진 선박의 안전성과 기술 가능성을 국제적으로 인정받은 것으로, SMR 선박 라인업 확대의 중요한 마일스톤이다. 컨테이너선에 이어 자동차운반선까지 인증을 획득함으로써 SMR 선박의 적용 분야를 확대하고 있으며, 국내 조선 산업의 원자력 추진 기술 역량을 입증하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "HD현대",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMicEFVX3lxTE5ycFlHR3phS1BnYzQ5TmdyRnd3SjVlVExibXlKU2djSFJ0N3hWRjJzdGZVV2JORXNKUDJBeEtPTFRLeUNKcUgydnlaQWVldnEzbGVham5ZbV9Tb00wQTc0UjlBQkx0TVBJUU9xRFM2TUQ?oc=5",
    "k": "hd현대 smr 추진 자동차운반선 개발 속도 영국선급 기본인증 획득",
    "id": "1847c129"
  },
  {
    "date": "2026-06-09",
    "title": "기장군 SMR 관련 여론 조작 논란, 정치인 '부적절' 평가",
    "summary": "부산 기장군에서 SMR 유치와 관련된 여론 조작 의혹이 제기되고 지역 정치인들의 비판이 나왔다.",
    "summaryLong": "부산 기장군에서 소형모듈원자로(SMR) 유치 관련 여론 조작 의혹이 보도되었으며, 일부 정치인들이 이를 '부적절하다'고 평가했다. 이는 SMR 유치 과정에서 투명성과 민주적 절차에 대한 우려를 나타내는 사례이다. SMR과 같은 주민 수용성이 중요한 사업에서 여론 형성의 투명성 문제가 부각되었다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiT0FVX3lxTE1RY0FzQVFUbVNEVzhxQkJUUmU2NHpLWTJoNjYyMks0N0xQUWtiY3NIZDZPaDJ5YUNfMnhENVFNSHZSLW13NGdUQ3ZIWnFIM1U?oc=5",
    "k": "smr 반드시 찬성을 기장군 여론 조작 우성빈 부적절",
    "id": "817f6001"
  },
  {
    "date": "2026-06-09",
    "title": "루마니아 항만에 SMR 도입 추진…전력난 해소",
    "summary": "루마니아(Romania)가 항만 지역에 소형모듈원자로 도입을 추진하고 있으며, 이는 전력난 해소를 목표로 한다.",
    "summaryLong": "에너지안전신문은 루마니아(Romania)의 항만 지역 소형모듈원자로(SMR) 도입 추진 계획을 보도했다. 루마니아(Romania)는 산업용 열 공급과 전력 수급 안정화가 필요한 항만 지역에 SMR 배치를 통해 에너지 자립성을 강화하려는 계획이다. 이는 유럽(EU) 내 SMR 실증과 상용화의 확대 추세를 보여주는 사례로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "EU",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZEFVX3lxTE9vZHNqd094amlxeUlyeVR4LVFmMHY4X3I5dWRyT2ExSWIwbmdSQ2p4SFI1ZlFISFRobUxId0lfajUxSm1JeXo3SmlLM0llc1BERDJaYUd2bnpJWUxHS09wdFZVczY?oc=5",
    "k": "루마니아 항만 smr 도입 추진 전력난 해소 시급",
    "id": "0044c401"
  },
  {
    "date": "2026-06-08",
    "title": "글로벌 소형모듈원자로(SMR) 시장, 첨단 원자력 기술 주도로 성장",
    "summary": "글로벌 소형모듈원자로(SMR) 시장이 첨단 원자력 기술 개발에 힘입어 확대되고 있다.",
    "summaryLong": "세계 소형모듈원자로(SMR) 시장이 차세대 원자력 기술의 혁신적 발전으로 인해 급속한 성장세를 보이고 있다. 고온가스로(HTGR), 용융염원자로(MSR), 마이크로 원자로 등 다양한 첨단 기술 개발이 산업 다각화를 추진하고 있다. 에너지 안보, 탈탄소화, 산업 열 수요 충족 등 다양한 응용 분야의 수요 증가가 시장 확대를 견인하고 있다. 주요국의 규제 틀 정비와 민간 투자 확대로 글로벌 소형모듈원자로(SMR) 상용화 시대가 도래할 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "global small modular reactor market growth driven by advanced nuclear technologies",
    "id": "316acdc0"
  },
  {
    "date": "2026-06-08",
    "title": "한국의 용융염원자로(MSR) 기반 소형 원자로, 자동차화물선 동력원으로 활용 가능한 개념설계 승인",
    "summary": "한국이 개발 중인 용융염원자로(MSR) 기반 소형 원자로가 자동차화물선 동력원으로의 활용 가능성을 검증하는 개념설계를 승인받았다.",
    "summaryLong": "한국이 독자적으로 개발 중인 용융염원자로(MSR) 기술이 해양 운송 부문으로의 적용 가능성을 입증했다. 소형 원자로를 자동차화물선의 추진 동력으로 활용하는 개념설계가 관계 당국으로부터 공식 승인받았다. 해운 산업의 탈탄소화 및 에너지 효율성 강화가 시급한 가운데, 용융염원자로(MSR) 기술의 고온 특성과 안전성은 해양 응용에 적합한 특징으로 평가된다. 이는 한국의 원자력 기술 고도화와 함께 신조선 시장에서의 기술적 우위 확보를 의미하는 중요한 성과이다.",
    "cat": "기술",
    "type": "MSR",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiVEFVX3lxTE9HQVpkUGlUSmg5dkJnUUdKSnpER1AwYXJ3ZzBuZlhWM043V0lZR1M5U0tLRF9rMW9VWlhQT2lOUHMxTDZGOE1Id3Y5UGpnWm5ScEU4Ug?oc=5",
    "k": "작은 원자로가 자동차화물선 움직인다 용융염원자로 기반 개념설계 승인",
    "id": "75816df9"
  },
  {
    "date": "2026-06-08",
    "title": "한국원자력연구원, SMR 추진 대형 화물선 국제 선급 기본승인 획득",
    "summary": "한국원자력연구원이 소형모듈원자로(SMR)를 추진 에너지로 사용하는 대형 화물선에 대해 국제 선급 기본승인을 획득했다.",
    "summaryLong": "한국원자력연구원이 소형모듈원자로(SMR)로 운행되는 대형 화물선 설계에 대해 국제 선급(Classification Society)의 기본승인(AIP, Approval in Principle)을 획득했다. 이는 SMR 기술을 해양 운송 분야에 적용하는 프로젝트로, 한국의 원자력 기술이 해양 에너지 분야로 확대되고 있음을 시사한다. 국제 선급 승인은 추후 상세 설계 및 건조 단계로 진행하는 데 필요한 규제 기초가 되며, 탄소중립 해운 실현을 위한 주요 마일스톤이다.",
    "cat": "기술",
    "type": "General",
    "dev": "한국원자력연구원",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiW0FVX3lxTFBJYjlHdWdFN0xmbHFJQXJkQjJyMkhNV3dwamVQVlZJR1ZoZWUwRVRCVHBtSVMteG81cjdwcy1UUEV3Z1FYZUZ1S1FQX3kzZ0Zxc3dISlpwMG85bHM?oc=5",
    "k": "원자력 smr 추진 대형 화물선 국제 선급 기본승인 획득",
    "id": "ab18c151"
  },
  {
    "date": "2026-06-08",
    "title": "경주(Gyeongju)가 한국(Korea) SMR 초도호기 최적지인 이유",
    "summary": "경주 지역이 원전·연구개발·산업생태계를 모두 갖춘 한국 SMR 초도호기 입지로 최적이라는 기획 분석.",
    "summaryLong": "경주(Gyeongju) 지역이 한국(Korea) SMR 초도호기 건설의 최적 입지로 평가받고 있다. 기존 대형 원전 운영 경험과 노하우, 원자력 관련 연구개발 인프라, 그리고 형성된 원자력 산업생태계 등 SMR 개발·건설·운영에 필요한 모든 조건을 갖추고 있기 때문이다. 이는 SMR의 조기 상용화 및 기술 검증을 위한 거점 역할을 할 수 있음을 시사한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZ0FVX3lxTE5BU3FpdU1Wck04X2k4WUpGSE5KbG8yR1EzRXo2U3U4dVpyXzN6WERoRkF3VW9CcTNNSXgyQ2tRRlRlUWV6UkUxaE9vX1pteEJIa25TdW85S242NDZJVkh6eGVPUm1Qbk0?oc=5",
    "k": "기획보도 원전 연구개발 산업생태계 모두 갖췄다 경주가 smr 초도호기 최적지인 까닭",
    "id": "606448c8"
  },
  {
    "date": "2026-06-08",
    "title": "한국(Korea) SMR 상용화, 설계 이후 공급망 확보가 관건",
    "summary": "한국 SMR의 상용화 성공을 위해서는 기술설계뿐 아니라 국내 부품·소재 공급망 체계 구축이 핵심이라는 분석.",
    "summaryLong": "한국(Korea) SMR의 성공적 상용화를 위해서는 노형 설계 개발을 넘어 안정적인 부품·소재 공급망(Supply Chain) 확보가 가장 중요한 과제로 지적되고 있다. SMR은 모듈화와 공장 제작 기반의 대량 생산이 경제성의 핵심인데, 이를 위해서는 국내 산업계의 기술력 강화와 협력 체계 구축이 필수적이다. 설계 우수성만으로는 시장 경쟁력을 확보할 수 없으며, 공급망 고도화가 글로벌 수출 전략의 성패를 좌우할 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZEFVX3lxTE0tM256MlZ6R2c2bGVyVVVFX1l2WUZiNFNwSllMT3cxMlJDVlNMUHZvSkl3SlJPSFBYbE4wemFhMC04MGk4ZGdpY2lESVV3NG93MGpDbFJJU3dINFlHSVo4TzFKUnc?oc=5",
    "k": "k smr 상용화 설계 넘어 공급망이 관건",
    "id": "4133d829"
  },
  {
    "date": "2026-06-08",
    "title": "현대중공업(HD Hyundai), SMR 추진 자동차운반선 개발 본격화",
    "summary": "현대중공업이 소형모듈원자로 기반 자동차운반선 개발을 본격적으로 진행하고 있다.",
    "summaryLong": "현대중공업(HD Hyundai)이 소형모듈원자로(SMR)를 추진 동력으로 활용하는 자동차운반선(PCTC) 개발을 본격화하고 있다. 이는 해상 운송의 탈탄소화와 에너지 효율성 향상을 목표로 하는 차세대 선박 기술 개발의 일환으로, SMR의 해상 응용 분야 확대를 의미한다. 현대중공업의 SMR 활용 프로젝트는 국내 조선업의 원자력 기술 통합 및 글로벌 경쟁력 강화를 보여주는 사례이다.",
    "cat": "기술",
    "type": "General",
    "dev": "HD Hyundai",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiekFVX3lxTE9scFR1MndibVJ4ODJBWGswSFpfbnpFRzQwT1BETFFCZlVPSXQ0eHI5RUtMaWduamlqX1BpR0NzbUFfUk9HZFBUV25wOThsVWwyQUFlVjBqNTUwNU5YWkZGMmxjQ1l2U3RYbUg1ZWw4dEt4SWtCTzFmQUFR?oc=5",
    "k": "hd현대 smr 추진 자동차운반선 개발 본격화",
    "id": "bb4fdfea"
  },
  {
    "date": "2026-06-08",
    "title": "HD현대, SMR 선박·자율항해 기술로 글로벌 시장 공략",
    "summary": "HD현대가 소형모듈원자로(SMR) 기술을 선박에 적용하여 무탄소 해운 시장을 공략하고 있다.",
    "summaryLong": "HD현대가 소형모듈원자로(SMR) 기술을 선박 추진 시스템에 통합하고 자율항해 기술을 결합하여 글로벌 무탄소 해운 시장 진출을 추진 중이다. 이는 탈탄소화 추세 속에서 해양 에너지 시장의 새로운 기회를 포착한 것으로, 향후 상용화 시 국내 원자력과 조선 산업의 융합 모델로서 의미가 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "HD현대",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "무탄소 해운 가속 hd현대 smr 선박 자율항해 기술로 글로벌 시장 공략",
    "id": "9a44a1d1"
  },
  {
    "date": "2026-06-08",
    "title": "원자력연구원, 용융염로 SMR 자동차운반선 개념설계·로이드 기본승인 획득",
    "summary": "한국원자력연구원이 용융염로(MSR) 기반 SMR을 적용한 자동차운반선 개념설계를 완료하고 로이드선급(Lloyd's Register) 기본승인을 획득했다.",
    "summaryLong": "한국원자력연구원(KAERI)이 용융염로(MSR) 기술을 적용한 소형모듈원자로(SMR) 기반 자동차운반선의 개념설계를 추진하였으며, 국제 선급 로이드선급(Lloyd's Register)으로부터 기본승인(AIP, Approval in Principle)을 획득했다. 이는 SMR 기반 해운 추진 기술이 국제 규제 기준을 만족하고 실용화 가능성을 입증한 중요한 성과이다. 이 프로젝트는 우리나라의 첨단 SMR 기술 개발과 해운 산업의 탈탄소화를 동시에 추진하는 의미있는 사례이다.",
    "cat": "기술",
    "type": "MSR",
    "dev": "원자력연구원(KAERI)",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZkFVX3lxTFB4cktRR1lZNlZya1V3SGJCenlBalYwWGNmTVRQSlBUSHVhbGJvVGxGS0ZYOFpZWnhSTUZrei1RN2ZWQWI1NW1rLXJUSlZsSHBxZExTRmk2SEU3Y3VXeWdYRmVvaklRZw?oc=5",
    "k": "원자력연구원 msr smr 적용 자동차운반선 개념설계 로이드선급 기본승인 획득",
    "id": "eb558615"
  },
  {
    "date": "2026-06-07",
    "title": "미국(USA), 마이크로 원자로로 핵 르네상스 추진, 글로벌 시장 진출",
    "summary": "미국이 소형모듈원자로(SMR) 및 마이크로 원자로 기술로 핵 르네상스를 견인하며 국제 시장 진출을 추진 중이다.",
    "summaryLong": "미국이 마이크로 원자로를 포함한 소형모듈원자로(SMR) 기술을 통해 글로벌 에너지 시장에서의 주도권을 강화하고 있다. 정부의 정책 지원과 민간 기업의 기술 혁신이 미국의 핵 르네상스를 견인하는 주요 요소이다. 미국의 SMR 기술은 높은 안전성과 소규모 수요처 대응 능력으로 국제 경쟁력을 갖추고 있다. 향후 미국은 동맹국 및 개발도상국을 중심으로 글로벌 시장 진출을 가속화할 것으로 예상되며, 이는 미국의 에너지 외교 확대를 의미한다.",
    "cat": "기술",
    "type": "Micro",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "us sees nuclear renaissance with new micro reactor eyes global markets",
    "id": "55ca5cbe"
  },
  {
    "date": "2026-06-07",
    "title": "현대중공업, SMR 추진 자동차운반선 개발 가속화",
    "summary": "현대중공업이 소형모듈원자로(SMR) 기반 자동차운반선 개발을 가속화하고 있다.",
    "summaryLong": "현대중공업(HD현대)이 소형모듈원자로(SMR)를 동력 시스템으로 활용한 자동차운반선 개발을 적극 추진 중이다. 이는 해운 산업의 탈탄소화 트렌드에 대응하면서 동시에 SMR의 새로운 응용 분야를 개척하려는 전략이다. 현대중공업의 선제적 개발은 국내 조선업이 미래 친환경 선박 시장에서 경쟁력을 확보하기 위한 중요한 움직임이다.",
    "cat": "기술",
    "type": "General",
    "dev": "현대중공업(HD현대)",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZkFVX3lxTE5tVFVTUHp4NXVUek92V0RCS0ctc3gtOFFVUE96WTM1ZzJ6UU5nLUtfdUlkYnQ2R0V4QmVwdmh5U3dqREtpQ2hfLXo3aEhDczVRZ3JHMWtNeEZLcGcwQ2ItaUpkQXQ0dw?oc=5",
    "k": "hd현대 smr 추진 자동차운반선 개발 가속화",
    "id": "11116abb"
  },
  {
    "date": "2026-06-07",
    "title": "HD현대, SMR 추진선 개발 드라이브",
    "summary": "HD현대(현대중공업)가 소형모듈원자로(SMR) 기반 추진선 개발에 집중하고 있다.",
    "summaryLong": "HD현대(현대중공업)가 선박용 소형모듈원자로(SMR) 기술 개발과 상용화에 박차를 가하고 있다. 원자력 추진 선박은 장거리 운항 및 저탄소 해운의 핵심 기술로 주목받으면서, HD현대는 세계 조선업계의 기술 리더십 확보를 목표로 SMR 추진선 개발 프로젝트를 본격화하고 있다. 이는 한국 조선산업의 고부가가치 기술 확보와 국제 해운 탈탄소화 시장에서의 경쟁력 강화를 의미한다.",
    "cat": "기술",
    "type": "General",
    "dev": "HD현대",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMif0FVX3lxTFB1eDVaT01pN0ttaUlJR0pCWmNnS2tXaTVFT0FVZExoUFFET1VaRjYxdzJKbGNnZVEzQ2VsYnYxNDZCMVZmZjNRXzlTTWNZOXJyMWpKenpBNHNHRWw5UXMxRDdmQlhjZksyY1UzemVNRkJlVzAxb19DWmdBdUFrQjA?oc=5",
    "k": "hd현대 smr 추진선 개발 드라이브",
    "id": "166127f5"
  },
  {
    "date": "2026-06-06",
    "title": "현대중공업(HD Hyundai) SMR 기반 자동차운반선(PCTC), 로이드선급 기본인증 획득",
    "summary": "현대중공업이 개발한 SMR 추진 자동차운반선(PCTC)이 로이드선급 기본인증을 획득하며 해상 원자력 선점을 앞당겼다.",
    "summaryLong": "현대중공업(HD Hyundai)이 소형모듈원자로(SMR)를 추진 동력으로 하는 자동차운반선(PCTC, Pure Car and Truck Carrier)에 대해 국제선급 로이드선급(Lloyd's Register)으로부터 기본인증(Basic Certification)을 획득했다. 이는 SMR의 해상 운송 부문 실제 적용 가능성을 공식 인증한 것으로, 현대중공업의 해상 원자력 에너지 분야 선도적 지위를 강화하는 계기가 된다. 향후 상세설계 및 건조 단계로 진전될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "HD Hyundai",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiU0FVX3lxTFBhNFMtamUyVFVPOE1tLXdodTFPWG9ETUFEbGFrZk5XVXZ4ZFBSSS1KQlhjUjg1NE1aZWpwRUR6a0tidXRaclB2X0pSU29pcHQ5ZHRj?oc=5",
    "k": "smr pctc 기본인증 hd현대 해상 원자력 선점",
    "id": "d3be0b16"
  },
  {
    "date": "2026-06-06",
    "title": "혁신형 SMR 기장 유치 염원하는 지역민",
    "summary": "부산 기장군에서 혁신형 소형모듈원자로(SMR) 유치를 원하는 주민들의 움직임이 보도되었다.",
    "summaryLong": "부산 일광읍 주민들이 혁신형 소형모듈원자로(SMR)의 기장군 유치를 염원하고 있는 것으로 알려졌다. 이는 지역 경제 활성화와 에너지 안보 강화를 기대하는 주민들의 의견을 반영한 것으로 보인다. SMR 유치는 지역 개발과 첨단 원전 기술 도입이라는 이중의 의미를 가지고 있다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiaEFVX3lxTE1wdjBtSXNYQXluVm9hUV8tdmx2TGowZEdKNWRRM0xjMDBfdjNfdjJYQXZLTm44c21fcGVIc3I2VzdHc2kzZkNsTGQzUnd3Z2FFb2tVblozM3p5VzJHTlRZTVpMZGVtRmdE?oc=5",
    "k": "혁신형 smr 기장유치 염원하는 일광읍민들",
    "id": "2de84666"
  },
  {
    "date": "2026-06-05",
    "title": "우즈베키스탄 첫 소형모듈원자로(SMR) 첫 콘크리트 타설 기념식 개최",
    "summary": "우즈베키스탄에서 첫 소형모듈원자로(SMR)의 첫 콘크리트 타설 기념식이 열려 본격적인 건설이 시작되었다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/ceremony-to-mark-first-concrete-for-uzbekistan-smr",
    "summaryLong": "중앙아시아의 우즈베키스탄이 국내 첫 소형모듈원자로(SMR) 프로젝트의 착공식을 개최하여 본격 건설 단계에 진입했다. 이는 우즈베키스탄의 에너지 정책 전환과 원자력 산업 확대를 반영하는 중요한 이정표이다. 소형모듈원자로(SMR)의 도입은 중앙아시아 국가의 에너지 독립성 강화와 탈탄소 목표 달성을 동시에 추진할 수 있는 전략으로 평가된다. 우즈베키스탄의 성공 사례는 인접 국가들의 원자력 에너지 도입에도 영향을 미칠 것으로 전망된다.",
    "id": "a261715c"
  },
  {
    "date": "2026-06-05",
    "title": "앤터러스 뉴클리어(Antares Nuclear) Mark-0 마이크로원자로, 미국 에너지부(DOE) 프로그램 첫 임계 달성",
    "summary": "앤터러스 뉴클리어의 Mark-0 마이크로원자로가 영점 전력 임계에 도달해 미국 에너지부 프로그램에서 첫 임계 달성 사례가 되었다.",
    "cat": "기술",
    "type": "General",
    "dev": "Antares Nuclear",
    "region": "US",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/first-criticality-for-us-microreactor-under-doe-programme",
    "summaryLong": "앤터러스 뉴클리어가 개발 중인 Mark-0 마이크로원자로가 임계 달성에 성공했다. 이는 미국 에너지부 지원 프로그램 참여 개발사 중 첫 번째 임계 달성 사례로 평가된다. 소형모듈원자로(SMR) 기술의 실증 단계 진입을 의미하며, 미국의 차세대 원자력 기술 개발 추진력을 보여준다. 향후 상용화 단계로의 진행 가능성을 시사한다.",
    "id": "dbb8fdf2"
  },
  {
    "date": "2026-06-05",
    "title": "블리칼라(Blykalla), 스웨덴 납냉각 소형모듈원자로 플랜트 정부 재정 지원 신청",
    "summary": "스웨덴 납냉각 소형모듈원자로 개발사 블리칼라가 6기의 SEALER 원자로 플랜트 건설을 위한 정부 재정 지원을 신청했다.",
    "cat": "정책",
    "type": "General",
    "dev": "Blykalla",
    "region": "EU",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/blykalla-applies-for-state-aid-for-smr-plant",
    "summaryLong": "스웨덴의 소형모듈원자로 개발사 블리칼라가 정부의 재정 지원을 신청했다. 지원 대상은 자체 개발 중인 SEALER 원자로 6기를 기반으로 하는 플랜트 건설 사업이다. 이는 스웨덴의 탄소 중립 에너지 정책과 산업 전환의 일환으로 진행되고 있다. 정부 지원이 승인될 경우 유럽의 소형모듈원자로 상용화가 한 단계 가까워질 것으로 예상된다.",
    "id": "4960e8a6"
  },
  {
    "date": "2026-06-05",
    "title": "롤스로이스(Rolls-Royce) 영국 소형모듈원자로, 한국 협력사로 부품 외주",
    "summary": "영국 롤스로이스가 자국 소형모듈원자로 프로젝트의 일부 부품을 한국으로 외주하면서 논란이 발생했다.",
    "summaryLong": "영국의 롤스로이스가 추진 중인 소형모듈원자로 프로젝트에서 일부 부품 제조를 한국 협력사에 외주하기로 결정했다. 이는 글로벌 공급망 다변화 및 비용 효율화 전략의 일환으로 보인다. 한편 국내 부품 공급으로 평가되는 부분도 있으나, 자국 내 부품 조달을 강조하는 선진국 정책과의 조화 문제가 제기되고 있다. 향후 국제 협력 모델의 사례가 될 것으로 보인다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMihAFBVV95cUxOMGUzcERuMHNCUDdPc2s2aDlwb2hybzRmSEc5Rzg1ekl1VS04LVhWaVFiR1VuRklobWFZRURRMTdsN3NhWmQzQU9VZ09zRmh2VXM1YWJqWkUtZHBlRGtlY2g4V0VRZ182ZXNWOGM2QjNYczNXN1gtZFBDYnVFaEJNeW5KbVA?oc=5",
    "k": "rolls royce under fire for outsourcing parts of uk nuclear project to south korea",
    "id": "cbea39e1"
  },
  {
    "date": "2026-06-05",
    "title": "소형모듈원자로, 첫 시운전에서 임계점 달성",
    "summary": "소형모듈원자로가 첫 시운전에서 임계점에 도달했다.",
    "summaryLong": "소형모듈원자로가 첫 시운전 과정에서 임계점 달성에 성공했다. 이는 설계 단계를 거쳐 실증 단계로 진입했음을 의미한다. 소형모듈원자로의 상용화 가능성을 보여주는 중요한 기술적 성과다. 향후 성능 검증 및 안전성 확인을 통해 본격적인 상용화가 추진될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMimAFBVV95cUxPc1pTZEpKVHowV2VpVEdNY3g3d2NwQ3dKTWFDa3dMeGgtRGhnbGkxbHdOSmpqVVo1NnRNQ3lwVWxCX2lvLTlLbUpWVGp1R1VfQk1PVTFxT0RQUjFtd1VQNlFVME9aOFgySTF2RTJNb3EyS1B1Y1M3SWhfWENjN1RpRzkzTkczSmxjZWxfRmFfTXhUSFROUVVmdg?oc=5",
    "k": "small modular nuclear reactor reaches criticality in first test",
    "id": "77139b6d"
  },
  {
    "date": "2026-06-05",
    "title": "우즈베키스탄 SMR 첫 콘크리트 타설식 개최",
    "summary": "우즈베키스탄 소형모듈원자로 프로젝트의 첫 콘크리트 타설식 거행.",
    "summaryLong": "우즈베키스탄 소형모듈원자로(SMR) 프로젝트의 첫 콘크리트 타설식이 개최되어 건설 단계에 진입했다. 이는 프로젝트가 계획대로 진행 중임을 확인하는 주요 마일스톤이며, 중앙아시아 지역의 저탄소 에너지 개발 추진을 의미한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMimgFBVV95cUxQY2xPb3hudlBWZUFhRHZPMWpaalg5STVGdE5LX0JMM3Y0UzB5NkJ0MU1NdXBma0dZU1ZNYzAtMDAwWnBONnlzalhBNTFYQWpiQlBtMWpkdmFWb0g1RlBRNEEwSC1CVWc4WmJTbFNyczFVclFDTEJ4NDdEazNSeFQwQ1M2bU45d3pjS0JtbS1lTTItMGF6dlhBa1hB?oc=5",
    "k": "ceremony to mark first concrete for uzbekistan smr",
    "id": "1a52afd8"
  },
  {
    "date": "2026-06-05",
    "title": "미국 정부, 안타레스(Antares) SMR 임계 성공 발표 상용화 기정사실화",
    "summary": "미국 정부가 안타레스 소형모듈원자로의 임계(criticality) 달성에 성공했다고 발표하며 상용화 가능성을 공식화했다.",
    "summaryLong": "미국 정부가 안타레스(Antares) 소형모듈원자로의 임계 달성에 성공했음을 발표하면서 SMR 상용화의 핵심 관문을 통과했다고 평가했다. 임계는 원자로가 지속적인 핵분열 반응을 유지하는 단계로, 설계·건설·시운전 과정에서 가장 중요한 기술적 마일스톤 중 하나다. 이번 성공은 미국의 SMR 개발 프로그램이 실질적 진전을 이루고 있음을 증명하며, 향후 상용화 및 시장 진출 가능성을 높이는 신호로 작용한다.",
    "cat": "기술",
    "type": "General",
    "dev": "Antares",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "정부 안타레스 smr 임계 성공 상용화 핵심 관문",
    "id": "8cb3e359"
  },
  {
    "date": "2026-06-05",
    "title": "HD현대, 원자력 추진 자동차운반선 첫 인증 획득",
    "summary": "현대중공업(HD현대)이 원자력 기술을 탑재한 자동차운반선에 대한 첫 인증을 획득하며 소형모듈원자로(SMR) 선박의 영역을 확대하고 있다.",
    "summaryLong": "현대중공업(HD현대)이 원자력 추진 자동차운반선에 대한 국제 인증을 획득함으로써 소형모듈원자로(SMR) 기술의 선박 적용 영역을 확대하고 있다. 이는 SMR 선박 기술의 다양한 용선에 대한 적용 가능성을 입증하는 중요한 성과이다. 선박용 원자력 추진 기술은 국제 해운 산업의 탄소중립 전환을 주도할 핵심 기술로 평가되며, 한국 조선업의 기술 경쟁력 강화에 기여할 것으로 예상된다. HD현대의 지속적인 인증 획득은 SMR 선박의 상용화 시기를 앞당기고 국제 수주 확대로 이어질 가능성이 높다.",
    "cat": "기술",
    "type": "General",
    "dev": "HD현대",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMibkFVX3lxTE1Vdy1zTTF4dFBHcDdydW1ObWxKRTlLa3ppUmd0SkVQeUxQU0dJTktFRzY1VGdCMnZ1YXRrQ3duQnZYVmFrLWhpb3RUNHExV2Vrc2lVamoybmQ3NXdWbEZnSThWMy1oZ2lCdUtiUlZR?oc=5",
    "k": "hd현대 원자력 추진 자동차운반선 첫 인증 smr 선박 영토 넓힌다",
    "id": "e579bccf"
  },
  {
    "date": "2026-06-05",
    "title": "HD현대, SMR 추진선 라인업 확대… 자동차운반선 설계 기본인증 획득",
    "summary": "현대중공업(HD현대)이 컨테이너선에 이어 소형모듈원자로(SMR) 추진 자동차운반선의 설계 기본인증을 획득하며 원자력 선박 라인업을 다양화하고 있다.",
    "summaryLong": "현대중공업(HD현대)이 소형모듈원자로(SMR) 기술 탑재 자동차운반선의 설계 기본인증을 획득함으로써 원자력 추진 선박의 라인업을 확대하고 있다. 기존 컨테이너선에 이어 자동차운반선까지 인증을 획득하면서 다양한 선형에 대한 SMR 적용 기술의 성숙도를 보여주고 있다. 이러한 라인업 확대는 선박주(배선사)들의 다양한 수요에 대응하고 국제 시장에서의 수주 경쟁력을 높이는 전략이다. 설계 기본인증의 단계적 획득은 원자력 선박의 실선 건조와 운영 인증으로의 진전을 향한 중요한 마일스톤으로, 상용화 시기 단축에 기여할 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "HD현대",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiXkFVX3lxTE9aaEhsRmJYMWxjZlZlb215enoyN205WjFNOUFXUGxpUFE3dFNnb2VldDRHaldma2syLTBOU1RvaWd0WHRJeEl1VTR4N1pzb2o3ZVc0SDRqZ2pWeGhiQ3c?oc=5",
    "k": "hd현대 smr 추진선 라인업 확대 컨테이너선 이어 자동차운반선 설계 기본인증 동아비즈니스리뷰",
    "id": "d92ca510"
  },
  {
    "date": "2026-06-05",
    "title": "현대중공업(HD현대), SMR 추진 자동차운반선 개발 가속화",
    "summary": "현대중공업(HD현대)이 소형모듈원자로(SMR) 기반 자동차운반선 개발을 적극 추진하여 시장 기회를 확대하고 있다.",
    "summaryLong": "현대중공업(HD현대)이 소형모듈원자로(SMR)를 추진 시스템으로 하는 자동차운반선 개발에 속도를 내고 있다. 이는 해운업의 저탄소 전환 추세와 SMR 기반 해양 추진 기술의 상용화 움직임에 부응하는 전략으로, 회사의 SMR 선박 시장 진출을 본격화하는 단계이다. 현대중공업의 이러한 노력은 국내 조선산업의 첨단 SMR 기술 적용을 선도하는 사례이다.",
    "cat": "기술",
    "type": "General",
    "dev": "현대중공업(HD현대)",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiX0FVX3lxTE9vWC1GVFBsTkQ0d1EtNEtOd3BIdE1kdW5QYl9ZaWcxVjVaMVlTMXZweVV0N2Q5QXprcmNOSGVESy1RUE11SzZPd1ZDOFk5bGxVUmpaZlFkaFlnZTZvU0hR?oc=5",
    "k": "hd현대 원자력 자동차운반선 개발 속도 smr선박 판 키운다",
    "id": "591e9925"
  },
  {
    "date": "2026-06-05",
    "title": "HD현대, 원자력 추진 선박용 SMR 기술 선점",
    "summary": "HD현대(현대중공업)가 원자력 추진 선박 시장에서 소형모듈원자로(SMR) 기술 선점에 나섰다.",
    "summaryLong": "HD현대(현대중공업)가 차세대 해운산업의 주요 수단으로 부상하는 원자력 추진 선박 개발에서 SMR 기술 우위를 확보하기 위해 역량을 집중하고 있다. 원자력 추진 선박은 장거리 운항에서 높은 효율성과 환경 친화성을 제공하는 기술로, 국제 해운업계의 탈탄소화 추세와 맞물려 주목받고 있다. 이는 국내 조선업체의 첨단 기술 확보와 글로벌 시장 경쟁력 강화라는 전략적 의의를 가진다.",
    "cat": "기술",
    "type": "General",
    "dev": "HD현대",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMibEFVX3lxTFAzYkdENHh2dm16QWJ2YVFmYW1RU0EyRmFlTmNoTnZaWExDa2o2LXhOZzNBLXBIN3JzTHJ4UF84cnZIR0NCS2loaGFndE9vaVpael95TnVHRnZPRnV3dHBiTnNBamNqWnhtT203bQ?oc=5",
    "k": "원자력 추진 선박 시대 hd현대 smr 기술 선점 나선다",
    "id": "573cd5ab"
  },
  {
    "date": "2026-06-05",
    "title": "미국 에너지부, SMR 개발 기술조건 달성 발표",
    "summary": "미국 에너지부(DOE)가 소형모듈원자로(SMR) 개발을 위한 기술조건 달성을 선언하며 민간 지원 정책의 성과를 강조했다.",
    "summaryLong": "미국 에너지부(DOE)가 소형모듈원자로(SMR) 개발의 핵심 기술조건 달성을 공식 발표하면서 정부 주도의 민간 기업 지원 정책이 성과를 거두고 있음을 확인했다. 이는 미국의 SMR 개발 로드맵 추진 과정에서 설정한 주요 마일스톤 달성을 의미하며, 향후 상용화 단계로의 진입 가능성을 높이는 신호이다. 미국의 적극적인 SMR 정책 추진은 글로벌 원자력 시장에서 미국의 기술 경쟁력 강화와 동맹국과의 협력 확대를 추동하는 계기가 될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMic0FVX3lxTE52aEhqQTYxNC1oYzJsSHdUTi1GcmlYc0FhMi1hOXk5UUl1UGtoNWw2eW9JUVZvOTVWam9xNmFYdG1kSmNOZTB5Y1NvUXRSV0o0WXk2YUxvU29iSnROT2xyT3pHVjM5bERyZkY3YTdtVDlsczA?oc=5",
    "k": "미국 에너지부 smr 개발 위한 기술조건 달성 민간 지원 정책에 성과 나와",
    "id": "00e98af5"
  },
  {
    "date": "2026-06-05",
    "title": "소형모듈원자로(SMR)의 담수화 및 전력 생산",
    "summary": "소형모듈원자로를 통한 전력과 담수화 수자원 생산의 활용 방안을 다룬 기사.",
    "summaryLong": "스마트 워터 매거진(Smart Water Magazine)은 소형모듈원자로(SMR)를 통한 담수화 및 전력 동시 생산의 가능성을 보도했다. 소형모듈원자로는 기존 대형 원전보다 유연한 열 활용이 가능해 담수화 플랜트와의 통합 운영으로 수자원 문제를 해결할 수 있는 기술로 주목받고 있다. 이는 물 부족 지역에서의 에너지와 담수 공급이라는 이중 문제 해결을 위한 핵심 솔루션으로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "power and desalinated water by small modular reactors",
    "id": "7072efc2"
  },
  {
    "date": "2026-06-04",
    "title": "엑스에너지(X-energy), 나스닥(Nasdaq) 상장 후 2026년 1분기 분기별 파이프라인 144기·11.5GWe 공시",
    "summary": "나스닥 상장 후 분기 공시에서 미국·영국 합산 144기, 약 11.5 GWe의 파이프라인(고객 권리 전량 행사 가정)을 공개했다.",
    "cat": "투자",
    "type": "HTGR",
    "dev": "X-energy",
    "region": "US·UK",
    "source": "X-energy IR",
    "url": "https://x-energy.com/news/x-energy-reports-first-quarter-2026-results/",
    "summaryLong": "엑스에너지가 나스닥(Nasdaq: XE) 상장 후 제출한 분기 공시에서 사업 파이프라인 규모를 공개했다. 미국과 영국 지역의 소형모듈원자로 사업 수주 기준으로 총 144기, 용량 약 11.5 GWe에 이른다. 이는 고객의 권리 전량 행사를 가정한 수치로, 엑스에너지의 사업 확장 가능성을 보여준다. 글로벌 소형모듈원자로 시장의 성장 추세를 반영하고 있다.",
    "id": "9f464055"
  },
  {
    "date": "2026-06-04",
    "title": "미국 소형모듈원자로 개발사들 협력 파트너십 발표",
    "summary": "Day & Zimmermann이 Deep Fission의 Gravity 원자로 건설 계획을, Sciaky가 NX Atomics의 소형모듈원자로 플랫폼 부품 제조를 각각 맡기로 했다.",
    "cat": "기술",
    "type": "General",
    "dev": "Deep Fission, NX Atomics",
    "region": "US",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/us-smr-developers-announce-partnerships",
    "summaryLong": "미국 소형모듈원자로 산업에서 주요 협력 파트너십이 발표되었다. 엔지니어링 회사 Day & Zimmermann(Day & Zimmermann)은 Deep Fission의 Gravity 원자로 건설을, 첨단 제조 기업 Sciaky(Sciaky)는 NX Atomics(NX Atomics)의 소형모듈원자로 플랫폼 부품 제조를 담당한다. 이는 소형모듈원자로 개발과 실제 건설, 제조 단계로의 전환을 시사한다. 산업 생태계 형성을 통해 상용화 가속화가 예상된다.",
    "id": "5aaba06a"
  },
  {
    "date": "2026-06-04",
    "title": "홀텍 인터내셔널(Holtec International) 파트너십, Blue Castle 유타 소형모듈원자로 프로젝트 재개",
    "summary": "Blue Castle 프로젝트가 홀텍 인터내셔널과의 파트너십으로 유타(Utah)주 소형모듈원자로 시설로 재추진된다.",
    "summaryLong": "Blue Castle 프로젝트가 원자력 기술 회사 홀텍 인터내셔널과 파트너십을 체결하여 새로운 방향으로 재시작되었다. 유타(Utah)주에서 소형모듈원자로 시설 건설을 추진할 계획이다. 홀텍의 기술과 경험이 프로젝트 성공 가능성을 높일 것으로 예상된다. 미국 내 소형모듈원자로 지역 거점 구축의 의미가 있다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "Holtec",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMia0FVX3lxTE1EbUY4UFc2ZUw2bU92b1J1bnlBQzNiT25BNVNaTEVYblNnejdsVHd2T084T2hCUkFQMXRockRXeVN4RjBUYTZ2R2ZwbXJ1RWtiLU10RFptNzZTSHQ2S2ZpWGhkWGhBVnZVQk1N?oc=5",
    "k": "blue castle nuclear project in utah revived as small modular reactor facility in partnership with holtec international",
    "id": "a49f7590"
  },
  {
    "date": "2026-06-04",
    "title": "테라파워(TerraPower), 미국 첫 상용 소형모듈원자로 건설 착수",
    "summary": "테라파워가 와이오밍(Wyoming)에서 미국 최초의 상용 소형모듈원자로 건설을 시작했다.",
    "summaryLong": "소형모듈원자로 개발사 테라파워가 와이오밍(Wyoming)주에서 미국 최초의 상용급 소형모듈원자로 건설을 개시했다. 이는 미국의 차세대 원자력 에너지 정책 추진의 실질적인 결과를 의미한다. 성공적인 완공 시 미국의 소형모듈원자로 상용화 모델이 될 것으로 기대된다. 향후 다른 개발사의 상용화 가속화도 촉발할 가능성이 있다.",
    "cat": "기술",
    "type": "SFR",
    "dev": "TerraPower",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMihAFBVV95cUxOTUp2UFIxUDRDdmRfS0Fidi1aY01lT2NrRjhfODhrNVl5WTRFSVNlV2RYRWhnSHloZnJYXzQxdV9CU2Jfbk9EVVRxbmJpY0ZOWnQ5cm41SjVTMGhKRUVtWHowOHNnNE9NQUwtRlVsd2dhM2wyN3l2Tmh2SDJwNkJwVXpvNzg?oc=5",
    "k": "terrapower starts first u s commercial smr construction in wyoming",
    "id": "15ae9c89"
  },
  {
    "date": "2026-06-04",
    "title": "미국 소형모듈원자로(SMR) 개발사들 파트너십 발표",
    "summary": "미국의 소형모듈원자로 개발사들이 파트너십을 체결했다.",
    "summaryLong": "미국의 소형모듈원자로 개발 기업들이 협력 체계를 구축했다. 이번 파트너십은 기술 개발 가속화와 시장 진출 전략 수립을 목표로 한다. 관련 당사자들의 역할 분담을 통해 소형모듈원자로 상용화 추진 속도를 높일 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiiwFBVV95cUxNdmtuLWdRQmM3UnVoUEtvMms3MnJtMnppcDdiZDN0U2RZczlmTVZGQWhXcnZkNXBOQjZINFpXVnJXQXE2dm1wenNFdkZkXzAyZlBWNGw3aW5xc2w0QkZpRnQ3blo5NS1PekNySFMyNDg3VWpPa2gtb0VacmJXT3NfV29qZEpjRm51N2pV?oc=5",
    "k": "us smr developers announce partnerships",
    "id": "cbc1910b"
  },
  {
    "date": "2026-06-04",
    "title": "미국 기업, 300MWe급 소형모듈원자로 개발 추진",
    "summary": "미국 기업이 300MWe 규모의 소형모듈원자로를 개발하여 핵심 프로젝트에 전력을 공급할 계획이다.",
    "summaryLong": "미국의 원자력 기업이 300MWe(메가와트 전기)급 소형모듈원자로 개발 프로젝트를 추진 중이다. 이 원전은 대규모 산업 시설이나 핵심 기반시설에 안정적인 전력을 공급하기 위해 설계되었다. 소형모듈원자로의 유연한 전력 공급 능력을 활용하여 산업 탈탄소화를 가속화할 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMijgFBVV95cUxNQUJ3ZGpJN0J4YnRObTVSTjlHX1MzbVZ2ZzVWMlpTblo1T21KWVlodGZVWEtoUVpMRlZON1FsdGxsSXg2UFRSSnA0ODh4NUMtcGJjQnJXRURKZU9IZGRhcHlCcVM2Xy1TZ0xwcXBNR0xiMEFxa1NXak55amtDOVl0Ql9pMmtBRTI0TUdWZHJB?oc=5",
    "k": "us firm s small modular reactor that generates 300 mwe electricity to power nuclear project",
    "id": "3cd47fb1"
  },
  {
    "date": "2026-06-04",
    "title": "NX Atomics와 Sciaky, 소형모듈원자로 부품 제조 기술 협력",
    "summary": "NX Atomics와 Sciaky가 전자빔 적층제조(EBAM) 기술을 소형모듈원자로 부품 제조에 적용하기 위해 협력한다.",
    "summaryLong": "NX Atomics와 Sciaky가 전자빔 적층제조(EBAM, Electron Beam Additive Manufacturing) 기술의 소형모듈원자로 부품 생산 적용을 위해 협력하기로 했다. 이 첨단 제조 기술은 복잡한 부품의 정밀 생산과 생산 효율성 향상을 가능하게 한다. 양사의 기술 결합을 통해 소형모듈원자로 부품 제조의 신뢰성과 생산성이 대폭 개선될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "NX Atomics, Sciaky",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "nx atomics and sciaky bring ebam to small modular reactor components",
    "id": "d691850b"
  },
  {
    "date": "2026-06-04",
    "title": "우즈베키스탄, 소형모듈원자로 건설 공식 승인",
    "summary": "우즈베키스탄이 소형모듈원자로 건설을 공식 승인했다.",
    "summaryLong": "우즈베키스탄 정부가 소형모듈원자로 건설 프로젝트를 공식적으로 승인했다. 이는 중앙아시아 지역에서 원자력 에너지 활용 확대의 신호로 해석된다. 소형모듈원자로는 지역의 에너지 안보 강화와 탈탄소 목표 달성에 중요한 역할을 할 것으로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMipgFBVV95cUxPOTVKM2dhLWxCUTF6aHpEeTBBdzVFMkktdVlkNzQ2WjY0Zm5aem1EMUhmeFhnMVFXV3M3cU4tOU0wZlFHY21BT3ozMHRnYmtKZmxPRmttbGdWY1EteEtHSENTVmRQVUNFdFRvYUw4cldtdTlBZjYxNXh6c2FrVzEtczRVNzhqRHY5ejNPbFlFY2hfWmZUSHVnYnNoWnZiQnJTYTZkVXFR?oc=5",
    "k": "uzbekistan approves construction of small modular nuclear power unit",
    "id": "d12b5508"
  },
  {
    "date": "2026-06-04",
    "title": "미국 에너지부(DOE), 소형모듈원자로 5가지 탄력성 특징 공개",
    "summary": "미국 에너지부(DOE)가 소형모듈원자로의 주요 탄력성 특징 5가지를 인포그래픽으로 소개한다.",
    "summaryLong": "미국 에너지부(DOE, Department of Energy)가 소형모듈원자로의 핵심 탄력성 특징 5가지를 시각화한 인포그래픽 자료를 공개했다. 이 자료는 소형모듈원자로가 다양한 환경과 상황에서 안정적으로 작동할 수 있는 기술적 우위를 설명한다. 정보 공개를 통해 소형모듈원자로에 대한 공중 이해 증진과 정책 지지 기반 조성을 목표로 한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMilAFBVV95cUxOUUl4QVNzZUIwRTB5bFp5MjdjWXI2blJJRS0xbHZOVUVvMnNSUHBoa3J5dVdTYmM3X3NmeGtTMFJHbmlkWUFGeGJZYTAxZTduWG9DSTEwVGxyQjNpLXRISm5vLURIS1pwcW9NUnN0R0Z0Y0J5d2ktQUQ1OWUyNS11UzlLdG50UEZrTTRVRWZXUU9EeVZI?oc=5",
    "k": "infographic 5 resilient features of small modular reactors",
    "id": "09766a49"
  },
  {
    "date": "2026-06-04",
    "title": "SMR의 안전성 강조, 인구 밀집 지역 건설 가능성 제기",
    "summary": "소형모듈원자로의 안전성이 우수해 도시·인구 밀집 지역에도 건설 가능하다는 주장이 제기되었다.",
    "summaryLong": "전문가들이 소형모듈원자로(SMR)의 우수한 안전 특성을 강조하며, 기존 대형 원자력발전소와 달리 인구 밀집 지역에 건설할 수 있다는 입장을 표명했다. SMR은 소형 용량, 수동형 안전 시스템, 노심 용융 가능성 감소 등의 특성으로 인해 기술적으로 대형 원전보다 안전성이 높다고 평가된다. 이러한 특성은 도시 근처 또는 산업 단지 내 설치를 가능하게 하여 수송 손실 감소, 폐열 활용 등 경제성 향상을 기대할 수 있다. 이는 향후 한국 내 SMR 입지 선정 및 사회적 수용성 확보에 중요한 근거로 작용할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMihAFBVV95cUxPemFSNnhYMjVGTDlhNVJ3amhNRF83YjRYU2hHTGhJYklxZHFPVV9vV1VfbzFvNFJ3N0xCUksyMEswc1JoNk0weTZEb2ZxamJVaTV1cGt5UTJiRHQ4RWgxejJkUG1IVzJiR3pKcm83ek1HN3J0U1hGdnVaYkhHSTdHcXNEd1A?oc=5",
    "k": "smr은 안전 인구 밀집 지역에 건설해도 문제 없어",
    "id": "fa1984f2"
  },
  {
    "date": "2026-06-04",
    "title": "SK가 투자한 SMR 건설 가속화",
    "summary": "SK(에스케이)가 투자한 소형모듈원자로(SMR) 프로젝트의 건설이 속도를 내고 있다.",
    "summaryLong": "SK(에스케이) 계열사가 주도적으로 추진하는 소형모듈원자로(SMR) 프로젝트의 건설 진행이 가시화되고 있다. SK는 차세대 원전으로서 SMR의 상용화 잠재력을 인식하고 상당한 자본을 투입하여 프로젝트를 추진 중이다. 이는 국내 대형 에너지·화학 기업의 SMR 시장 진입과 산업화 가속화를 보여주는 사례로, 국내 SMR 생태계 형성에 중요한 역할을 하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "SK",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "차세대 원전 여기서 시작 sk가 투자한 smr 건설 속도",
    "id": "674ac4ed"
  },
  {
    "date": "2026-06-04",
    "title": "아이다호 국립연구소(INL)에 미국 최초 소형모듈원자로 설치",
    "summary": "미국 에너지부(DOE)가 아이다호 국립연구소(INL)에 국내 최초의 소형모듈원자로를 구축하여 핵 연구에 전력을 공급하게 된다.",
    "summaryLong": "미국 에너지부(DOE)는 아이다호 국립연구소(INL)에 미국 최초의 소형모듈원자로(SMR) 설비를 배치하여 핵 연구 활동에 전력을 공급할 계획을 발표했다. 이는 미국 정부가 추진하는 소형모듈원자로 상용화 및 실증 프로젝트의 중요한 이정표로, 연구 시설의 탄소중립 운영을 지원하면서 동시에 소형모듈원자로 기술 검증의 기회를 제공한다. 아이다호 국립연구소는 전통적으로 원자력 연구와 개발의 중심지로서, 이 프로젝트는 SMR 기술의 신뢰성과 성능을 입증하는 데 중요한 역할을 할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "DOE",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "nation s first small modular reactor plant to power nuclear research at idaho national laboratory",
    "id": "0334d20c"
  },
  {
    "date": "2026-06-04",
    "title": "소형모듈원자로 외부 전력 상실 시 대응 방안",
    "summary": "미국 에너지부(DOE)가 소형모듈원자로의 외부 전력 상실 상황에 대한 대응 및 안전 조치를 설명하는 자료를 제공했다.",
    "summaryLong": "미국 에너지부(DOE)는 소형모듈원자로(SMR)가 외부 전력 공급을 잃었을 때의 대응 방안과 안전 메커니즘에 관한 정보를 공개했다. 이는 소형모듈원자로의 안전성과 신뢰성에 대한 공중의 우려를 해소하고 규제 기준 충족 여부를 확인하는 과정의 일부로 보인다. 소형모듈원자로는 대형 원전보다 소형이지만 동일한 수준의 안전 기준을 충족해야 하며, 특히 자동 안전 종료(SCRAM) 및 수동 냉각 능력이 중요한 설계 특성이다.",
    "cat": "기술",
    "type": "General",
    "dev": "DOE",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMimAFBVV95cUxPakpTYmlJRWFNQ2ViVW52V0wtOGYxbE90VlpNOE1fUVFDR01vZlo3cW55QzJMaGlzS2JtMzBOWkJuWGlMNVA2dnZPYmVPOUVzajNfeGdha3RZTjA4TUw5TjVLZ1hPbEVXaUxEQ0IzUEtsR1k3VlRRZ0Y5VW9CV2VkRTN4aW0wYnpqVEwyNmxDRDh6LW85UzJfSw?oc=5",
    "k": "what should i do if a small modular reactor loses off site power",
    "id": "9298fb43"
  },
  {
    "date": "2026-06-04",
    "title": "미국, 40년 만에 첫 상용 SMR 승인…AI 시대 전력 공급",
    "summary": "미국(US)이 40년 만에 첫 상용 소형모듈원자로를 승인했으며, 이는 AI 시대의 전력 수요 충족을 목표로 한다.",
    "summaryLong": "조선일보는 미국(US)이 40년 만에 첫 상용 소형모듈원자로(SMR) 승인을 보도했다. 이는 데이터센터와 AI 인프라로 증가하는 전력 수요를 충족하기 위한 차세대 원전 기술로 평가된다. 상용 SMR 승인은 미국의 탄소 중립 에너지 정책과 첨단 산업 전력 수급 전략의 핵심 이정표이며, 글로벌 SMR 시장 확대의 신호탄이 될 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMihAFBVV95cUxNRG5xSHBtV1E5WmtCY2w2bHBOWl9mZkw5SG9YQzh6bUZvT2E0eDd6MFhSUGNuTl9qNWhKSVRxNUE3cjI0U3lPT0xxRzVYRDJhamJ0bzhCaWxOMG5OLTBaWlZyS3VDTmdqdmQ2SWFZbzcxYnhKNEZ5bVdDYVR4RUJOVDNUY1E?oc=5",
    "k": "us approves first commercial smr in 40 years powers ai era",
    "id": "79fd6edf"
  },
  {
    "date": "2026-06-03",
    "title": "X-energy, 영국 규제기관에 Xe-100 고온가스냉각로 인허가 신청",
    "summary": "미국의 첨단원전 개발사 X-energy가 고온가스냉각로(HTGR) Xe-100을 영국 제네릭 설계 평가 절차에 신청했다.",
    "cat": "기술",
    "type": "HTGR",
    "dev": "X-energy",
    "region": "UK",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/x-energy-submits-xe-100-for-uk-regulatory-assessment",
    "summaryLong": "미국의 첨단원자로 개발 기업 X-energy가 자사의 고온가스냉각로(HTGR, High Temperature Gas-cooled Reactor) Xe-100을 영국 규제기관의 제네릭 설계 평가(Generic Design Assessment) 절차에 제출했다. 이는 영국 시장 진출을 위한 중요한 단계이며, 국제적 규제 승인 확보의 신호이다. 성공 시 유럽 시장에서의 첨단로 상용화 가능성이 높아질 것으로 예상된다.",
    "id": "7b0da62d"
  },
  {
    "date": "2026-06-03",
    "title": "네덜란드 아인트호펜, 용융염로 테스트 시설 개소",
    "summary": "네덜란드 아인트호펜(Eindhoven)에 용융염로 기술 개발·테스트를 위한 첨단 시설이 개설되었다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "EU",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/dutch-msr-testing-facility-launched",
    "summaryLong": "네덜란드 아인트호펜(Eindhoven)에 용융염로(Molten Salt Reactor) 기술의 개발 및 검증을 위한 고급 테스트 시설이 문을 열었다. 이 시설은 용융염로 기술의 안전성과 성능 입증을 가능하게 한다. 유럽의 첨단원자력 기술 개발 생태계 강화에 기여할 것으로 기대된다.",
    "id": "156f0f46"
  },
  {
    "date": "2026-06-03",
    "title": "스웨덴 알레이마(Alleima), 소형모듈원자로 부품 공급 확대",
    "summary": "스웨덴 알레이마(Alleima)가 샌드비켄(Sandviken)에 신규 튜브밀 시설을 개설해 소형모듈원자로·첨단로 부품 공급 체계를 강화했다.",
    "cat": "기술",
    "type": "General",
    "dev": "Alleima",
    "region": "EU",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/alleima-expands-capacity-as-nuclear-demand-grows",
    "summaryLong": "스웨덴의 소재·부품 공급기업 알레이마(Alleima)가 샌드비켄(Sandviken)에 첨단 튜브밀 생산 시설을 신규 개설했다. 이 시설은 소형모듈원자로(SMR) 및 첨단원자로의 핵심 부품 공급 역량을 대폭 확대하기 위해 구축되었다. 전 세계 소형모듈원자로 시장 수요 증가에 대응하는 공급망 강화 전략의 일환으로 평가된다.",
    "id": "23df5dec"
  },
  {
    "date": "2026-06-03",
    "title": "한국 i-SMRDA, 터키 Nuclean과 SMR 협력 협약 체결",
    "summary": "한국의 i-SMRDA(i-Small Modular Reactor Development Alliance)와 터키의 Nuclean이 소형모듈원자로(SMR) 개발·보급을 위한 협력 양해각서를 체결했다.",
    "summaryLong": "한국의 i-SMRDA와 터키의 Nuclean이 소형모듈원자로(SMR) 분야 협력을 위한 협약을 체결했다. 이는 한국의 SMR 기술과 터키의 시장 진출 의향이 결합된 전략적 파트너십으로, 양국이 SMR 개발과 국제 보급 네트워크 구축에 협력하기로 합의한 것이다. 이 협력은 아시아와 중동 지역의 에너지 안보 강화 및 탄소중립 달성에 기여할 것으로 예상된다.",
    "cat": "계약",
    "type": "PWR",
    "dev": "i-SMRDA",
    "region": "KR",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "south korea s i smrda t rkiye s nuclean ink co op deal on smrs daily sabah",
    "id": "2d549fc5"
  },
  {
    "date": "2026-06-03",
    "title": "빌 게이츠의 SMR 프로젝트에 SK 등 한국 기업이 참여",
    "summary": "SK 등 한국 기업들이 빌 게이츠가 주도하는 소형모듈원자로(SMR) 사업에 '날개' 역할을 하며 참여하고 있다.",
    "summaryLong": "빌 게이츠가 설립한 테라파워(TerraPower) 등의 소형모듈원자로(SMR) 프로젝트에 SK 등 한국 기업들이 부품 공급, 기술 협력 등으로 핵심 역할을 담당하고 있다. 국내 기업의 원자력 기술력과 제조 역량이 글로벌 차세대 원자로 개발에 활용되고 있으며, 이는 한국의 SMR 산업 생태계 강화를 의미한다.",
    "cat": "기술",
    "type": "General",
    "dev": "SK",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiVkFVX3lxTFA5MnFTUldfbkhJbGRjT2pSbUxYQVBkbWd3ZXYxSnV3SzR4c1hmM09BYmNvMDdsdGZIMmVMQmhSQmRWbi1GdnU3cEowNWJsWVNwVWNWUS1n?oc=5",
    "k": "미래 원전 smr 짓는 빌 게이츠 sk 등 기업이 날개 달았다 르포",
    "id": "ba50e330"
  },
  {
    "date": "2026-06-03",
    "title": "시티(Citi), SMR 개발 가속화 속 주요 원자력 주식 순위 발표",
    "summary": "금융사 시티(Citi)가 소형모듈원자로 개발 가속화 추세 속에서 유망 원자력 관련 주식들의 순위를 공개했다.",
    "summaryLong": "글로벌 금융사 시티(Citi)가 소형모듈원자로(SMR) 개발이 가속화되는 시장 환경 속에서 투자 가치가 있는 원자력 관련 기업들의 주식 순위를 발표했다. 이는 SMR 산업의 성장 전망이 높아지고 있으며, 글로벌 금융시장에서도 관련 기업들에 대한 관심과 투자 수요가 증가하고 있음을 시사한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMid0FVX3lxTE1obTdQdXVQWmt3UDhiMWtaYXZ6U3JQRndLTkVCTVRJdVBBc3UxSC11ZHJablduemN1c3JLOUZyeWZaUW9YUDZLeVF4NHJ5ZXlvR0xpRjdOSnhNUExqM1JoS1JvbTZZdjJ2RVNrNnhPcGowU1dyeGVz?oc=5",
    "k": "citi smr 개발 가속화 속 주요 원자력 주식 순위 공개",
    "id": "40196cdf"
  },
  {
    "date": "2026-06-03",
    "title": "롤스로이스, 웨일즈 부지에 SMR 3기 건설 추진",
    "summary": "롤스로이스(Rolls-Royce)가 웨일즈의 조용한 지역에 소형모듈원자로(SMR) 3기를 건설하는 대담한 투자를 진행 중이다.",
    "summaryLong": "영국의 롤스로이스(Rolls-Royce)가 웨일즈 지역의 특정 부지를 선정하여 소형모듈원자로(SMR) 3기 건설 프로젝트를 추진하고 있다. 이 3기의 SMR이 상당한 수의 가정에 전력을 공급할 수 있는 규모라고 보도되고 있다. 이는 영국 내 첫 SMR 발전소 건설 관련 프로젝트로서, SMR 상용화의 구체적 진전을 보여주는 사례다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "rolls royce is turning a quiet welsh site into a nuclear bet and the strange part is how many homes three small reactors could power",
    "id": "a8b84984"
  },
  {
    "date": "2026-06-02",
    "title": "혁신형 소형모듈원자로(SMR), 통제봉구동기구(CRDM) 검증 과제 직면",
    "summary": "뉴스케일(NuScale)을 포함한 혁신형 소형모듈원자로(SMR)가 통제봉구동기구(CRDM) 검증이라는 인허가 과제를 안고 있다.",
    "summaryLong": "혁신형 소형모듈원자로(SMR) 개발사들이 원자로 운영의 핵심 기기인 통제봉구동기구(CRDM) 검증 문제로 인허가 절차에서 도전과제를 마주하고 있다. 뉴스케일(NuScale) 등 주요 개발사들이 이 기술 검증에 집중하고 있는 상황이다. 통제봉구동기구(CRDM)는 원자로 안전성의 중요한 요소로, 이 검증 과정을 통과하는 것이 상용화의 필수 조건이다. 이러한 기술적 과제 해결이 향후 소형모듈원자로(SMR)의 상용화 일정을 좌우할 것으로 예상된다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "뉴스케일도 거친 crdm 검증 혁신형 smr도 도전장",
    "id": "9190f4ac"
  },
  {
    "date": "2026-06-02",
    "title": "Day & Zimmermann-Deep Fission 파트너십, 지하형 소형모듈원자로(SMR) 건설 추진",
    "summary": "Day & Zimmermann과 Deep Fission이 지하형 소형모듈원자로(SMR) 건설을 위한 파트너십을 체결했다.",
    "summaryLong": "건설·엔지니어링 회사 Day & Zimmermann과 핵기술 개발사 Deep Fission이 지하형 소형모듈원자로(SMR) 건설 프로젝트를 추진하기 위한 전략적 파트너십을 체결했다. 이 협력을 통해 두 회사는 지하 건설 기술과 원자로 기술을 결합한 혁신적인 소형모듈원자로(SMR) 개발을 목표로 하고 있다. 지하형 소형모듈원자로(SMR)는 안전성과 환경 친화성 측면에서 차별화된 장점을 제공할 것으로 기대된다. 이러한 협력은 소형모듈원자로(SMR) 산업의 다양한 기술 경로 추진을 보여주는 사례이다.",
    "cat": "기술",
    "type": "General",
    "dev": "Deep Fission",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "day zimmermann deep fission partner to build underground small modular reactor",
    "id": "2064c2dc"
  },
  {
    "date": "2026-06-01",
    "title": "SMR 선박으로 무탄소 해양시장 선점...원자력·조선·해양 학회 'K-문샷' 지지 선언",
    "summary": "원자력·조선·해양 분야 학회들이 소형모듈원자로(SMR) 선박을 통한 무탄소 해운 시장 선점을 목표로 하는 'K-문샷'을 공동 지지하기로 선언했다.",
    "summaryLong": "국내 원자력, 조선, 해양 분야의 주요 학회들이 소형모듈원자로(SMR) 선박 기술을 활용한 무탄소 해양시장 선도를 위한 'K-문샷' 프로젝트를 공동으로 전폭 지지한다는 선언을 발표했다. 이는 정부 정책과 산학 연대를 통해 차세대 무탄소 해운 분야에서 한국의 경쟁력을 확보하려는 범부처·범산업 차원의 협력 의지를 보여주는 것으로, 향후 기술 개발과 상용화 가속화가 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMickFVX3lxTFBiSllFVkVILWE2TW92S2pXcnp0dHBvUk5CZm5jTVdQZVExQmU0a0ZaMDBpS0xXdVpqZHluQWZtTE1YLUJtZEtuenJfSy16ZklTMHE4SUx0dWpKSUVRTUFzcmJ6alZhNmhfMXVNNTZlWDh4Zw?oc=5",
    "k": "smr 선박 무탄소 해양시장 선점 원자력 조선 해양 학회 k 문샷 전폭 지지 선언",
    "id": "d12b5cb2"
  },
  {
    "date": "2026-06-01",
    "title": "SMR 선박 개발에 원자력·조선·해양학계 역량 결집",
    "summary": "소형모듈원자로를 선박 추진 동력으로 활용하기 위해 관련 산업과 학계가 협력을 강화하고 있다.",
    "summaryLong": "원자력, 조선, 해양학 분야의 전문가와 산업계가 소형모듈원자로(SMR) 기반 선박(원자력 추진선) 개발 사업에 역량을 결집하고 있다. 이는 탈탄소 해운, 극지 항해, 심해 운영 등 미래 해양 산업 수요에 대응하기 위한 기술 개발 전략으로, 한국의 조선 산업 경쟁력을 고도화하는 차원의 의미를 갖는다. 원자력-조선-해양의 학·산 협력체계 강화는 기술 개발 속도 향상과 시장 창출 가능성을 높이는 기초가 될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMickFVX3lxTE1MZWJ3R2xfeVphbkVtaHpBN0cyOE5mS1U5ZjA3TVRKQk1IRE1iQm9UbEFXUXoyVDV4VERJb0hsZlRlTFBOSFJvNmhLTmY0bE4zMHFJOXRWcDlMWGV4bi1Ic0ZoeWZFNmtoNElzUzh6M01UUQ?oc=5",
    "k": "smr 선박 개발에 역량 결집 원자력 조선 해양학계 한목소리",
    "id": "f79acb21"
  },
  {
    "date": "2026-06-01",
    "title": "소형모듈원자로 선박 개발 지지·추진을 위한 학·산 공동 선언",
    "summary": "원자력, 조선, 해양학계가 소형모듈원자로 기반 선박 개발의 성공적 추진을 위해 공동 선언을 발표했다.",
    "summaryLong": "국내 원자력·조선·해양 분야의 학계와 산업계가 소형모듈원자로(SMR) 기반 선박 개발의 지지 및 성공적 추진을 다짐하는 공동 선언문을 발표했다. 이 선언은 탈탄소 선박 추진 기술로서의 원자력 활용, 한국 조선산업의 기술 혁신, 해양 에너지 안보 강화 등의 목표를 명시하는 것으로 보인다. 학·산 협력 선언은 정부 정책 추진에 대한 민간 부문의 적극적 지지와 공감을 표현하며, 향후 기술 개발, 규제 정비, 시장 형성 등 관련 사항들의 추진 동력을 강화하는 신호로 작용할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZEFVX3lxTE4xY3JCQmVJd3JXQWZDSW1nUjhPTE9jVVBQdDFvMml4bDMydEhrUjZfeHFqM0hnR1FGQlZSX1hNOEdER1BucWtDQlhROEgzOVBTX1hOWm9DTDVZMy1mZk1XZE1tQWc?oc=5",
    "k": "소형모듈원자로 smr 선박 개발 지지 및 성공적 추진을 위한 공동 선언",
    "id": "86d3180b"
  },
  {
    "date": "2026-06-01",
    "title": "원자력·조선학계 SMR 선박 개발에 협력",
    "summary": "원자력과 조선해양 학계가 소형모듈원자로(SMR) 추진 선박 개발을 한국의 주요 전략으로 추진하기 위해 협력하고 있다.",
    "summaryLong": "원자력과 조선해양 분야 학계가 소형모듈원자로(SMR) 기술을 활용한 선박 개발을 국가 전략 과제로 집중 추진하고 있다. 이는 한국의 조선·해양·원자력 산업을 융합하는 'K-문샷' 미션으로 불리며, 탄소중립 달성과 국제 경쟁력 강화를 목표로 한다. 학계의 전폭적인 지지와 공동 선언을 통해 SMR 선박 개발의 중요성을 강조하고 있으며, 이는 향후 기술 개발과 상용화의 탄력을 받을 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiTEFVX3lxTE8tb0dRc010M0VmMW92d1JaVkZuOVljUm1NSGcxbjBURno2TG02M1BSNUNuclEtdFhOSTB3M1VuTGxQaVdrZ1BFdEx6TW4?oc=5",
    "k": "원자력 조선학계 뭉쳤다 smr 선박 k 문샷 성공 열쇠",
    "id": "6175226e"
  },
  {
    "date": "2026-06-01",
    "title": "원자력·조선해양 학계, SMR 선박 개발 K-문샷 미션 지지 선언",
    "summary": "원자력과 조선해양 분야 학계가 소형모듈원자로(SMR) 선박 개발을 국가 핵심 전략인 'K-문샷 미션'으로 추진하기로 공동 선언했다.",
    "summaryLong": "원자력 및 조선해양 분야의 학계가 소형모듈원자로(SMR) 기술을 활용한 선박 개발을 국가 핵심 전략 과제인 'K-문샷 미션'으로 추진하기로 공동 선언했다. 이는 학계의 전폭적인 지지를 바탕으로 정부와 산업계의 협력을 강화하려는 의도를 담고 있다. 원자력 추진 선박은 탄소중립 달성과 조선 산업 고도화의 핵심 기술로 평가되며, 한국의 조선·해양·원자력 산업 융합을 통한 신성장동력 창출을 목표로 한다. 공동 선언문 발표로 SMR 선박 개발의 학술적·정책적 기반이 더욱 견고해질 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZ0FVX3lxTFBtMTFLVEd2OW9xeVFKdy16emg2NE5hMGFpMjFhOEZYNmtTVFh5LTlaYURkQ0xfUTA1TXFvcjRGZ1lRMWxLNFlGTkZjNFZVMFRQSlZzR3pFdWlKeTl1bWE0X0FGcXNsdU0?oc=5",
    "k": "원자력 조선해양 학계 smr 선박 개발 k 문샷 미션 전폭 지지 공동 선언문 발표",
    "id": "88577647"
  },
  {
    "date": "2026-06-01",
    "title": "원자력·조선·해양학회, SMR 선박 개발 적극 지원 촉구",
    "summary": "원자력·조선·해양 분야 학회가 소형모듈원자로 선박 개발에 대한 정부의 적극적 지원을 강조했다.",
    "summaryLong": "원자력, 조선, 해양 분야의 전문가들로 구성된 학회가 소형모듈원자로(SMR) 기술을 활용한 선박 개발이 미래 산업의 핵심이라며 정부의 적극적인 재정 지원과 정책적 뒷받침이 필요함을 주장했다. 이는 학계와 산업계가 원자력 추진 선박의 국가 경쟁력 확보의 중요성을 강조하고 있음을 보여준다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "원자력 조선 해양학회 smr 선박 개발 적극 지원해야",
    "id": "75cca106"
  },
  {
    "date": "2026-05-31",
    "title": "미국, 해운 상용화용 소형모듈원자로 추진 본격화",
    "summary": "미국이 상용 해운 부문에 소형모듈원자로(SMR)를 도입하기 위한 주요 정책을 추진하고 있다.",
    "summaryLong": "미국이 해양 운송 부문의 탄소 감축을 위해 소형모듈원자로(SMR) 기반 선박 추진 기술을 상용화하려는 주요 정책을 추진 중이다. 이는 해운업의 저탄소 전환과 SMR의 새로운 시장 개척을 동시에 추구하는 전략으로 평가된다. 미국의 이러한 움직임은 글로벌 SMR 시장 확대에 중요한 신호로 작용할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMingFBVV95cUxOS2dZLUpJVkFFYm03cFh1cmhZWkoyaXNmSmx5aElNN3BjcEtpbXBPYjJob2FVRUVkQUlmRFNqaFJaMlk3U1lKN2xMbEMxeHgteWpablkwSDkyaUIyVzRWRmVfTGRLNDdjMkZIU0g4TTBmSEFDSzNwaHloWE1pa2d6UWZzMVZ2Y1d5RmhVR21wS3hpRzVaeVdiS0lZWUlEQQ?oc=5",
    "k": "u s pushing small modular reactors for commercial shipping in major nuclear maritime push",
    "id": "d4b1e9fb"
  },
  {
    "date": "2026-05-31",
    "title": "부산시, 2030년까지 원자력산업 육성에 1882억 투입",
    "summary": "부산시(Busan)가 SMR·원전해체 허브 조성을 목표로 2030년까지 1882억 원을 투입하기로 결정했다.",
    "summaryLong": "부산시(Busan)가 차세대 원자력산업의 중심지로 도약하기 위해 2030년까지 1882억 원의 재정을 투입하는 원자력산업 육성 계획을 발표했다. 이 계획은 소형모듈원자로(SMR) 개발·산업화와 원전해체(decommissioning) 허브 구축을 핵심 목표로 삼고 있다. 부산의 조선·해양산업 기반과 연계하여 SMR 추진선 개발, 해체기술 개발 등 다층적 산업 생태계 조성을 추진함으로써 지역 경제 활성화와 국가 에너지 정책 기여를 동시에 추구하는 전략이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiaEFVX3lxTFBNSlVvUHJLUERiRTNEZncxTXBLOWVEZHFQQ0JtSm5jMy1GdUtQVXVGVUM5ckgzNHVJS0pKcWE1SGVRX3Zfb3c0R3N1ZFJjQzEzajl0OE9rdEhiaW5wQnE1ZFI4ZnR1OW1F?oc=5",
    "k": "부산시 2030년까지 원자력산업 육성에 1882억 투입 smr 원전해체 허브 도약",
    "id": "b720d531"
  },
  {
    "date": "2026-05-29",
    "title": "유타 그린리버 소형모듈원자로(SMR) 프로젝트 합작사 출범",
    "summary": "Fulcrum Point Holdings과 Blue Castle Holdings이 홀텍(Holtec)의 소형모듈원자로(SMR)-300 기술을 이용한 유타(Utah)주 소형모듈원자로(SMR) 건설 프로젝트 추진을 위해 합작사를 설립했다.",
    "cat": "계약",
    "type": "PWR",
    "dev": "Holtec",
    "region": "US",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/nineteen-years-on-companies-team-up-for-us-new-build-project",
    "summaryLong": "Fulcrum Point Holdings과 Blue Castle Holdings이 홀텍(Holtec)의 소형모듈원자로(SMR)-300 기술을 기반으로 미국 유타(Utah)주에서 소형모듈원자로(SMR) 건설 프로젝트를 추진하기 위해 합작사를 설립했다. 이 프로젝트는 그린리버 지역을 중심으로 진행될 예정이다. 홀텍(Holtec)의 소형모듈원자로(SMR)-300은 모듈화된 설계와 높은 안전성으로 주목받고 있다. 이러한 합작은 미국 내 소형모듈원자로(SMR) 상용화 노력이 단계적으로 진행되고 있음을 보여준다.",
    "id": "d132fd23"
  },
  {
    "date": "2026-05-29",
    "title": "미국 정부, 소형모듈원자로(SMR) 기술 장려",
    "summary": "미국 정부가 소형모듈원자로(SMR) 기술을 대외적으로 홍보하고 있다.",
    "summaryLong": "미국 정부는 에너지 정책의 일환으로 소형모듈원자로(SMR) 기술을 전략적으로 장려하고 홍보하고 있다. 이는 탄소 감축과 에너지 안보라는 정책 목표를 달성하기 위한 핵심 기술 분야로 평가되고 있다. 정부 차원의 적극적 지원은 소형모듈원자로(SMR) 산업의 성장을 가속화할 것으로 기대된다. 미국의 이러한 정책 기조는 향후 국제 원자력 시장에도 영향을 미칠 전망이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMijwFBVV95cUxNQ3k0MFFtZUVDZ01RSlFTRENvaktLb0QtSHAwTFVudV8xdWhJYm1PejNTYW0xcF9UWEhHNU1mSF9CNk1YX3pPZ0pwV0ZsUWgzc3JmclotNEprU2pjT2s0ZjlvQ1lHZ182bHBDR2ZNUG4zYU9iYTVfejJ5ZG9xOWFKSDBiZUJpbWVmTjNySjlicw?oc=5",
    "k": "u s government promotes small modular reactor smr technology",
    "id": "d34955c3"
  },
  {
    "date": "2026-05-29",
    "title": "영국 원자력 70년 경험, 한국형 SMR 발전 방향 제시",
    "summary": "영국의 70년 원자력 산업 경험과 통찰이 한국형 소형모듈원자로 개발의 나아갈 길을 조명한다.",
    "summaryLong": "영국의 70년에 걸친 원자력 산업 경험과 교훈이 한국형 소형모듈원자로(SMR) 개발 전략 수립에 시사점을 제공하고 있다. 영국은 가압경수로(PWR), 비등경수로(BWR)를 포함한 다양한 원자로 개발과 운영 경험을 축적해왔으며, 현재 선진국 중 SMR 개발에 적극적인 국가 중 하나다. 이 기사는 영국의 규제 체계, 기술 개발 전략, 산업 생태계 구축 경험 등이 한국의 SMR 사업 추진에 어떻게 활용될 수 있는지를 분석하는 내용으로 보인다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZEFVX3lxTE1GS3pPYkN3ZGxKbGhpRGRHemlwSzBZSmlWSl9BbGhJNGZvMjdhSmU3UWhwekR6ZFJQRUJrTlFmYURSdk1QNDAzNExMRHk5NDl6aVpmYUlVMHpFb1lSazhXZVBZMmE?oc=5",
    "k": "영국 원자력 70년의 통찰 한국형 smr이 가야 할 길",
    "id": "913cef3f"
  },
  {
    "date": "2026-05-29",
    "title": "SMR 개발 촉진을 위한 '가상원자로' 공개",
    "summary": "원자력 관련 기관이 소형모듈원자로(SMR) 적시 개발을 촉진하기 위한 가상원자로(virtual reactor) 기술의 베일을 벗었다.",
    "summaryLong": "원자력 산업계가 소형모듈원자로(SMR) 개발 과정의 시간과 비용 효율화를 위해 개발한 가상원자로(virtual reactor) 시뮬레이션 기술이 공개되었다. 가상원자로는 설계·검증·안전분석 등의 단계에서 실물 시제품 제작 전 디지털 환경에서 반복 실험을 가능하게 하는 차세대 개발 도구이다. 이 기술의 도입으로 SMR 개발 주기 단축과 비용 절감이 기대되며, 국내 SMR 프로젝트의 경쟁력 강화에 기여할 것으로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMickFVX3lxTE92Mm5ZM3ZRdzE0X1FJQl9weFhPN3hadDRJb0VXeXo1UWpWOEtRZVpjNUNvS3hyN0RFX3B1bUZRbTVoX19Yc29RcHBMQnE0Z0FMMEljcWJLaXBVNjU3THVlYlQ5c2xZajdYa08zdWRndTNPZw?oc=5",
    "k": "smr 적기 개발 촉진할 가상원자로 베일 벗다",
    "id": "6ff8566d"
  },
  {
    "date": "2026-05",
    "title": "미국 원자력규제위원회(NRC), Dow(Long Mott) Xe-100 환경평가(EA) 완료",
    "summary": "건설허가 신청의 일부로 미국 원자력규제위원회(NRC)가 약식 환경평가(EA) 완료 — 상업 첨단로 첫 사례이며, 건설허가(CP) 승인은 2027년 1분기 예상.",
    "cat": "인허가",
    "type": "HTGR",
    "dev": "X-energy",
    "region": "US",
    "source": "NRC Long Mott",
    "url": "https://www.nrc.gov/reactors/new-reactors/advanced/who-were-working-with/applicant-projects/long-mott",
    "summaryLong": "미국 원자력규제위원회(NRC)가 X-energy의 Xe-100 원자로에 대한 약식 환경평가(EA)를 완료했으며, 이는 상업용 첨단원자로 최초 사례이다. Dow Chemical이 Long Mott 지역에서 추진 중인 이 프로젝트의 건설허가(CP) 승인은 2027년 1분기에 예상된다. 환경평가(EA) 완료는 인허가 절차의 중요한 단계를 넘긴 것으로, 첨단 원자로 기술의 상용화가 실질적으로 진행되고 있음을 의미한다. 이 사례는 향후 다른 첨단 원자로 프로젝트의 인허가 진행에 참고가 될 것으로 보인다.",
    "id": "77d1b0cc"
  },
  {
    "date": "2026-05-28",
    "title": "롤스로이스(Rolls-Royce) 소형모듈원자로(SMR), 슈코다·두산에너빌리티와 핵심 부품 계약",
    "summary": "롤스로이스(Rolls-Royce) 소형모듈원자로(SMR)가 원자로 압력용기 등 핵심 부품의 사전 생산 작업을 슈코다(Skoda) 주식회사(JS)와 두산에너빌리티에 발주했다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "World Nuclear News",
    "url": "https://www.world-nuclear-news.org/articles/skoda-js-doosan-enerbility-get-key-rolls-royce-smr-work",
    "summaryLong": "롤스로이스(Rolls-Royce)가 자사의 소형모듈원자로(SMR) 프로젝트를 위해 원자로 압력용기 등 핵심 부품의 사전 생산 작업을 슈코다(Skoda) 주식회사(JS)와 두산에너빌리티에 발주했다. 이는 국제적 협력을 통해 소형모듈원자로(SMR)의 상용화를 가속화하기 위한 전략이다. 슈코다(Skoda) 주식회사(JS)는 유럽의 주요 원자력 부품 제조사이고, 두산에너빌리티는 한국의 핵심 원자력 기업이다. 이러한 계약은 롤스로이스(Rolls-Royce) 소형모듈원자로(SMR)의 실질적 건설이 임박했음을 시사한다.",
    "id": "0117a254"
  },
  {
    "date": "2026-05-28",
    "title": "혁신형 i-SMR 경주 유치, 주민수용성 확보가 핵심",
    "summary": "경주시 정책 자문관이 혁신형 i-SMR 유치 성공의 핵심을 주민수용성으로 지적했다.",
    "summaryLong": "경주시 원자력정책 관련 자문관은 혁신형 소형모듈원자로(i-SMR) 유치가 성공하기 위해서는 지역 주민의 수용성 확보가 가장 중요하다고 강조했다. 주민수용성은 지역 에너지 프로젝트의 추진과 완성에 있어 결정적인 요소이다. 경주시는 과거 원자력 시설 관련 경험이 있어 이러한 점을 충분히 인식하고 있다. 향후 i-SMR 유치 추진 과정에서 주민과의 적극적 소통과 신뢰 구축이 핵심 과제가 될 것으로 보인다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZkFVX3lxTFBMMk5nQUNjZ2VtQlBsb3I4bERRbWJQZXBfVW96MXlRTUotYXAyUUZPeG51RDlRMjJkY3NkTUZPbVpINndJMjlhTkZDVGtKYlBmLWVVWkNybmFKMGJkdW90dVBEakpLQQ?oc=5",
    "k": "기고 이재근 경주시 원자력정책 자문관 혁신형 i smr 경주유치는 주민수용성이다",
    "id": "7cb43295"
  },
  {
    "date": "2026-05-28",
    "title": "경주시 원자력정책 자문관, 혁신형 i-SMR 소형모듈원자로(SMR) 유치 강조",
    "summary": "경주시 원자력정책 자문관이 혁신형 i-SMR의 경주 유치가 주민수용성 확보의 핵심이라고 주장한다.",
    "summaryLong": "경주시의 원자력정책 자문관은 국내 개발 혁신형 소형모듈원자로(i-SMR)를 경주로 유치하는 것이 중요하며, 이를 위해 주민수용성 확보가 핵심 요소라고 강조했다. 혁신형 i-SMR은 한국이 자체 개발한 원자로 기술로서 경주 유치는 지역 경제 발전과 기술 자립에 의미가 있다. 주민수용성 확보는 장기적 프로젝트 추진의 기초이며, 투명한 정보 공개와 지역사회와의 적극적 협력이 필요하다. 경주시는 이러한 조건을 갖춘 입지로 평가되고 있으며, 성공적 유치를 위한 다각적 노력이 요구된다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "i-SMR",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiY0FVX3lxTE5EQmlmWE1xMWkydWRaMVFia1JkUWt5OTlBU21VRGNhRUZ0eVByeUl0THZQa3lVb1ItclRTd3RsSElxc3A3RlFPT1RXNUpoQmxZd19DSXRHdDF0Y0VNTDVyU0ZLNA?oc=5",
    "k": "조은기고 이재근 경주시 원자력정책 자문관 혁신형 i smr 소형모듈원자로 경주유치는 주민수용성이다",
    "id": "a93d9225"
  },
  {
    "date": "2026-05-28",
    "title": "두산에너빌리티, 롤스로이스(Rolls-Royce) SMR 핵심 파트너 선정",
    "summary": "두산에너빌리티가 영국(UK) 롤스로이스(Rolls-Royce)의 소형모듈원자로(SMR) 사업의 핵심 파트너로 선정되었다.",
    "summaryLong": "두산에너빌리티가 영국(UK) 롤스로이스(Rolls-Royce)의 소형모듈원자로(SMR) 개발·상용화 사업에 핵심 공급 파트너로 지정되었다. 이는 국내 원자력 기업이 글로벌 선도 SMR 기업과의 전략적 협력을 공식화한 것으로, 향후 부품 공급, 기술 협력, 공동 사업 추진 등이 예상된다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "두산에너빌리티",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "두산에너빌리티 롤스로이스 smr 핵심 파트너 선정",
    "id": "ef4aed25"
  },
  {
    "date": "2026-05-28",
    "title": "원자력연, 인도·유럽에 국산 SMR 수출 추진",
    "summary": "한국원자력연구원(원자력연)이 국내 개발 소형모듈원자로(SMR)를 인도(India)와 유럽(EU)으로 수출할 계획을 발표했다.",
    "summaryLong": "한국원자력연구원(원자력연)이 국내 개발 소형모듈원자로(SMR) 기술의 인도(India)·유럽(EU) 지역 수출을 적극 추진하겠다고 발표했다. 이는 한국의 SMR 기술을 글로벌 시장에 판매하려는 정부·연구기관의 전략적 노력이며, 향후 국제 원자력 시장에서 한국의 위상 강화와 기술 수출 확대를 목표로 하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "원자력연",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiVEFVX3lxTFBlaXNYM042N1Q5UEwtQ3VPNFBEam42ZDZwSDh6akhhV2ltNGk2SzN0U3dRRmIxX0R1UWRtUlFib3pBamRnVzhMdWRQajFNU1FCY3ducg?oc=5",
    "k": "원자력연 인도 유럽에 국산 smr 수출 추진",
    "id": "70155af4"
  },
  {
    "date": "2026-05-27",
    "title": "오클로(Oklo), 미국 에너지부(DOE) 핵탄두 플루토늄 재활용 사업 참여",
    "summary": "샘 올트먼(Sam Altman)이 지원하는 소형모듈원자로 개발사 오클로(Oklo)가 미국 에너지부의 핵탄두 추출 플루토늄 연료화 사업에 참여한다.",
    "summaryLong": "오픈에이아이(OpenAI) CEO 샘 올트먼(Sam Altman)이 투자한 소형모듈원자로(SMR) 개발사 오클로(Oklo)가 미국 에너지부(DOE)의 감축된 핵탄두에서 추출한 플루토늄을 원자로 연료로 활용하는 사업에 선정되어 참여하게 된다. 이는 미국의 핵무기 폐기 자산을 민간 원자력 기술에 활용하는 정책의 일환이며, 오클로의 기술 수준을 입증하는 중요한 계약이다.",
    "cat": "기술",
    "type": "SFR",
    "dev": "Oklo",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiY0FVX3lxTE4wZjIzN2tkS3I4Wk5UYTNTZktnTk9XcUZESHFtVU9uVm00eFFRYnVxWDBZZHZLSmptb1RfeUdfd2swYW9vd3dpTkYxYTZ6RUVZZW9JZER4TUNRNEFaWmdfOVZNQQ?oc=5",
    "k": "샘 올트먼 smr 오클로 에너지부 핵탄두 추출 플루토늄 원자로 연료 사업 참여",
    "id": "80632f4b"
  },
  {
    "date": "2026-05-26",
    "title": "스웨덴 소형모듈원자로(SMR) 발전소, 세 번째 인허가 신청 제출",
    "summary": "스웨덴에서 세 번째 소형모듈원자로 발전소 건설에 대한 규제 신청이 접수되었다.",
    "summaryLong": "스웨덴은 탄소중립 전환을 추진하는 가운데 소형모듈원자로에 대한 관심을 지속하고 있다. 이번 세 번째 인허가 신청은 스웨덴의 소형모듈원자로 개발이 활발히 진행되고 있음을 보여준다. 소형모듈원자로는 기존 대형 원자력발전소 대비 낮은 초기 투자비와 유연한 입지 선정이 가능한 것으로 평가받고 있다. 향후 규제당국의 검토를 거쳐 프로젝트 진행 여부가 결정될 예정이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMilAFBVV95cUxOVDBOT1BETGh0bXZBR2UtNTVycWl2VFQtbnVuUXZPVG1OdVFmN0RGdS05SmI2bURVQ0gxTGhNTjVqcVZaV09fcXpkbXVTSV8wNXJEb2hRWmxuVm5UZjhoMU9Tb25pMW9BNE9FNnhQSmV1MjdhVmhBY0NrcXJ6S3hVS3FmNG9mZE5XTTFuNkFtaFV6Nno4?oc=5",
    "k": "third application lodged for swedish smr plant",
    "id": "588c14ef"
  },
  {
    "date": "2026-05-26",
    "title": "혁신형 i-SMR(소형모듈원자로) 경주유치, 주민수용성이 관건",
    "summary": "경주 지역의 혁신형 i-SMR 유치를 둘러싼 주민수용성 문제에 대한 논의.",
    "summaryLong": "경주 지역에 혁신형 i-SMR(소형모듈원자로) 유치를 추진하는 과정에서 주민수용성이 주요 과제로 대두되고 있다. 지역 주민의 동의와 신뢰 확보가 프로젝트 성공의 핵심 요소로 지적되고 있으며, 이를 통해 경주의 원자력 산업 중심지로서의 위상 강화를 목표하고 있다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiWkFVX3lxTE9kcHlyclltems3Q0hka2x2a3BxcnpmWHByc3NSWm92YzZqT1h6TWprZ0E1THpLZ0diX3gwQ21oRk1mYlB5dDRNYzZRN2Q5b2pHSG9oMElRRDJBdw?oc=5",
    "k": "혁신형 i smr 소형모듈원자로 경주유치는 주민수용성이다",
    "id": "56f63b65"
  },
  {
    "date": "2026-05-25",
    "title": "미국 에너지부, 소형모듈원자로 프로젝트 자금 지원",
    "summary": "미국 에너지부(DOE)가 소형모듈원자로 프로젝트 추진을 위해 자금을 지원.",
    "summaryLong": "미국 에너지부(DOE)가 소형모듈원자로(SMR) 프로젝트를 신속화하기 위한 자금을 지원하기로 결정했다. 정부 차원의 재정 지원을 통해 SMR 개발 가속화 및 상용화 추진을 목표로 하며, 미국의 에너지 독립성과 기후 목표 달성을 지원한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "energy department awards funding to expedite small modular reactor projects",
    "id": "b04d9866"
  },
  {
    "date": "2026-05-24",
    "title": "실증 원전 넘어 상업용 SMR로…HD현대重, 최종 목표는 '원자력 추진선'",
    "summary": "현대중공업(HD현대重)이 SMR 실증을 거쳐 상업용 모듈 개발을 추진하며, 궁극적으로 원자력 추진 선박 시장을 목표하고 있다.",
    "summaryLong": "현대중공업(HD현대重)은 소형모듈원자로(SMR) 기술 개발을 단계적으로 진행 중이다. 현재 진행 중인 실증 원전 사업을 거쳐 상업용 SMR 모듈을 개발하고, 장기적으로 원자력 추진 선박(nuclear-powered vessel) 시장 진출을 최종 목표로 삼고 있다. 이는 한국 조선산업의 친환경화와 고부가가치 기술 확보 전략으로, SMR의 해양 응용 분야 개척을 의미한다.",
    "cat": "기술",
    "type": "General",
    "dev": "HD현대重",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiT0FVX3lxTE44TjRBbkpoWG5SbFBoZnJZRjdvMEtOSVRKeS1CM1pZaE5ld1huRzBIZmVtYUpUZUF2cmlNRl8tdG9GR01MTHVBYVJDVmhUdW8?oc=5",
    "k": "실증 원전 넘어 상업용 smr로 hd현대 최종 목표는 원자력 추진선",
    "id": "80a1a08d"
  },
  {
    "date": "2026-05-22",
    "title": "SMR 시대 원자력 안전규제와 사전검토제도의 의미",
    "summary": "소형모듈원자로 시대의 원자력 규제 체계 개선과 사전검토제도(Pre-application Review)의 역할을 논의하는 기고문.",
    "summaryLong": "SMR 개발 및 상용화 단계에 진입하면서 기존 대형 원전 중심의 안전규제 체계의 개선이 시급한 상황이다. 특히 사전검토제도(Pre-application Review, PAR)는 설계 단계부터 규제 당국과 개발사 간 소통을 강화하여 인허가 절차의 투명성과 효율성을 높이고 불필요한 지연을 줄일 수 있는 방안으로 평가받고 있다. 이는 SMR의 특수성을 반영한 규제 혁신의 일환으로, 글로벌 규제 추세와의 동향 맞춤도 고려한 논의이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiVEFVX3lxTE83U3I2YXREbllpQk5iOUxZa2hBb0xPMy1Rd0hUX3hjeFJHeFNseGlWWGVyNHo4Q3Iwek1kUzVJSVJzRnpBUlFSNlRTUHc2bnYxMk9PMA?oc=5",
    "k": "특별기고 smr 시대 원자력 안전규제 사전검토제도의 의미",
    "id": "c9ce475a"
  },
  {
    "date": "2026-05-21",
    "title": "미국 네브래스카 공공전력지구(NPPD), 서덜랜드 지역 소형모듈원자로 부지 선정 진행",
    "summary": "미국 네브래스카 공공전력지구(NPPD)가 소형모듈원자로 입지 선정을 추진 중이며, 서덜랜드(Sutherland)가 주요 후보지로 검토되고 있다.",
    "summaryLong": "미국 네브래스카 공공전력지구(NPPD)는 지역의 에너지 수급 및 탄소중립 목표 달성을 위해 소형모듈원자로 도입을 검토하고 있다. 서덜랜드는 기존 발전 인프라와 전력망 접근성 등을 고려한 유력한 후보지로 평가되고 있다. 이 사업은 미국의 소형모듈원자로 상용화 가속화 전략의 일환이다. 부지 확정 후 규제 승인 및 건설 단계로 진행될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "NPPD",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "continuing coverage lincoln county leaders push for sutherland as nppd small modular reactor search narrows",
    "id": "11137a58"
  },
  {
    "date": "2026-05-20",
    "title": "미국 네브래스카 공공전력지구(NPPD), 네브래스카주 4개 지역을 소형모듈원자로 후보지로 선정",
    "summary": "미국 네브래스카 공공전력지구(NPPD)가 네브래스카주 내 4개 지역을 소형모듈원자로 건설 후보지로 공식 지정했다.",
    "summaryLong": "미국 네브래스카 공공전력지구(NPPD)는 소형모듈원자로 도입 타당성을 검토하기 위해 체계적인 부지 선정 과정을 진행해왔다. 이번에 4개 지역이 공식 후보지로 선정됨에 따라 각 지역에 대한 심화 평가가 시작될 것으로 보인다. 선정된 후보지들은 기존 발전소 부지, 인프라 현황, 지역사회 수용성 등이 종합적으로 평가된 결과이다. 향후 최종 부지 결정 및 프로젝트 추진 일정이 공지될 예정이다.",
    "cat": "기술",
    "type": "General",
    "dev": "NPPD",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "nppd identifies 4 nebraska communities as potential small modular reactor sites",
    "id": "d45c422e"
  },
  {
    "date": "2026-05-20",
    "title": "미국 네브래스카 공공전력지구(NPPD), 네브래스카주 4개 지역을 소형모듈원자로 부지로 선정",
    "summary": "미국 네브래스카 공공전력지구(NPPD)가 소형모듈원자로 건설을 위한 잠재적 부지로 4개 지역사회를 식별했다.",
    "summaryLong": "미국 네브래스카 공공전력지구(NPPD)는 지역 전력 수요 및 에너지 안보 강화를 목표로 소형모듈원자로 도입을 추진하고 있다. 체계적인 부지 평가 과정을 거쳐 4개 지역이 잠재 부지로 선정되었으며, 각 지역은 기술적·경제적·사회적 요건을 만족하는 것으로 검토되었다. 이는 미국의 소형모듈원자로 상용화 전략과 에너지 전환 목표를 반영한 주요 프로젝트이다. 향후 부지별 심화 평가와 규제 승인 절차가 진행될 예정이다.",
    "cat": "기술",
    "type": "General",
    "dev": "NPPD",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "nppd identifies 4 nebraska communities as potential small modular reactor sites koln nebraska local news weather sports",
    "id": "afdb85b0"
  },
  {
    "date": "2026-05-20",
    "title": "Deep Fission(딥 피션), 1억 5천만 달러 규모 기업공개 신청",
    "summary": "소형모듈원자로 개발사 Deep Fission(딥 피션)이 1억 5천만 달러 규모의 기업공개를 신청했다.",
    "summaryLong": "Deep Fission(딥 피션)은 소형모듈원자로 기술 개발에 주력하는 신생 핵심 기업으로, 기업공개를 통해 추가 자금 확보를 추진하고 있다. 1억 5천만 달러의 IPO 규모는 소형모듈원자로 시장의 성장 잠재력과 투자자들의 관심을 반영한다. 조성된 자금은 기술 개발, 인허가 획득, 상용화 추진 등에 활용될 것으로 예상된다. 이는 소형모듈원자로 산업의 성장 단계 진입을 나타내는 신호로 평가된다.",
    "cat": "투자",
    "type": "General",
    "dev": "Deep Fission",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "fisn ipo news small modular nuclear reactor developer deep fission files for a 150 million ipo",
    "id": "1eb98125"
  },
  {
    "date": "2026-05-19",
    "title": "과기부, 차세대 SMR 조기확보 위한 미래 원자력 정책 추진",
    "summary": "과학기술정보통신부(과기부)가 차세대 소형모듈원자로(SMR) 조기 확보를 목표로 미래 원자력 정책 수립에 돌입했다.",
    "summaryLong": "과학기술정보통신부(과기부)가 국내 원자력산업의 미래 전략으로 차세대 소형모듈원자로(SMR)의 조기 확보와 상용화를 핵심 내용으로 하는 정책 수립을 본격화했다. 글로벌 에너지 전환 추세 속에서 SMR의 중요성이 높아지고 있는 상황에 대응하여, 정부 차원의 집중적인 정책 지원과 기술 개발 투자를 통해 국제 경쟁력 확보를 추구하는 움직임이다. 이는 국내 원자력산업의 다각화와 첨단 기술 강국 지위 확립을 위한 전략적 결정으로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMibkFVX3lxTE9OMndPM1hYcUlia250SHNHZ21aUk1KUHhMYVE4WVBXc3E2M1NQckE1dHF6M0NTRzNBdGFSSVN5MjJ1aFdZM2k4enFHWG5vR0lrUGtyUF93cERicTM1SWN6bmdtZTFOWUVORGJHaGNB?oc=5",
    "k": "차세대 smr 조기확보 과기부 미래 원자력 정책 수립 돌입",
    "id": "62ae6ead"
  },
  {
    "date": "2026-05-19",
    "title": "정부, SMR 메가프로젝트 본격 추진",
    "summary": "한국 정부가 차세대 원전인 소형모듈원자로(SMR)를 국가 에너지 정책의 핵심 사업으로 삼아 메가프로젝트를 본격 가동하기로 결정했다.",
    "summaryLong": "한국 정부가 차세대 원자력 에너지의 핵심 기술인 소형모듈원자로(SMR) 개발과 상용화를 국책사업으로 추진하는 메가프로젝트를 공식 시작했다. 이는 에너지 안보 강화, 탄소중립 달성, 그리고 글로벌 원자력 시장에서의 기술 리더십 확보라는 다층적 전략 목표를 반영한다. 정부의 집중적 정책 지원과 재정 투자를 통해 SMR 개발 인프라 구축, 인력 양성, 규제 정비 등 종합적 추진 체계가 갖춰질 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiW0FVX3lxTE9HME1UQjFLY1V3b1psLWpmS0FPN1pVTVJESVB4ZjJvZFpKYi1fNk5HTmdINWlxNjFCMUxwNDZaNnJBTG9HYUlpWWFPeHc1dVh5ektQcU50VlZiX2_SAWBBVV95cUxNREtQZ2k3Q2ltS0R1TjZXSVRKbmY4Z3ZzTkVFSHYxTjZ2b1V1WWwwcjJkX3BGWlM3OENtajBYRnc5bWd6Y1E5Tk10eUM1dEthd1RHcjZQRVE0TEFmLXhWdEc?oc=5",
    "k": "정부 차세대 원전 승부수 smr 메가프로젝트 본격화",
    "id": "1244eb55"
  },
  {
    "date": "2026-05-19",
    "title": "소형모듈원전·AI로 2031년까지 글로벌 원자력 최강국 달성 청사진",
    "summary": "한국이 소형모듈원자로와 인공지능(AI) 기술을 활용하여 2031년까지 글로벌 원자력 최강국이 되기 위한 전략을 마련했다.",
    "summaryLong": "한국 정부가 소형모듈원자로(SMR) 기술과 인공지능(AI)을 결합한 차세대 원자력 전략을 통해 2031년을 목표로 글로벌 원자력 분야 최강국 달성을 위한 종합적 청사진을 제시했다. 이 계획은 SMR의 상용화 촉진, 국제 표준 선점, 수출 확대 등을 포함하며, 한국의 원자력 산업 경쟁력을 대폭 강화하기 위한 중장기 국책 프로그램을 의미한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiVEFVX3lxTE5VV214bzhocl96M0hZUGtMcEdNVGJlTHVnRDBCUkphT0gwMTR3dHJMRkFqTFJCQzJrblMtT0ZxbFI5amU1YkN0MS1vTU5qeUU0dHVKOA?oc=5",
    "k": "소형모듈원전 ai 내세워 2031년까지 글로벌 원자력 최강국 청사진 마련",
    "id": "a3cdfc43"
  },
  {
    "date": "2026-05-15",
    "title": "미국 에너지부(DOE), 소형모듈원자로 배치 가속화를 위해 8개사에 9,400만 달러 지원",
    "summary": "미국 에너지부(DOE)가 소형모듈원자로 배치 가속화를 위해 8개 기업에 총 9,400만 달러를 지원한다.",
    "summaryLong": "미국 에너지부(DOE)는 소형모듈원자로의 상용화 속도를 높이기 위해 8개 기업에 대규모 자금을 지원한다. 9,400만 달러 규모의 지원은 미국 정부의 소형모듈원자로 개발 정책의 강한 의지를 보여준다. 지원 대상 기업들은 기술 개발, 제조 인프라 구축, 인허가 취득 등 다양한 분야에서 활용할 수 있다. 이러한 정부 지원은 소형모듈원자로의 시장 진입 시간을 단축하고 산업 경쟁력 강화에 기여할 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMif0FVX3lxTE5RTUVJaUpaNUVKSWliWDNuUXdmNXZmVTRzTXI5cG81VThFY3B1aUlNZ19maVN4c25tRGZRZHNReTNqaE1Ic3VOY1UxcG85c0NYemVpbWpUU1FXTGlzNm9xSlNLOEw5YmR0djE4TUtST1VWUFhHWXllT3lXNEhnZzQ?oc=5",
    "k": "doe awards 94m to 8 companies to accelerate small modular reactor deployment",
    "id": "d3188cc4"
  },
  {
    "date": "2026-05-15",
    "title": "소형모듈원자로(SMR) 시장 규모, 연 14.8% 성장률로 확대 예상",
    "summary": "소형모듈원자로 시장이 연 14.8% 성장률로 확대될 것으로 전망되고 있다.",
    "summaryLong": "글로벌 소형모듈원자로 시장은 탄소중립 전환과 에너지 안보 강화 수요에 따라 빠른 성장 추세를 보이고 있다. 연 14.8%의 복합연간성장률(CAGR)은 업계 전문가들의 낙관적 전망을 반영한다. 이러한 성장은 기술 성숙도 향상, 규제 환경 개선, 정부 지원 확대 등 여러 요인에 의해 견인될 것으로 예상된다. 향후 10년간 소형모듈원자로는 전 지구적 에너지 전환의 중요한 역할을 담당할 것으로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMicEFVX3lxTE82T0NkTkhWVjd0cjI2RGJaeEkyVEhNUks2ZzdrZm90QnNWOVdTM0UweFduVnhyQXYxczhQV2hEc2ZCS1VJWnNlYl94bVFLWVdjN0Y3V3N1Uk05cGtOWkt1cDROeU0wcjloVXNQRGxyQXA?oc=5",
    "k": "nuclear small modular reactors market size cagr of 14 8",
    "id": "e1416aad"
  },
  {
    "date": "2026-05-12",
    "title": "소형모듈원자로 폐기물 관리 통합 연구",
    "summary": "VTT(핀란드 기술연구센터)가 소형모듈원자로(SMR)의 폐기물 관리 통합 방안에 대한 연구를 진행하고 있다.",
    "summaryLong": "핀란드 기술연구센터(VTT)가 소형모듈원자로(SMR) 운영 시 발생하는 방사성폐기물 관리를 효율적으로 통합하기 위한 기술 개발을 추진하고 있다. 이 연구는 SMR의 경제성 및 안전성 향상을 위한 중요한 기술 과제로, 폐기물 취급의 표준화 및 최적화를 목표로 한다. 연구 결과는 향후 SMR 설계 및 운영 기준에 반영될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "VTT",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "integrating waste management into small modular vtt news",
    "id": "8684c2b2"
  },
  {
    "date": "2026-05-12",
    "title": "SMR·핵연료물질 안전 규제 혁신을 위한 원자력안전법 개정",
    "summary": "한국이 소형모듈원자로(SMR)와 핵연료물질의 안전 규제를 현대화하기 위해 원자력안전법을 개정했다.",
    "summaryLong": "한국 정부가 소형모듈원자로(SMR)와 핵연료물질 분야의 규제 혁신을 추진하며 원자력안전법을 개정했다. 이번 개정은 SMR의 설계·건설·운영 단계에서 필요한 안전기준을 현대화하고, 핵연료물질의 관리·수송·저장 규제를 합리화하는 것을 목표로 한다. 이를 통해 한국의 SMR 및 첨단 핵연료 기술 개발을 법적으로 뒷받침하고, 규제 체계의 국제 경쟁력을 강화하려는 의도로 해석된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiXkFVX3lxTFAtNlVILTBEakJCWS1veE5JQVR1UGxiV0NuUmtGR3BxQktDeTlubWNxSnRMbXpWT2VRRkp3LV9IVzlwdzFnbXB3T0hKQ3BmOWFRT2swNnNiT1k3SmhsWUE?oc=5",
    "k": "소형모듈원자로 smr 핵연료물질 안전 규제 혁신을 위한 원자력안전법 개정",
    "id": "8121f3c1"
  },
  {
    "date": "2026-05-11",
    "title": "소형모듈원자로(SMR), 기술이 아닌 경제성에서 경쟁력 부족",
    "summary": "분석에서 소형모듈원자로의 경제성 문제가 기술 문제보다 더 심각한 것으로 지적되었다.",
    "summaryLong": "최근 산업 분석에 따르면 소형모듈원자로는 기술적으로는 성숙도를 갖추고 있으나 경제성에서 여전히 개선이 필요한 것으로 평가되고 있다. 높은 건설비, 초기 자본 투자 부담, 대량 생산에 따른 규모의 경제 미실현 등이 주요 과제로 지적되었다. 이러한 경제성 문제는 상용화 확대와 시장 점유율 확보에 걸림돌이 될 수 있다. 업계는 대량 생산 체계 구축과 비용 최적화 방안 추진을 통해 경쟁력 제고에 주력하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.-AUFVX3lxTE5JQnVoVTNFMlFBSGR5MkRLTEdKUWloZlB3a1FRbFByZWdxTkd4Nkl6UVc1RVpnQWIyVjFnaDUzUDJ5TUluMlBtSGFPTXJ4RGcxcEFpZEtMR2tNbHNSb2t6YkdYQWRhbjVMZnlQWXNwaUxYSVZJb191RnIyaV9xYjZkSkxja0l4YUx2ZXppNXdMYTB0N1JJdW1ZeUl2UGVKRFhDRlV5bkNDb19oY0l0SllGaUt0TWdwbFJLZ2RyMGc?oc=5",
    "k": "smrs aren t losing on technology they re losing on economics crude oil prices today",
    "id": "8414c09c"
  },
  {
    "date": "2026-05-08",
    "title": "미국 뉴햄프셔 SMR 법안 통과, 2030년대 원전 배치 본격화",
    "summary": "미국 뉴햄프셔(New Hampshire) 주에서 SMR 배치를 지원하는 법안이 통과되어 2030년대 상용화가 가속화될 전망이다.",
    "summaryLong": "미국 뉴햄프셔 주 의회에서 소형모듈원자로(SMR) 배치를 지원하는 법안이 통과했다. 이 법안은 SMR 설치 허용 기준, 인센티브, 규제 절차를 정비하여 2030년대 상용 SMR 배치를 본격화하려는 정책이다. 뉴햄프셔를 포함한 미국의 SMR 도입 가속화는 탄소중립 목표 달성과 지역 전력 공급 다원화 전략의 일환으로 평가된다.",
    "cat": "정책",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZEFVX3lxTE9pNVJXYVhzTm0zSWZ6R28xRG9BTUk0c09YRmtNcDRZQVpjY0RpZlppOGpFYVY4RzBvR21BNHByZTFtTDl2RmlVSUh4MlpLMUdDMjNDM2lqTGtxaW1uM3NGMDNIUV8?oc=5",
    "k": "뉴햄프셔 smr 법안 통과 2030년대 원전 배치 본격화",
    "id": "9c50fe92"
  },
  {
    "date": "2026-05-04",
    "title": "바다 위에 띄우는 원자로 MSR, 2035년 건조 착수",
    "summary": "해상에 설치되는 용융염원자로(MSR) 건조가 2035년경 시작될 것으로 예상된다.",
    "summaryLong": "바다 위에 설치되는 용융염원자로(MSR, Molten Salt Reactor) 개발이 진행 중이며, 건조 착수가 2035년으로 계획되고 있다. 용융염 냉각 방식의 첨단 원자로로서 해상 플랫폼에 배치되어 해양 산업, 담수화, 열 공급 등 다양한 산업용 에너지원으로 활용될 전망이다. 이는 SMR 및 첨단로 개발의 국제적 경쟁에서 혁신적 노형 개발이 진행 중임을 보여주는 사례이다.",
    "cat": "기술",
    "type": "MSR",
    "dev": "",
    "region": "",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMicEFVX3lxTE53ZUhUcnpoTHItd3l5MW5PcXZZTTBUcW1wRzI5U3lZbTVBY2huaDZsM09jNGdxczh4UjdObHhKVkpoaWFfcDJYVERxZGdGUXI5cFd3c3Q4a0x6SUIyOUQwTzZOdXY5NWx5aXVqdjZSeng?oc=5",
    "k": "smr 패권경쟁 바다 위에 띄우는 원자로 msr 2035년 건조 착수",
    "id": "7fc4922e"
  },
  {
    "date": "2026-05-01",
    "title": "LG&E and KU, 소형모듈원자로(SMR) 도입 검토",
    "summary": "미국 전력회사 LG&E and KU가 소형모듈원자로 도입 가능성을 탐색 중이다.",
    "summaryLong": "미국의 전력회사 LG&E and KU가 소형모듈원자로(SMR) 도입 검토를 진행 중이다. 이는 미국 내 전통 전력회사들의 신형 원자력 기술에 대한 관심 증가를 보여주는 사례다. 소형모듈원자로는 기존 대형 원자로 대비 소형·모듈식으로 설계되어 유연한 배치와 낮은 초기 투자가 특징이다. 향후 미국 내 SMR 도입 추진이 확대될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "LG&E and KU",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiqwFBVV95cUxNT3A1N3ZfV0tjekJmSzhLalpNU3N1bm11WVFpOWhzOVRxX2tEUWt1ZmRNWnFuRXVyaXVzZG1EdEFmNnZTZVBieTIyUlRSbWkwMWNCU2tGUlBZRDhPTEkzcVgwdUpYODF1dExwZXZyM0VONGVYeHdrOVM2d3E5RFBoZzAxWlQzLUlSalhTSlZ3SFNvWU5GQ3ExNmRQQ1c2bGo0dkpwYXAtZlRlWVE?oc=5",
    "k": "lg e and ku exploring use of small modular reactor nuclear energy",
    "id": "153886b7"
  },
  {
    "date": "2026-05-01",
    "title": "다링턴(Darlington) SMR 프로젝트, 기초 모듈 마일스톤 달성",
    "summary": "캐나다 다링턴의 소형모듈원자로 프로젝트가 기초 모듈 완성이라는 주요 기술 마일스톤을 달성했다.",
    "summaryLong": "온타리오 전력공사(Ontario Power Generation)가 추진 중인 캐나다 다링턴 소형모듈원자로 프로젝트가 기초 모듈 완성 마일스톤을 달성했다. 이는 상용화를 향한 중요한 기술적 진전을 의미한다. 다링턴 프로젝트는 캐나다의 탈탄소 에너지 공급 전략의 핵심 사업으로 진행 중이다. 향후 설계 확정 및 규제 인허가 단계로 진행될 예정이다.",
    "cat": "기술",
    "type": "General",
    "dev": "Ontario Power Generation",
    "region": "CA",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMipAFBVV95cUxOZS1YMUFUeGdvMlNtb2JDM3RpYjE5c0FBbUswa1VyY3NNOHloUlhJTzdZWlZRQzU2ZUxza0lzRV9tdWxCNS00SVY3cjJGalp6akIybndhdFI1ZV80RUxGejhCQXJJUldzMEFCSlp3Q01vbVprUDBpd0pkMTlzRllkb0tUNHc1aF95ZUF1T0E3eGFXQWd6bGxNcXl1TTRIVzRhOXZicA?oc=5",
    "k": "darlington smr project s foundation module milestone",
    "id": "e7266e3f"
  },
  {
    "date": "2026-04-30",
    "title": "기장군, i-SMR 유치 공감대 확산...경주와 경쟁 본격화",
    "summary": "기장군이 i-SMR 유치에 대한 주민 공감대를 확산시키며 경주시와의 본격적인 유치 경쟁에 나선다.",
    "summaryLong": "부산시 기장군이 i-SMR(혁신형 소형모듈원자로) 유치를 위한 주민 공감대 확산에 나서며 경주시와의 유치 경쟁을 본격화하고 있다. i-SMR은 한국형 소형모듈원자로로 국내 원자력 산업의 핵심 사업이다. 기장군과 경주시는 각각 유치의 경제적·기술적 이점을 강조하며 경쟁 중이다. 향후 정부의 입지 선정 결정에 따라 지역 원자력 산업의 거점이 결정될 것으로 예상된다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "i-SMR",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "기장군 i smr 유치 공감대 확산 경주와 유치 경쟁 본격화",
    "id": "670a51e9"
  },
  {
    "date": "2026-04-29",
    "title": "미국 핵규제위원회(NRC) Part 53 발효, 첨단로 전용 인허가 트랙 개설",
    "summary": "위험정보·성능기반·기술포괄 신규 인허가 트랙 발효. 현재 신청자 0건이나 향후 비경수로 소형모듈원자로의 주요 인허가 경로 역할 전망.",
    "cat": "정책",
    "type": "General",
    "dev": "NRC",
    "region": "US",
    "source": "Federal Register 2026-06048",
    "url": "https://www.ecfr.gov",
    "summaryLong": "미국 핵규제위원회(NRC)의 Part 53 규정이 발효되어 첨단원자로 전용 인허가 트랙이 공식 개설되었다. 이 트랙은 위험정보 기반(Risk-informed)·성능기반(Performance-based)·기술 포괄적 특성을 갖춘 신규 인허가 경로다. 현재 신청자는 없으나 비경수로 및 소형모듈원자로(SMR) 개발사들의 향후 활용이 예상된다. Part 53은 기존 인허가 체계(Part 50, 52)의 한계를 보완하여 혁신형 원자로의 상용화 진입장벽을 낮출 것으로 평가된다.",
    "id": "313af40b"
  },
  {
    "date": "2026-04-29",
    "title": "Aker Solutions, Rolls-Royce SMR과 MOU 체결",
    "summary": "Aker Solutions가 Rolls-Royce SMR과 소형모듈원자로 에너지 프로젝트 양해각서를 체결했다.",
    "summaryLong": "노르웨이 해상 에너지 기업 Aker Solutions가 영국의 Rolls-Royce SMR과 소형모듈원자로 에너지 프로젝트 협력을 위한 양해각서(MOU)를 체결했다. 이는 산업계와 에너지 공급자 간 SMR 활용을 위한 협력 확대를 보여준다. Aker Solutions의 해상 기술과 Rolls-Royce의 SMR 기술 결합은 해상·육상 에너지 공급 다양화에 기여할 것으로 예상된다. 양사는 향후 구체적인 프로젝트 개발을 진행할 예정이다.",
    "cat": "계약",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "aker solutions signs mou with rolls royce smr for small modular reactor energy projects",
    "id": "29ea8e90"
  },
  {
    "date": "2026-04-29",
    "title": "대림산업, 미국 소형모듈원자로 회사 지분 6배 확대",
    "summary": "대림산업(DL E&C)이 미국의 소형모듈원자로 회사에 대한 지분을 6배 증가시켰다.",
    "summaryLong": "한국의 대림산업(DL E&C)이 미국의 소형모듈원자로 개발사에 대한 지분을 6배로 대폭 확대했다. 이는 국내 건설·에너지 기업의 SMR 산업 진출 강화를 의미한다. 미국 SMR 시장의 성장성과 기술 선도성을 고려한 전략적 투자로 평가된다. 향후 대림산업은 미국 SMR 프로젝트 참여 확대 및 기술 협력을 통해 국내 원자력 산업 경쟁력 강화에 기여할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "DL E&C",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "dl e c s stake in u s small modular reactor firm jumps sixfold",
    "id": "963ae4c6"
  },
  {
    "date": "2026-04-29",
    "title": "2035년 소형모듈원자로(SMR) 선박 건조 착수 계획",
    "summary": "중국의 원자력 선박 개발에 대응하여 2035년 소형모듈원자로 선박 건조 착수를 계획하고 있다.",
    "summaryLong": "한국이 중국의 원자력 선박 개발 추진에 대응하여 2035년부터 소형모듈원자로(SMR) 기술을 적용한 선박 건조를 시작할 계획을 제시했다. 이는 원자력 추진 선박 분야에서 중국과의 기술 격차를 좁히고 글로벌 경쟁력을 확보하기 위한 전략으로, 조선 산업과 원자력 기술의 융합을 통해 새로운 성장 동력을 확보하려는 의도를 반영한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiVEFVX3lxTE8zSDFlUGlUVnpaMnpuSHFPMlp4ME9nbDhXVkVtQjFIMHRJZUdfUGVpLXMtbDVncEx0c3VCNW45SEdpN0ZFQlQ1d3BEa2p2UUN6bE45Zg?oc=5",
    "k": "2035년 소형원자로 선박 건조 착수 중국 따라잡을 것",
    "id": "4a853060"
  },
  {
    "date": "2026-04-28",
    "title": "경남, 275억 규모 SMR 시험검사지원센터 유치",
    "summary": "경상남도(Gyeongnam)가 275억 원 규모의 소형모듈원자로 시험검사지원센터를 유치했다.",
    "summaryLong": "전기신문은 경상남도(Gyeongnam)가 275억 원 규모의 소형모듈원자로(SMR) 시험검사지원센터 유치를 보도했다. 시험검사지원센터는 SMR 부품·설비의 인증, 안전성 검증, 표준화 등을 담당하는 핵심 인프라로서 한국 SMR 산업 생태계 구축의 중요한 이정표이다. 이는 정부의 SMR 국책사업 추진과 지역 신산업 육성의 결과로, 향후 SMR 기술 상용화와 글로벌 수출을 지원할 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "경남 275억 규모 smr 시험검사지원센터 유치",
    "id": "07984727"
  },
  {
    "date": "2026-04-27",
    "title": "X-Energy IPO 후 1.5배 급등, AI 전력 수요 견인",
    "summary": "소형모듈원자로 개발사 X-Energy가 기업공개(IPO) 직후 1.5배 상승하며 AI 전력 수요 증가에 주목받고 있다.",
    "summaryLong": "미국의 SMR 개발사 X-Energy가 기업공개(IPO) 직후 주가가 1.5배 상승했다. 이는 인공지능(AI) 데이터센터의 전력 수요 급증에 따른 소형모듈원자로에 대한 투자자 관심 증가를 반영한다. AI 산업의 급속한 확대로 기존 에너지 인프라의 한계가 드러나면서 청정·고효율 전력 공급원으로서 SMR의 가치가 부각되고 있다. 향후 AI 전력 수요 증가와 함께 SMR 개발사들의 시장 가치 상승이 지속될 것으로 전망된다.",
    "cat": "투자",
    "type": "HTGR",
    "dev": "X-Energy",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "small modular reactor stocks soaring x energy surges 1 5x right after ipo is ai power hunger accelerating capital shifts spillover effects on related stocks",
    "id": "7ed222ac"
  },
  {
    "date": "2026-04-27",
    "title": "캐나다 온타리오주·유콘 준주, 소형모듈원자로 개발 협력 추진",
    "summary": "캐나다 온타리오주와 유콘 준주가 소형모듈원자로 개발을 위한 파트너십을 체결했다.",
    "summaryLong": "캐나다의 온타리오주와 유콘 준주가 소형모듈원자로(SMR) 개발 협력을 위한 파트너십을 체결했다. 이는 캐나다 내 지역 간 SMR 산업 협력 확대 및 탈탄소 에너지 전환 가속화를 의미한다. 온타리오주는 이미 다링턴 SMR 프로젝트를 추진 중이며, 유콘 준주의 참여로 북부 지역의 에너지 자립성 강화가 기대된다. 향후 양 지역은 기술 공유, 규제 협력 등을 통해 캐나다의 SMR 선도적 지위를 공고히 할 전망이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "CA",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "ontario yukon partner on small modular reactor development",
    "id": "fe43a092"
  },
  {
    "date": "2026-04-27",
    "title": "미국 내 개발 중인 소형모듈원자로(SMR)와 마이크로원자로",
    "summary": "미국 에너지정보청(EIA)이 미국에서 개발 중인 소형모듈원자로와 마이크로원자로에 대한 현황을 정리했다.",
    "summaryLong": "미국 에너지정보청(EIA)은 미국 내에서 개발되고 있는 소형모듈원자로(SMR)와 마이크로원자로의 현황을 소개하고 있다. 이는 차세대 원자력 기술의 발전 동향을 추적하고 미국의 소형 원자로 개발 생태계를 보여주는 기초 정보 제공 목적으로 보인다. 미국 정부는 에너지 안보와 탄소중립 목표 달성을 위해 이러한 첨단로 기술 개발을 적극 지원하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiY0FVX3lxTE5uMlpRX0FJc01oVmJ6VXY1d3VreWdEVTR2RFNZQzgzcUotdC1OUkFwWlhmVENxYzdUanp1MV9Ga1VIQ1p0cDhsNDVGMlNiNE1VY1JHc2g1c2dGc2MxTDduWmVSMA?oc=5",
    "k": "small modular reactors and microreactors under development in the united states",
    "id": "540d669f"
  },
  {
    "date": "2026-04-25",
    "title": "경남도, SMR 패스트트랙 원자력안전법 개정안 국회 통과 환영",
    "summary": "경남 지역이 소형모듈원자로(SMR) 인허가 절차 단축을 위한 원자력안전법 개정안 통과를 환영했다.",
    "summaryLong": "한국 경상남도가 소형모듈원자로(SMR) 개발·건설 인허가 절차를 신속 처리(패스트트랙)하도록 하는 원자력안전법 개정안이 국회를 통과한 것을 환영하는 입장을 표명했다. 이는 SMR 사업의 규제 애로 해소와 건설 일정 단축을 도모하는 정책으로, 경남이 국내 SMR 산업의 주요 거점이 되기를 원하는 지자체의 이해관계를 반영하고 있다. 안전법 개정을 통한 인허가 절차 개선은 SMR 산업화의 제도적 기반 구축을 의미한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "경남도 smr 패스트트랙 원자력안전법 개정안 국회 통과 환영",
    "id": "41c7dab6"
  },
  {
    "date": "2026-04-24",
    "title": "X-Energy, 소형모듈원자로(SMR) 개발사 최초 IPO 성공",
    "summary": "소형모듈원자로 개발사 X-Energy가 공모가 범위를 초과하여 주당 $23에 신규공개상장(IPO)을 실시했다.",
    "summaryLong": "X-Energy는 소형모듈원자로 기술 개발을 추진 중인 기업으로, 이번 IPO는 SMR 분야 기업의 상장 사례로 주목된다. 공모가 범위를 초과한 수요를 받아 주당 $23에 상장했으며, 이는 민간 핵에너지 시장의 투자 심화를 보여준다. 향후 SMR 기술 상용화 추진에 자금을 투자할 것으로 예상된다.",
    "cat": "투자",
    "type": "HTGR",
    "dev": "X-Energy",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "xe ipo news small modular reactor developer x energy prices upsized ipo at 23 above the range",
    "id": "06d46f9b"
  },
  {
    "date": "2026-04-24",
    "title": "인도의 SMR 전략과 야망",
    "summary": "인도의 소형모듈원자로(SMR) 개발 전략과 향후 계획을 분석한 기사.",
    "summaryLong": "ORF 미들이스트(ORF Middle East)는 인도의 소형모듈원자로(SMR) 전략을 보도했다. 인도는 에너지 자립과 탈탄소 목표 달성을 위해 SMR 개발에 대규모 투자를 추진하고 있으며, 이는 인도 정부의 국책 에너지 정책의 일환이다. SMR은 인도의 분산된 지역 전력 공급과 산업용 열 수요 충족을 위한 전략적 기술로 평가되고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMirAFBVV95cUxPdHJJWjFxWkdQa0oxdUFiZDJOQ2UtVVczcU94QWpDcXNXeGd5UThKOFRrUDAzRzRQOE96cmFpVGlINVRldTY3bzZTZ1htSE4zZGE4NW5rb2F4cG43LUwwWnhhT2pjWU5EX1NFZHZEZVkyRGdZRkh2TWdIMnFwREUwYXduYzBhS3FtUmV0enlSNWtDbHVxWjZ1dnNFSnYtYXQzQTBJSWcxS0l3YXJt?oc=5",
    "k": "big aspirations for small modular reactors understanding india s strategy on smrs",
    "id": "ecd18210"
  },
  {
    "date": "2026-04-22",
    "title": "캐나다 온타리오·유콘, 소형모듈원자로 공동 개발 추진",
    "summary": "캐나다의 온타리오(Ontario)주와 유콘(Yukon) 준주가 소형모듈원자로 개발을 공동 추진하기로 결정했다.",
    "summaryLong": "캐나다의 온타리오주와 유콘 준주는 SMR 기술 개발을 위한 협력 체계를 구축했다. 이는 캐나다 북부·원격 지역의 에너지 수급 문제 해결을 목표로 하고 있다. 양 지역은 SMR의 소형성, 모듈식 구조 등의 장점을 활용하여 지역별 에너지 자립도 제고를 추진할 계획이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "CA",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "ontario and yukon working together to advance small modular reactor development",
    "id": "6f9ac32e"
  },
  {
    "date": "2026-04-16",
    "title": "미국 원자력 회사, 240MW 소형모듈원자로 건설 계획 제출",
    "summary": "미국의 한 원자력 회사가 240MW 규모의 소형모듈원자로 건설 계획을 관계 당국에 제출했다.",
    "summaryLong": "미국 원자력 회사가 240MW 용량의 SMR 건설 계획을 공식 제출했다. 이는 중소규모 전력 수요를 충족시키기 위한 SMR 실증 프로젝트로 평가된다. 승인 시 미국 내 SMR 상용화의 중요 사례가 될 것으로 예상되며, 향후 미국의 에너지 안보 강화에 기여할 수 있을 것으로 전망된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMihgFBVV95cUxPQUlUOFNSMm1qNnJwWVk5YjJSUlNTUFJMZ3RaQnlLV2NDbE1rbk1reTJ6djFmUnNEeWo4VHVsaGdEVktyRXZVQUxUZ0dMWU84dlZmZG5BdVE5WXRsRlRicXZQYzNIN0VXYXNxNElOdlRtdTc0ZldUWWs2ZG54SjEwd1JMcWxOQQ?oc=5",
    "k": "us nuclear firm submits plan for 240 mw small modular reactor to power 1 5 million homes",
    "id": "3a35a14a"
  },
  {
    "date": "2026-04-15",
    "title": "First American Nuclear, 고속 스펙트럼 소형모듈원자로 규제 협력 계획 제출",
    "summary": "First American Nuclear이 고속 스펙트럼 소형모듈원자로(SMR)에 대한 규제 협력 계획을 규제당국에 제출했다.",
    "summaryLong": "First American Nuclear은 고속 스펙트럼 기술 기반 SMR 개발을 추진 중인 기업으로, 규제당국과의 사전 협력 체계를 마련했다. 고속로(Fast Reactor) 기술은 기존 경수로 대비 연료 활용 효율성이 높고 폐기물 감소 효과가 있다. 규제 협력 계획 제출은 향후 설계 인증(Design Certification) 단계로의 진행을 준비하는 과정으로 해석된다.",
    "cat": "기술",
    "type": "SFR",
    "dev": "First American Nuclear",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "first american nuclear submits regulatory engagement plan for fast spectrum small modular reactor smr",
    "id": "f8d808db"
  },
  {
    "date": "2026-04-15",
    "title": "X-Energy, 7억 5천만 달러 규모 신규공개상장(IPO) 공시",
    "summary": "소형모듈원자로 개발사 X-Energy가 7억 5천만 달러 규모의 신규공개상장(IPO)을 진행한다.",
    "summaryLong": "X-Energy는 소형모듈원자로 기술 개발 회사로, 약 7억 5천만 달러 규모의 공개 자금 조달을 추진했다. 이는 민간 원자력 기술 분야에 대한 자본시장의 높은 관심을 반영한다. IPO를 통한 자금 조달은 기술 개발 및 상용화 추진에 활용될 예정이며, SMR 산업 성장의 가속화를 예상하게 한다.",
    "cat": "투자",
    "type": "HTGR",
    "dev": "X-Energy",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "xe ipo news small modular reactor developer x energy sets terms for 750 million ipo",
    "id": "d2827aed"
  },
  {
    "date": "2026-04-13",
    "title": "롤스로이스, 영국 첫 SMR 발전소 공급 계약 체결",
    "summary": "롤스로이스(Rolls-Royce)가 영국 최초의 소형모듈원자로(SMR) 발전소 공급 계약을 확보했다.",
    "summaryLong": "롤스로이스(Rolls-Royce)는 영국 내 첫 번째 소형모듈원자로(SMR) 발전소 건설 및 공급 계약을 성공적으로 체결했다. 이 계약은 영국의 SMR 상용화 추진 전략에 있어 획기적인 마일스톤으로, 정부 정책 지원 하에 민간 산업이 구체적인 프로젝트 계약으로 진전하고 있음을 의미한다.",
    "cat": "계약",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "rolls royce secures deal to deliver uk s first small modular reactor power plant",
    "id": "9c0260ea"
  },
  {
    "date": "2026-04-08",
    "title": "SMR 경쟁 대응 '원자력 진흥법' 개정 추진, 기술·인력·재정 3축 강화",
    "summary": "한국이 국제 SMR 경쟁에 대응하기 위해 원자력 진흥법 개정을 통해 기술·인력·재정 지원을 강화할 계획이다.",
    "summaryLong": "한국 정부가 글로벌 소형모듈원자로(SMR) 경쟁에서 경쟁력을 확보하기 위해 원자력 진흥법 개정을 추진하고 있다. 개정안은 SMR 기술 개발 지원, 원자력 전문 인력 양성, 관련 사업 투자·펀딩 등 기술·인력·재정의 3대 축을 강화하는 내용을 담고 있다. 이는 정부 차원의 종합적인 SMR 산업화 전략이자 국내 원자력 산업의 미래 경쟁력 확보를 위한 정책 노력으로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMickFVX3lxTFBTN3pfT0s4TGZLU2hXbmlUY3djM2xDV1J1dGU3ODYzdTRLMENmVFpMVkNEYWRYaGtVWFpJN0E0cUp5YjlWbzc2QjZpdkpHcTJGeDdiQi1VbWp1U29tZ1VjYmt2ck1FZGVtYlZpQlRWT3A4QQ?oc=5",
    "k": "smr 경쟁 대응 원자력 진흥법 개정 추진 기술 인력 재정 3축 강화",
    "id": "664231a6"
  },
  {
    "date": "2026-04-06",
    "title": "Fluor, X-Energy와 텍사스(Texas) 첨단원전 계약 체결",
    "summary": "건설회사 Fluor가 X-Energy와 텍사스주의 첨단 소형모듈원자로 프로젝트 계약을 체결했다.",
    "summaryLong": "건설 및 엔지니어링 회사 Fluor와 SMR 개발사 X-Energy가 텍사스에서의 첨단원전 프로젝트 계약을 체결했다. 계약은 설계, 건설, 운영 관련 역할 분담을 포함하고 있다. 이 협력은 미국 내 SMR의 상용화 추진을 가속화하며, 향후 유사 프로젝트 확산의 모범 사례가 될 가능성이 있다.",
    "cat": "계약",
    "type": "HTGR",
    "dev": "X-Energy",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "fluor signs contract with x energy for advanced nuclear project in texas",
    "id": "a23bde45"
  },
  {
    "date": "2026-04-04",
    "title": "팰리세이즈(Palisades) 발전소에 소형모듈원자로 건설 준비 착수",
    "summary": "Palisades 발전소 부지에 새로운 소형모듈원자로 건설을 위한 준비 작업이 진행 중이다.",
    "summaryLong": "미국 팰리세이즈 발전소 부지가 소형모듈원자로 건설 프로젝트의 입지로 선정되어 준비 작업이 시작되었다. 유휴 부지의 재활용은 기존 에너지 인프라를 효율적으로 활용하는 전략이다. 팰리세이즈 SMR 프로젝트는 미국의 탄소중립 에너지 정책 추진 및 지역 경제 재활성화에 기여할 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMioAFBVV95cUxPWG9nSnBaRVo3OVliMnczMGNPOUltV19Dbld3Y2drLUdHNkFmU3JKSXlyS19aTVljeThZQTNEXzhOQzVyMmlxYUhaRlNmUGN6d0JrbTJ6TFFxRGI2Qlk2WFRDeHVLSWktLWtQcHNwX1dqRFh2c0U3dzN2azcxeVZvaVFORlZULUlkV0p0cm5KMG5ma2JsVGFRRG1TVlBod2VU?oc=5",
    "k": "prep work for new small modular reactors at palisades now underway",
    "id": "1c8f26ab"
  },
  {
    "date": "2026-03",
    "title": "TerraPower Natrium, 건설허가(CP) 발급 — 첫 상업용 비경수로 CP",
    "summary": "미국 원자력규제위원회(NRC)가 와이오밍(Wyoming) Kemmerer Natrium에 건설허가를 발급했으며, 약 40년 만의 상업용 제4세대(Gen-IV) 비경수로 허가이다.",
    "cat": "인허가",
    "type": "SFR",
    "dev": "TerraPower",
    "region": "US",
    "source": "DOE",
    "url": "https://www.energy.gov/ne/articles/nrc-issues-construction-permit-terrapowers-natrium-advanced-reactor",
    "summaryLong": "TerraPower의 Natrium 프로젝트가 와이오밍주 Kemmerer에서 미국 원자력규제위원회(NRC)로부터 건설허가(CP)를 획득했다. 이는 약 40년 만에 미국에서 발급되는 상업용 제4세대(Gen-IV) 및 비경수로(액체금속로) 건설허가로, 역사적 의의가 크다. Natrium은 액체 나트륨을 냉각재로 하는 고속로 기술로, 높은 에너지 밀도와 안전성을 장점으로 한다. 이 사업의 성공은 미국의 첨단 원자력 기술 발전 및 탄소중립 에너지 전환의 중요한 이정표가 될 것으로 전망된다.",
    "id": "9aef3916"
  },
  {
    "date": "2026-03-25",
    "title": "미국 BWRX-300(보라엑스-300) 프로젝트, 일본 투자 유입",
    "summary": "GE Hitachi(지이 히타치)의 BWRX-300 소형모듈원자로(SMR) 프로젝트에 일본 투자자들이 투자를 진행하고 있다.",
    "summaryLong": "GE Hitachi(지이 히타치)가 개발 중인 BWRX-300은 비등수형 원자로(BWR) 기반의 소형모듈원자로(SMR) 기술이다. 일본의 주요 투자자들이 이 프로젝트에 자금을 투입하고 있으며, 이는 일본의 원자력 기술 다각화 전략을 반영한다. BWRX-300은 수동 안전 기능과 설계 단순화를 특징으로 하여 건설 비용과 기간 감소를 목표로 하고 있다. 일본의 투자 유입은 프로젝트의 상용화 가능성을 높이고 국제 시장 진출의 기반이 될 것으로 예상된다.",
    "cat": "기술",
    "type": "BWR",
    "dev": "GEH",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMingFBVV95cUxQMzhndEdYYThMWTFrYWowUFhZY0EzYlhodjdJUENFbVV1c053aHdjNmU5eFNVdWJUNng4am9pU1VxNkpOcE1DbTBWSl96R2M3NzFXRi14U1NqRHJhMkNmX3ZFLVRXN2kwUUhXNVkwVUJMZHloZFB1R21rbUxLV3N4UFlEX0pnYjVmb2p6X29ubE0tY3RieU9rRF81azRyUQ?oc=5",
    "k": "new u s bwrx 300 projects get japanese investment american nuclear society ans",
    "id": "791da203"
  },
  {
    "date": "2026-03-24",
    "title": "핀란드, 미국 주도 소형모듈원자로 국제협력 프로그램 참여",
    "summary": "핀란드가 미국 주도의 소형모듈원자로 기술 책임 있는 이용 프로그램에 협력국으로 참여했다.",
    "summaryLong": "핀란드는 미국 에너지부(DOE) 주도의 소형모듈원자로(SMR) 책임 있는 이용에 관한 국제협력 프로그램에 가입했다. 이 프로그램은 SMR 기술의 안전하고 지속가능한 활용을 위한 국제 협력 체계를 구축하는 것을 목표로 한다. 핀란드는 선진 원자력 기술 국가로서 높은 원자력 안전 기준을 보유하고 있어 이 프로그램에 중요한 역할을 할 것으로 기대된다. 이번 참여는 핀란드의 탄소중립 에너지 전환 전략에도 부합한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "finland becomes contributing partner in us led programme promoting responsible use of small modular reactor technology finnish government",
    "id": "c9572491"
  },
  {
    "date": "2026-03-23",
    "title": "스웨덴 소형모듈원자로 발전소 건설허가 신청",
    "summary": "스웨덴에서 소형모듈원자로(SMR) 발전소 건설을 위한 허가 신청이 제출되었다.",
    "summaryLong": "스웨덴의 원전 운영 업체가 소형모듈원자로(SMR) 발전소 건설을 위한 규제 허가를 신청했다. 스웨덴은 높은 원자력 안전 기준과 투명한 규제 체계를 갖춘 국가로, SMR 도입을 통해 에너지 안정성과 탄소중립 목표를 동시에 달성하려 하고 있다. 허가 신청은 SMR 기술의 상용화 단계로의 진행을 의미하며, 유럽 시장에서의 SMR 확산 가능성을 시사한다. 스웨덴의 허가 사례는 다른 유럽 국가들의 SMR 도입 결정에도 영향을 미칠 것으로 전망된다.",
    "cat": "인허가",
    "type": "General",
    "dev": "",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMikAFBVV95cUxOQ2JCVWZlVnJLQ1ZzOXRsMTV3bHVjRlc1bzhSMTRlMnVnTUt1TDBLSnJBWTdUT2d2blc0Q3AydDhJOWN2UGRFNWVRc0ZQZ1huN1dKOEktLU1uRWF0TUR4QUJoN2lJVWcxRzFvQXVxckdMM2FNVnFvcWxxZ3BwTXc3cTJYNkVkRWdQeEg4QV9vRzg?oc=5",
    "k": "application submitted for swedish smr plant",
    "id": "adb2b719"
  },
  {
    "date": "2026-03-19",
    "title": "스웨덴 SMR 발전소 기획 진행 중",
    "summary": "스웨덴의 소형모듈원자로(SMR) 발전소 건설 계획이 진행되고 있다.",
    "summaryLong": "스웨덴에서 소형모듈원자로(SMR) 기반 발전소의 기획 및 사전 개발이 추진 중이다. 이는 스웨덴의 탄소중립 에너지 전환 전략 내에서 SMR의 역할을 확대하려는 움직임을 보여준다. 이러한 계획은 유럽 내 SMR 사업화의 중요한 사례가 될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiiwFBVV95cUxPdFZsSmlDd1J4d2ZJZkktR0JaZ1Z4V3VrTWt6eG5GT0ZvZU9LVTFoMjcyQVFvWVkxbS1fX1dhVlFnYldkbmtOVG5NdHpzcllRNVg4UFVWSi1XU2lLZjFIaVhrSWU3YWJQcERkZm5hQlgxdEU0eUh3NUVaU3oxWmZfWTZlWFVaMXpCekNV?oc=5",
    "k": "planning for swedish smr plant proceeds",
    "id": "7444dd20"
  },
  {
    "date": "2026-03-17",
    "title": "NuScale Power, 유일한 미국 NRC 인증 소형모듈원자로 기업",
    "summary": "NuScale Power는 미국 원자력규제위원회(NRC)로부터 인증받은 유일한 상용 소형모듈원자로(SMR) 기업이다.",
    "summaryLong": "NuScale Power는 미국 원자력규제위원회(NRC)로부터 설계 인증을 획득한 유일한 상용 소형모듈원자로(SMR) 기업의 지위를 유지하고 있다. 이는 NuScale의 기술이 미국의 가장 엄격한 원자력 규제 기준을 통과했음을 의미하며, 상용화의 가장 큰 장애물을 극복했다는 의미를 갖는다. 다른 SMR 기업들이 인증을 추진 중이지만, 현재까지 NuScale만이 최종 설계 인증을 획득했다. 이 지위는 NuScale에게 시장 진입에서 우선적 이점을 제공하고, 향후 원전 수주 및 투자 유치에 긍정적으로 작용할 것으로 예상된다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "NuScale",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "nuscale power inside the only nrc certified small modular reactor company nyse smr",
    "id": "93c0f5ac"
  },
  {
    "date": "2026-03-15",
    "title": "NuScale, 산업용 핵에너지 공급 역할 강조",
    "summary": "NuScale Power가 소형모듈원자로(SMR) 기술을 산업용 핵에너지 공급에 활용하는 역할을 부각하고 있다.",
    "summaryLong": "NuScale Power는 자사의 소형모듈원자로(SMR) 기술이 고온 열과 전력이 필요한 산업 부문에 핵에너지를 공급할 수 있음을 강조하고 있다. 수소 생산, 담수화, 산업용 열 공정 등 다양한 분야에서 SMR의 활용 가능성을 제시하고 있다. 이는 전통적인 대형 원전의 전력 생산 중심 역할에서 벗어나 산업 분야의 탈탄소화를 주도할 수 있는 기술로서 SMR을 포지셔닝하려는 전략이다. 산업용 핵에너지 공급은 글로벌 탄소중립 달성에 중요한 역할을 할 것으로 전망된다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "NuScale",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMigAFBVV95cUxOR01GZzl4N3VVSElXY3pVRnRYMks0TGFQbWpReHF2TnpqaU82X05uS1RiLWNkY2oybEJMT29Sa3JTSEdNS05JYXVrcVlsVUdfcjZiai1KakR3NVBDTkRXbDBxQ2xfa0tFdjRlWjZ1UDY2aGI2Wk83MzczYjMwWXJwTA?oc=5",
    "k": "nuscale highlights role of its small modular reactor technology in supplying dependable industrial nuclear energy",
    "id": "dce67c4a"
  },
  {
    "date": "2026-03-13",
    "title": "Rolls-Royce SMR, 규제 정당성 검토 진행",
    "summary": "영국의 Rolls-Royce가 자사 소형모듈원자로에 대한 규제 정당성(Regulatory justification)을 제시하고 있다.",
    "summaryLong": "영국의 Rolls-Royce는 자사가 개발 중인 소형모듈원자로(SMR)에 대해 규제 당국에 규제 정당성을 입증하는 과정을 진행하고 있다. 규제 정당성 검토는 원자로 도입의 사회적·경제적·환경적 편익을 증명하는 단계로, 영국의 원자력 규제 체계에서 필수 절차이다. Rolls-Royce의 SMR은 영국의 에너지 안정성과 탈탄소화 목표에 부합하는 기술로 평가되고 있다. 규제 정당성 검토의 완료는 향후 허가 및 상용화 단계로의 진행을 앞당길 것으로 예상된다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMikgFBVV95cUxQVXVTM2xpQS15TjRLWTN2S2dMUjZfckVZRVJMZndPRTRNVHl4bl9OVHd1TzF1V016ZHpmZ3N6Q1BWRGxfZTJ4RG44dXBOVTJFTHoyNHFZZW5zUkVVbWVnRzZTbFFwMUdBeV8yRW5WNnp6ZHUxY0ZHMDdReHBmSGdCZTdQemJLUDhUOVFRNndaZGtKdw?oc=5",
    "k": "regulatory justification for rolls royce smr",
    "id": "b5311d54"
  },
  {
    "date": "2026-03-12",
    "title": "소형모듈원자로·마이크로원자로 전문가 인사이트",
    "summary": "미국 정책연구소가 소형모듈원자로(SMR)와 마이크로원자로에 대한 전문가 의견을 제시하고 있다.",
    "summaryLong": "미국의 정책연구기관이 소형모듈원자로(SMR)와 마이크로원자로(Microreactor) 기술에 대한 전문가들의 관점과 분석을 공개했다. 전문가들은 SMR이 분산형 에너지 공급, 원격 지역 전력 공급, 산업용 핵열 등 다양한 응용 분야에서의 잠재력을 강조했다. 마이크로원자로는 더 소형화되어 극한 환경에서의 활용 가능성이 높은 것으로 평가되었다. 이러한 전문가 의견은 정책 결정자들과 투자자들의 SMR 및 마이크로원자로 기술에 대한 인식을 높이는 데 기여할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMihAFBVV95cUxPaURLN1hFQWVNZFNLdHc5R2R1VnZiQUlSS2N4ejVYWnpVMXBTSEltampDMWhOUWpzT0k4ZW40VlR1UFg4NFNjdEFZendzZFd2WFU0enIxNVozd25jRlhHUVVkeHozSzJtOWNkRURjRnB4djlkbzliT29KWXE0VDQ0VHNvVk8?oc=5",
    "k": "small modular reactors microreactors expert insights energy environment",
    "id": "7d1672e8"
  },
  {
    "date": "2026-03-12",
    "title": "SMR 특별법안 통과, 차세대 원자력 산업 육성 및 지원 체계 확대",
    "summary": "소형모듈원자로(SMR) 산업 육성을 위한 특별법안이 국회를 통과했다.",
    "summaryLong": "한국 국회에서 소형모듈원자로(SMR) 차세대 원자력 산업 육성과 다각적 지원 체계를 규정하는 특별법안이 통과되었다. 이 법안은 SMR 개발·건설·운영 전주기에 걸친 정부 지원책과 규제 완화, 민간 참여 활성화 방안을 담고 있다. 정부의 SMR 산업화 전략을 법적으로 뒷받침하고 국내 원자력 산업의 경쟁력 강화를 목표로 하는 중요한 정책 수단이 될 것으로 기대된다.",
    "cat": "정책",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "smr 특별법안 통과 차세대 원자력 산업 육성 및 다각적 지원 체계의 주요 내용",
    "id": "5cda0acd"
  },
  {
    "date": "2026-03-11",
    "title": "산단 SMR 추진, 원전 PPA 허용·민간 참여가 관건",
    "summary": "산업단지 내 소형모듈원자로(SMR) 사업 활성화를 위해 원전 전력구매계약(PPA) 허용 및 민간 부문 참여가 핵심 과제로 지적된다.",
    "summaryLong": "한국 산업단지 내 소형모듈원자로(SMR) 도입 확대를 위해서는 원전 전력구매계약(PPA) 체계 도입과 민간 기업의 참여 활성화가 필수적이라는 분석이 제기되었다. 현행 규제 체계에서 PPA 허용 여부가 SMR 사업 수익성과 민간 투자 유입을 좌우하는 관건으로 작용하고 있다. 이는 정부의 SMR 산업화 정책과 규제 개선의 필요성을 강조하는 의견으로, 앞으로의 정책 결정이 산업 활성화의 주요 변수가 될 것으로 예상된다.",
    "cat": "계약",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "산단 smr 띄우려면 원전 ppa 허용 민간 참여가 관건",
    "id": "125e807d"
  },
  {
    "date": "2026-03-09",
    "title": "Rolls-Royce, 항공용 지속가능연료 생산에 소형원자로 활용 검토",
    "summary": "Rolls-Royce가 항공용 지속가능연료(e-SAF) 생산을 위해 소형모듈원자로 활용 가능성을 파트너들과 검토 중이다.",
    "summaryLong": "Rolls-Royce는 항공업계의 탈탄소화를 위해 소형모듈원자로(SMR) 기술을 활용하여 항공용 지속가능연료(e-SAF)를 생산하는 방안을 검토하고 있다. 수소 생산 등 연료 제조 공정에 필요한 고온 열과 전력을 SMR로부터 공급받는 개념이다. 항공사, 에너지 기업, 정부 기관 등 다양한 파트너들과 협력하여 기술적·경제적 타당성을 평가 중에 있다. 이는 항공산업의 국제 탄소중립 목표 달성에 기여할 수 있는 혁신적 솔루션으로 평가되며, 성공 시 SMR의 새로운 응용 분야 개척을 의미한다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "rolls royce partners to explore small nuclear reactors for e saf",
    "id": "65ec4932"
  },
  {
    "date": "2026-03-06",
    "title": "퍼듀대학교(Purdue University)·듀크에너지(Duke Energy), 소형모듈원자로(SMR) 핵학 연구 프로그램 협력",
    "summary": "퍼듀대학교(Purdue University)가 듀크에너지(Duke Energy)와 협력하여 소형모듈원자로(SMR) 관련 핵학 연구 프로그램을 운영한다.",
    "summaryLong": "퍼듀대학교(Purdue University)와 듀크에너지(Duke Energy)가 소형모듈원자로(SMR) 분야의 인재 양성과 기술 개발을 목표로 핵학 연구 프로그램을 공동으로 추진하고 있다. 이 협력은 차세대 원자력 기술 인력 확보와 소형모듈원자로 산업 육성의 중요성을 반영한다. 두 기관은 학술 연구와 실무 경험을 결합하여 소형모듈원자로 분야의 경쟁력을 강화할 계획이다. 이러한 산학협력은 북미 지역의 소형모듈원자로 상용화 추진에 기여할 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "Duke Energy",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiYkFVX3lxTE92Q3BQcVdtdjEybzJNUC1qRUV6YnBMUGdqekpGTzc0dWlnRnpDcnJoSjAzbkIwTHRiUUVNaWJmTFNwcXg1Tmxzc01SZUVNdjNJbnA1NXVKSXNqMWkxV2YyRDNR?oc=5",
    "k": "study with duke energy small modular reactor smr nuclear studies",
    "id": "b76c8c23"
  },
  {
    "date": "2026-03-05",
    "title": "테라파워(TerraPower), 상용 소형모듈원자로(SMR) 발전소 건설 미국 승인 획득",
    "summary": "테라파워(TerraPower)가 상용 소형모듈원자로(SMR) 발전소 건설에 대한 미국 정부 승인을 획득했다.",
    "summaryLong": "테라파워(TerraPower)가 소형모듈원자로(SMR) 상용 발전소 건설 프로젝트에 대해 미국 정부의 공식 승인을 받았다. 이는 테라파워의 소형모듈원자로 기술이 규제 기준을 충족하고 상용화 단계로 진입했음을 의미한다. 테라파워는 혁신적인 냉각 기술과 설계를 통해 소형모듈원자로의 경제성과 안전성을 강화하고 있다. 이번 승인은 미국 내 소형모듈원자로 상용화 추진에 중요한 이정표가 될 것으로 평가된다.",
    "cat": "기술",
    "type": "SFR",
    "dev": "TerraPower",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiaEFVX3lxTE1wcG9wUG9kSDNNUV9fYVQ0cXFkQTdsa2FNd3BKU3JJTmJLcDZCYjBxRWVBeFVqU2cyY1dEb0xvZWxmTG5ad1hfbDRVbXNuU3I4SWhrb0dBbnhpSFBNU1BkYWdEcXhXQk1u?oc=5",
    "k": "nuclear startup terrapower wins us approval to build commercial smr plant",
    "id": "668b07e8"
  },
  {
    "date": "2026-03-05",
    "title": "아멘텀(Amentum)·롤스로이스 에스엠알(Rolls-Royce SMR), 영국·체코 소형모듈원자로(SMR) 사업 협력",
    "summary": "아멘텀(Amentum)이 롤스로이스 에스엠알(Rolls-Royce SMR)과 파트너십을 체결하여 영국과 체코에서 소형모듈원자로(SMR) 프로젝트를 진행한다.",
    "summaryLong": "아멘텀(Amentum)과 롤스로이스 에스엠알(Rolls-Royce SMR)이 소형모듈원자로(SMR) 개발·배치를 위한 전략적 파트너십을 구축했다. 두 기관은 영국과 체코(Czech Republic)를 중심으로 소형모듈원자로 프로젝트를 공동 추진할 계획이다. 아멘텀의 엔지니어링·건설 역량과 롤스로이스의 원자로 기술이 결합되어 유럽 지역의 소형모듈원자로 상용화를 가속화할 것으로 예상된다. 이 협력은 유럽의 에너지 안보와 탄소중립 목표 달성에 기여할 것이다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "Rolls-Royce SMR, Amentum",
    "region": "UK",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMif0FVX3lxTFBlM1NncEdHR2lEOWc0NU5IeC1fMUNvX0xLakNiZUVKczFLczZoaVZ1MVBDbXE2b1JRRWV6Z2VqNFNwTnFsazgzTWRGWmVPRV9lVHpFT0wtbnFnMVp4aGlYaEJhQzMxYUUyV0RmZHRoY2swOFBKck9DaFRab1Y5dWs?oc=5",
    "k": "amentum teams with rolls royce smr to advance small modular reactor projects in the uk and czech republic",
    "id": "d5d58594"
  },
  {
    "date": "2026-03-04",
    "title": "아이-에스엠알(i-SMR), 표준설계인가(SDA) 추진",
    "summary": "아이-에스엠알(i-SMR)이 표준설계인가(SDA) 취득을 추진 중이다.",
    "summaryLong": "아이-에스엠알(i-SMR)이 한국 원자력안전위원회로부터 표준설계인가(SDA, Standard Design Approval)를 취득하기 위한 절차를 진행하고 있다. 표준설계인가는 원자로 설계의 안전성과 기술 타당성을 미리 검증하는 제도로, 향후 발전소 건설 허가 절차를 단축할 수 있다. 아이-에스엠알의 표준설계인가 추진은 국산 소형모듈원자로의 기술적 신뢰도 확보와 국제 경쟁력 강화를 의미한다. 이는 한국의 소형모듈원자로 산업 자립화 추진의 중요한 성과가 될 것이다.",
    "cat": "인허가",
    "type": "PWR",
    "dev": "i-SMR",
    "region": "CA",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMijgFBVV95cUxNbV9jMko3WkpMYmY1MlhORG51N3EzXzAwQU9xbGx5WHZJUWFOV1lVdVlxWFZlcHFHcngtTXhKMlh1V2FiRGRRcXNiVmFWMGQ3T0syaG4xYmdoTDA5RHVXRzBESzRPNEJTR3M4R0tHMmhhYW14VW9DZjIydjlmTFE0UW5qVzVMNUNiX3c2OUlB?oc=5",
    "k": "standard design approval sought for i smr",
    "id": "99f3ab8a"
  },
  {
    "date": "2026-03-04",
    "title": "인도네시아·미국·일본, 소형모듈원자로(SMR) 배치 국제 컨퍼런스 개최",
    "summary": "인도네시아, 미국, 일본이 소형모듈원자로(SMR) 배치에 관한 국제 컨퍼런스를 공동 개최했다.",
    "summaryLong": "인도네시아, 미국, 일본이 소형모듈원자로(SMR)의 실제 배치와 활용 방안을 논의하는 국제 컨퍼런스를 공동으로 개최했다. 이 행사는 아시아·태평양 지역의 소형모듈원자로 도입 활성화와 국제 협력 강화를 목표로 마련되었다. 세 국가의 정부 관계자, 산업계, 전문가들이 참여하여 기술 표준, 규제 체계, 사업화 전략 등을 공유했다. 이러한 국제 협력은 지역 에너지 안보와 탄소감축 목표 달성을 위한 소형모듈원자로의 역할을 강조하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "indonesia united states and japan host conference on nuclear small modular reactor deployment",
    "id": "3671042d"
  },
  {
    "date": "2026-02",
    "title": "홀텍(Holtec), 팰리세이즈 에스엠알-300(Pioneer) 건설허가 신청 미국 원자력규제위원회(NRC) 접수",
    "summary": "미시간 팰리세이즈(Palisades) 부지에 쌍둥이 에스엠알-300(SMR-300) 건설허가 신청을 미국 원자력규제위원회(NRC)에 접수하였으며, 제한작업승인(LWA) 포함 안전·환경평가를 2027년 상반기 완료 목표로 추진 중이다.",
    "cat": "인허가",
    "type": "PWR",
    "dev": "Holtec",
    "region": "US",
    "source": "ANS",
    "url": "https://www.ans.org/news/article-7673/holtec-submits-partial-construction-permit-application-for-smrs-at-palisades/",
    "summaryLong": "홀텍(Holtec)이 미시간 팰리세이즈(Palisades) 발전소 부지에 에스엠알-300(SMR-300) 쌍둥이 원자로의 건설허가 신청을 미국 원자력규제위원회(NRC)에 제출했다. 신청서는 제한작업승인(LWA, Limited Work Authorization)을 포함하고 있어 사전 허가 단계부터 일부 공사가 가능하도록 요청했다. 홀텍은 안전성 검토와 환경영향평가를 2027년 상반기 내에 완료하는 것을 목표로 진행하고 있다. 팰리세이즈 부지의 폐지된 기존 원자력 발전소 인프라를 활용한 소형모듈원자로 건설은 미국의 에너지 전환 전략의 중요한 사례가 될 것이다.",
    "id": "de392ef7"
  },
  {
    "date": "2026-02-27",
    "title": "소형모듈원자로, 큰 꿈: 캐나다 온타리오의 핵 에너지 전략",
    "summary": "캐나다 온타리오(Ontario) 지역이 소형모듈원자로(SMR)를 핵심 에너지 정책으로 추진하고 있다.",
    "summaryLong": "캐나다 온타리오(Ontario) 지역이 탄소중립 에너지 체계 구축을 위해 소형모듈원자로(SMR)를 핵심 정책으로 채택했다. 온타리오는 지역 전력 공급 안정화와 산업용 열 공급, 수소 생산 등 다양한 용도로 소형모듈원자로의 활용을 추진하고 있다. 기존 원자력 발전소 부지와 새로운 입지의 활용을 통해 단계적으로 소형모듈원자로를 배치할 계획이다. 이러한 정책은 캐나다의 2050년 탄소중립 달성 목표와 지역 경제 활성화를 동시에 추구하는 전략이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "CA",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMilAFBVV95cUxQUWtCc3J2NWdpelRHSmpIaGZhZHI2eDJmT25FdDJqTHh2a05POUlGNDJaUGhXYWpkVV9qX3JtVmZNSXRpVVFyZUhPLWprbldNUjN1T0hWbW0wUE5KRmFEenZ5QmIxTTQwQ2hhbDFISXVlaHVNMExUdHJaRUdORGY2R2ItX19PSkk1aWxpeWxka0l4ZWtT?oc=5",
    "k": "small modular reactors big dreams ontario s nuclear pitch",
    "id": "525dd889"
  },
  {
    "date": "2026-02-27",
    "title": "2026년 국가별 소형모듈원자로(SMR) 시설 현황 통계",
    "summary": "스타티스타(Statista)가 2026년 기준 국가별 소형모듈원자로(SMR) 시설 현황을 데이터로 제시한다.",
    "summaryLong": "데이터 분석 플랫폼 스타티스타(Statista)가 2026년의 국가별 소형모듈원자로(SMR) 시설 통계를 발표했다. 이 통계는 각국의 소형모듈원자로 개발 현황, 계획 중인 프로젝트, 운영 중인 시설 등을 종합적으로 제시한다. 데이터는 글로벌 소형모듈원자로 시장의 지역별 분포와 발전 추이를 파악하는 데 중요한 지표가 된다. 이러한 통계 정보는 정부, 산업계, 투자자들의 소형모듈원자로 정책 수립과 사업 의사결정을 지원한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiekFVX3lxTE9MLU5MeGpfcG9SRGVOdzdxSENCT3R1dTk5clVqUV9hT1RmM21MZnJKMnY3UUNsR2JyOXNkd2lUbHAyYWdXWnlLQ29sZ1h1Zm81T195TExrZ25ZQTFhUDZ4blRYd1YzQ1N3b1hCZHFkcEpIbUlPUXBKLThB?oc=5",
    "k": "smr nuclear facilities by country 2026",
    "id": "5bd7dc39"
  },
  {
    "date": "2026-02-27",
    "title": "혁신형 SMR, 원안위에 표준설계인가 신청…'본격 인허가 단계'",
    "summary": "혁신형 소형모듈원자로(SMR)가 원자력안전위원회(원안위)에 표준설계인가를 신청하며 공식 인허가 절차에 진입했다.",
    "summaryLong": "국내 혁신형 소형모듈원자로(SMR) 개발 사업이 원자력안전위원회(원안위)에 표준설계인가(SDI, Standard Design Approval)를 신청했다. 이는 기술 개발 단계를 거친 후 규제 기관과의 공식적인 설계 인증 절차에 들어가는 것으로, 상용화를 향한 본격적인 인허가 단계에 진입했음을 의미한다. 표준설계인가 취득은 향후 건설 허가와 운영 허가로 이어지는 관문이 된다.",
    "cat": "인허가",
    "type": "PWR",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMibEFVX3lxTE9peHZSY21EQm9JUjloNzUwb2xQNGN2bUVONEFaVUxFakN4QjY3cU01SkljTzl6QXFZUGV6V3pBU0FIQ1FScTUycW93SDFVVTdOT0l0b3RKYzdId0xjYzlZMkdJeUtzaVdkVlJycw?oc=5",
    "k": "혁신형 smr 원안위에 표준설계인가 신청 본격적 인허가 단계 진입",
    "id": "37572cc0"
  },
  {
    "date": "2026-02-25",
    "title": "기장군, 혁신형 SMR 유치 본격화…TF 출범·주민 설명회",
    "summary": "기장군이 혁신형 SMR 유치를 위해 전담반(TF)을 출범시키고 주민 설명회를 병행하며 본격적인 유치 활동을 추진 중이다.",
    "summaryLong": "부산 기장군이 혁신형 소형모듈원자로(SMR) 유치를 위한 전담팀(TF)을 공식 출범시키고, 지역 주민들을 대상으로 설명회를 개최하는 등 구체적인 유치 활동을 본격화했다. 이는 지자체 차원에서 SMR 프로젝트의 실현 가능성을 검토하고 주민 동의 기반을 마련하려는 단계로 볼 수 있다. 국내 원전 부지에서의 첫 혁신형 SMR 입지 가능성이 현실화되고 있는 상황이다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "기장군 혁신형 smr 유치 본격화 tf 출범 주민 설명회 병행",
    "id": "3c5ae6d8"
  },
  {
    "date": "2026-02-22",
    "title": "퍼듀 대학교(Purdue University)의 소형모듈원자로(SMR) 핵 연구 패널 토론",
    "summary": "퍼듀 대학교에서 소형모듈원자로 핵 연구에 관한 패널 토론을 개최했다.",
    "summaryLong": "퍼듀 대학교는 소형모듈원자로의 핵 관련 연구를 주제로 패널 토론을 개최했다. 이 토론은 소형모듈원자로 기술 개발 및 연구 현황을 논의하는 자리로 마련되었다. 학계 전문가들이 참여하여 소형모듈원자로의 기술적 과제와 발전 전망을 공유했다. 이는 북미 지역의 소형모듈원자로 연구 생태계 활성화에 기여하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMieEFVX3lxTFBSbnF4SHUwYTB5cGZiV1FRYmtFNHIwRGRLR3pvQUp3UEZWcVVMODhKZjJFZjlObjQtbkdTUURoWjFLVUgxdmplOE1UZVFfNjktY3VQVU0tYlF6UlFBRkh0MFhSOG1IOWdYNUQ3ZmNjMnB6aWZEU09tMg?oc=5",
    "k": "panel discussion small modular reactor smr nuclear studies",
    "id": "ec33785f"
  },
  {
    "date": "2026-02-17",
    "title": "SMR과 BWXT의 소형모듈원자로 관련 주식 비교 분석",
    "summary": "투자 관점에서 SMR과 BWXT의 소형모듈원자로 관련 주식을 비교 분석한 기사다.",
    "summaryLong": "금융 매체가 소형모듈원자로 산업의 핵심 기업인 SMR과 BWXT(캐나다 원자력기술회사, BWXT)의 주식 투자 가치를 비교 분석했다. 두 회사의 기술 경쟁력, 프로젝트 포트폴리오, 시장 전망 등을 종합적으로 평가했다. 소형모듈원자로 시장의 성장 잠재력이 투자 결정에 미치는 영향을 분석했다. 투자자들에게 소형모듈원자로 산업 진출 기업의 선택 기준을 제시하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "SMR, BWXT",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMie0FVX3lxTE9oZmNtUnpRNGJWbHNCNTBHTEdLbDk2U1lEX0hKLVp5ZHJ6VE5hQ0pKd3NHb0dOM0RPeWNhLU9jU3puanlpTHd2U0F0Wlk5bHVoT051OTRsTnlfT1FBbU9vWEhaWWcxOUxIVjhnUG5OMExyWGVBNGswSlg3aw?oc=5",
    "k": "smr vs bwxt which small modular reactor stock is a buy right now",
    "id": "b64a5d19"
  },
  {
    "date": "2026-02-13",
    "title": "루마니아(Romania) Nuclearelectrica, 폐탄광 부지 소형모듈원자로 프로젝트 승인",
    "summary": "루마니아 국영 전력회사 Nuclearelectrica의 주주들이 폐 석탄광 부지에 소형모듈원자로 건설 프로젝트를 승인했다.",
    "summaryLong": "루마니아의 국영 전력회사인 Nuclearelectrica가 주주 총회에서 폐 석탄광 부지를 활용한 소형모듈원자로 건설 프로젝트를 공식 승인했다. 이 프로젝트는 기존 석탄 에너지 기반시설을 재활용하여 에너지 전환을 추진하는 사례다. 유럽의 탈탄소화 목표 달성과 에너지 안보 강화에 기여할 것으로 기대된다. 동유럽 지역의 소형모듈원자로 도입 가속화를 시사하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "Nuclearelectrica",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "nuclearelectrica shareholders approve plans for small modular reactor project at former romania coal site",
    "id": "6970e949"
  },
  {
    "date": "2026-02-13",
    "title": "루마니아 소형모듈원자로 최종투자결정(FID) 승인",
    "summary": "루마니아의 소형모듈원자로(SMR) 프로젝트가 최종투자결정을 획득했다.",
    "summaryLong": "루마니아의 소형모듈원자로(SMR) 프로젝트가 최종투자결정(FID)을 확보했다. 이는 SMR 상용화를 위한 중요한 마일스톤으로, 프로젝트의 경제성 검토와 기술 검증이 완료되었음을 의미한다. 해당 프로젝트는 루마니아의 저탄소 전력 생산 목표 달성과 에너지 안보 강화에 기여할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMikwFBVV95cUxNX08xZEs2Ukh2aTZZcmYybE41VzNIaHVjWmQyMF92UEg4WHBGTFd4eGxDbzlyM1l5ZjI1WkZKbGF2RlB4Zk14dU4zT3NfaXVuSVBfV2FCeHlpd01MeGI3MHpvNGFfNm9Gd0NXdVZTazY1SDNUQXBqdjdySm9ibVhTMkpuZzJmaWMwRzNsNkpCbmlvLTQ?oc=5",
    "k": "final investment decision taken for romanian smr project",
    "id": "ee7aca70"
  },
  {
    "date": "2026-02-13",
    "title": "'SMR 특별법' 국회 통과…R&D·실증 가속",
    "summary": "한국 국회가 소형모듈원자로 특별법을 통과시켜 SMR 연구개발·실증을 가속화하기로 했다.",
    "summaryLong": "대한민국 정책브리핑은 한국 국회의 'SMR 특별법' 통과를 보도했다. 특별법은 소형모듈원자로(SMR) 연구개발, 실증, 인프라 구축을 위한 정부 지원과 규제 개선 사항을 포함하는 국책 프로그램이다. 이 법안은 한국의 SMR 기술 자립과 국제 경쟁력 강화, 탄소 중립 에너지 정책 이행의 법적 기반이 되어 2030년대 상용화를 가속할 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMibEFVX3lxTFBSdVY1bjhFTnhUSlQ4YVR4MnFWZExXdThqelNkSXB4RU03X1o4QlBENXlRQXRGSGtyVVJFQ1VrN3BYclMyRk5ZbTVMXzNrR2hlVmhGWk9LR0xiWEpteDd6eWtDcExHOTJsWEFubg?oc=5",
    "k": "smr 특별법 국회 통과 소형모듈원자로 r d 실증 가속",
    "id": "a5a8b44b"
  },
  {
    "date": "2026-02-06",
    "title": "롤스로이스(Rolls-Royce) SMR, 요코가와(Yokogawa)와 제어시스템 공급 계약 체결",
    "summary": "롤스로이스 SMR이 요코가와로부터 제어시스템 공급 계약을 체결했다.",
    "summaryLong": "영국의 롤스로이스 SMR(Rolls-Royce SMR)이 일본의 요코가와 일렉트릭(Yokogawa Electric)과 제어시스템 공급 계약을 체결했다. 이 계약은 소형모듈원자로의 안전하고 효율적인 운영을 위한 첨단 자동화 및 제어 기술 도입을 의미한다. 요코가와는 산업 제어 및 계측 분야의 국제적 선도기업으로, 이번 계약은 소형모듈원자로 프로젝트의 기술 신뢰성을 높인다. 글로벌 공급망 협력을 통해 소형모듈원자로의 상용화 시기 단축에 기여할 것으로 전망된다.",
    "cat": "계약",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "rolls royce smr taps yokogawa to supply control systems for small modular reactor fleet",
    "id": "231c1c6c"
  },
  {
    "date": "2026-02-05",
    "title": "SMR, 원자력의 미래를 어떻게 바꿀까",
    "summary": "소형모듈원자로(SMR)의 원자력 산업에 대한 영향과 미래 가능성에 대한 분석 기사이다.",
    "summaryLong": "소형모듈원자로(SMR)가 기존 대형 원전과 달리 에너지 공급 체계, 산업용 열·전력 공급, 지역 분산형 에너지 모델 등 원자력 산업의 구조를 어떻게 변화시킬 수 있을지에 대한 종합적인 분석 기사이다. SMR의 기술적 특성(모듈화, 소규모 투자, 다목적 활용)과 산업 활용 전망을 다루면서 원자력 에너지의 미래상을 제시하고 있다. 원자력 산업의 새로운 성장 동력으로서 SMR의 역할과 가능성을 조명하는 내용이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMiZkFVX3lxTE9VSndjaGtRdzk1dl9FV3R5bWltN3huVFJOVXdwcXVMREhmRFdNaDM3cjFXY2dmdEZUTkFsTEl6eTVNVUx1ckJISW41ZW5MdnNzdTVqNGxQZ3NkbU1VU1gtZExMOVliZw?oc=5",
    "k": "집콕사이언스 smr 원자력의 미래를 어떻게 바꿀까",
    "id": "50684b68"
  },
  {
    "date": "2026-01-30",
    "title": "경제협력개발기구 원자력기관(OECD NEA) 소형모듈원자로 디지털 대시보드 3.1판 출시",
    "summary": "경제협력개발기구 원자력기관(OECD NEA)이 소형모듈원자로 데이터베이스의 최신판(3.1판)을 출시하고 지속적 업데이트 체계를 확대했다.",
    "summaryLong": "경제협력개발기구 원자력기관(OECD NEA)이 소형모듈원자로 정보를 집계·관리하는 디지털 대시보드의 3.1판을 출시했다. 이 플랫폼은 전 세계 소형모듈원자로 프로젝트의 현황, 기술 스펙, 개발 진척도 등의 데이터를 통합 관리한다. 지속적인 업데이트 체계 확대로 정보의 신뢰성과 최신성을 보장하고 있다. 소형모듈원자로 산업의 투명성 강화와 국제 정보 공유 활성화를 촉진하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "NEA",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "launch of edition 3 1 and new rolling update nea small modular reactor digital dashboard",
    "id": "5d9da12f"
  },
  {
    "date": "2026-01-23",
    "title": "소형모듈원자로 법제가 미국 각 주(State)의 원자력 정책을 재편",
    "summary": "소형모듈원자로 관련 법제 정비가 미국 각 주의 핵에너지 정책을 재구성하고 있다.",
    "summaryLong": "미국 연방정부와 각 주의 소형모듈원자로 관련 규제 및 지원 법제 정비가 주(State) 수준의 원자력 정책 기조 변화를 주도하고 있다. 여러 주들이 소형모듈원자로 도입 촉진을 위한 입법을 진행 중이며, 이는 기존의 대형 원전 중심 정책으로부터의 전환을 의미한다. 지역 에너지 수급 계획, 에너지 안보, 탈탄소화 목표 달성에 소형모듈원자로를 전략적으로 포함시키는 추세가 확산되고 있다. 이러한 법제 개혁이 미국 원자력 산업의 구조적 변화를 이끌고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "how small modular reactor laws are reshaping state nuclear energy policy",
    "id": "f22389ca"
  },
  {
    "date": "2026-01-22",
    "title": "소형모듈원자로(SMR) 관련 상장사 주식 비교 분석",
    "summary": "금융 매체가 소형모듈원자로 관련 상장사들의 주식 투자 가치를 비교 분석하고 있다.",
    "summaryLong": "금융 매체가 소형모듈원자로 산업 관련 여러 상장사들의 주식 투자 가치를 종합적으로 분석했다. 각 기업의 기술 경쟁력, 프로젝트 진척 현황, 시장 전망, 주가 흐름 등을 평가 대상으로 삼았다. 소형모듈원자로 시장의 성장성과 상용화 일정이 투자자의 포트폴리오 선택에 영향을 미치고 있다. 신흥 에너지 분야로서 소형모듈원자로 산업의 투자 기회와 위험 요소를 균형있게 제시하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiekFVX3lxTE5NeVVNSy01XzY2Z2UwNlRhMDB0NFNGdm50a0x6YUtfa3AzRWZ1bVpnM1Ffb2xuYXV1OWFOSWs1ektOLWIwOUJuMHVHUVE0VWRZTFd4cnFxcHpYUm5VM0VuenRIbFR2ckhqNHBJM0ZrVHBhcG95S1RrSzF3?oc=5",
    "k": "smr vs gev which small modular reactor stock is a better pick",
    "id": "03a10c64"
  },
  {
    "date": "2026-01-19",
    "title": "캐나다 레지나에 소형모듈원자로 시험센터 계획",
    "summary": "캐나다 레지나(Regina)에 소형모듈원자로 시험·평가 센터 건립이 계획되고 있다.",
    "summaryLong": "캐나다는 레지나(Regina) 지역에 소형모듈원자로(SMR) 시험 및 검증 센터 설립을 추진하고 있다. 이 센터는 SMR 프로토타입 시운전, 성능 평가, 안전 검증 등을 수행할 수 있는 기반 시설로 기능할 예정이다. 캐나다 정부와 지역 당국은 이를 통해 SMR 기술 개발 생태계를 확대하고 국내 산업 경쟁력을 강화하려 하고 있다. 이 프로젝트는 캐나다의 원자력 기술 리더십 구축과 향후 상용화 기반 마련의 중요한 마일스톤이 될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "CA",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiigFBVV95cUxOb2x3b0Zha0VYZGdwektVQ2syX2kwSUU1bVVGYW9YbTlFM3B2aTFnckp5M21MWEUyd2RNcHE2eGV5T0RhS090am95eUJfbEpzYXhsUTdkc2ZYX19WNGZybGEzeEhLMzV6NlNmOHpQbFFJZEQ3dmZZeWMwd2RVMHhtR084Z3JGV0VvcXc?oc=5",
    "k": "small modular nuclear reactor testing centre planned for regina",
    "id": "6715f7ce"
  },
  {
    "date": "2026-01-17",
    "title": "소형모듈원자로의 핵확산 위험성 분석",
    "summary": "국제 학술 기관이 소형모듈원자로 도입 시 발생 가능한 핵확산 위험을 분석했다.",
    "summaryLong": "국제 전문 학술 기관이 소형모듈원자로의 확대 도입에 따른 핵확산(Nuclear Proliferation) 위험성을 종합적으로 분석했다. 소형모듈원자로의 분산된 배치, 상대적으로 낮은 기술 진입 장벽, 증가된 핵물질 수송 빈도 등이 핵비확산 체계에 미칠 영향을 검토했다. 국제원자력기구(IAEA) 등 국제 규범의 적용 한계와 개선 과제를 도출했다. 소형모듈원자로 산업 성장 과정에서 핵안보와 국제 비확산 규범 강화의 균형이 필수적임을 강조하고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMilwFBVV95cUxPeE5DUzBrNlJXck9DTHllb0ZiNkhIZlJkcDE1T0xGdnRYZ01Bc2QtbTBUdGU5UmZ3Qnp3bU45TkRNZE5tUEpiZ202TDk3TlBmZlVkU0lKY0hjZ0pSOTZ6WHB4YnpMbnZqY0dvdGZrLXhzcFJScFRJalFCSEZRR28wMWhVdngzNy1NaFN6RWFjM1ZwUWNZZkdR?oc=5",
    "k": "proliferation risks associated with small modular reactors e international relations",
    "id": "e24c2008"
  },
  {
    "date": "2026-01-16",
    "title": "인도, 기후목표 달성을 위해 소형모듈원자로(SMR) 추진",
    "summary": "인도가 기후변화 목표 달성을 위해 소형모듈원자로(SMR) 도입을 추진 중이다.",
    "summaryLong": "인도는 탄소중립 목표를 달성하기 위한 에너지 다원화 전략의 일환으로 소형모듈원자로(SMR) 기술 도입을 검토하고 있다. 소형모듈원자로는 기존 대형 원자로에 비해 건설 기간이 짧고 초기 투자비가 낮아 개도국의 에너지 전환에 적합한 솔루션으로 평가되고 있다. 인도의 SMR 추진은 아시아 지역의 원자력 에너지 확대 추세를 반영하며, 국내 기술 개발과 국제 협력을 통해 구체화될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "JP",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMingFBVV95cUxQRk1mNU90Mzl4MEMzR3pMS3pLRnFVYURtQVl4VlRTcUx2ZS0tc3hJLVlDNXdYbGhQc3M3aDhHeGNzbXF4YTVMY2paaHh3c1NRc0tXQThWcXlKMkE5Y3lTaGpCc2ozUXEtallXMC1hbFB3QWxCMUpuR09YQUdYTE1UZ19xaHkwUFdBNXpCRkVUOWRZVmk0RENDREZuZFFqQQ?oc=5",
    "k": "india turns to small modular nuclear reactors to meet climate targets",
    "id": "7ae6aa16"
  },
  {
    "date": "2026-01-15",
    "title": "몽골, 신수도 건설에 소형모듈원자로 도입 검토",
    "summary": "몽골이 신수도 건설 사업에 소형모듈원자로 도입을 검토하고 있다.",
    "summaryLong": "몽골 정부가 신수도 건설 프로젝트의 에너지 공급 방안으로 소형모듈원자로(SMR) 도입 가능성을 검토 중이다. 신수도는 대규모 전력 수요가 예상되는 만큼, SMR의 모듈식 확장성과 소규모 전력망 적응성이 주목받고 있다. 이는 몽골과 같이 광활한 지역에 분산된 전력 공급이 필요한 국가에서 SMR이 차세대 에너지 솔루션으로 인식되고 있음을 보여준다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "mongolia could build small modular reactor for new capital city says report",
    "id": "955aa614"
  },
  {
    "date": "2026-01-10",
    "title": "산업단지에도 SMR 배치 길 열린다",
    "summary": "소형모듈원자로(SMR)를 산업단지에 배치할 수 있는 규제 또는 정책 환경이 조성되고 있다.",
    "summaryLong": "산업단지에 소형모듈원자로(SMR)를 직접 설치할 수 있는 길이 열리고 있다. 이는 기존의 대규모 중앙집중식 원전과 달리 SMR의 소형·모듈화 특성을 활용하여 산업 수요처 인근에 배치하는 분산 에너지 공급 모델을 가능하게 한다. 산업단지 내 SMR 배치는 고온 공정열이 필요한 화학·제철·수소생산 등 산업 부문의 탄소중립 전환을 촉진할 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.?oc=5",
    "k": "산업단지에도 smr 배치 길 열린다",
    "id": "eff30083"
  },
  {
    "date": "2026-01-08",
    "title": "중국 소형모듈원자로 비핵 증기 시운전 완료",
    "summary": "중국이 개발한 소형모듈원자로가 비핵 증기 시운전(non-nuclear steam test)을 성공적으로 완료했다.",
    "summaryLong": "중국의 소형모듈원자로(SMR) 개발 프로젝트가 비핵 증기 시운전(non-nuclear steam start-up test)을 완료했다. 이 시험은 원자로의 열수력 계통과 제어 체계가 설계대로 작동하는지 확인하는 중요한 선행 테스트로, 이후 핵 연료를 장입하여 임계 달성으로 진행되는 단계이다. 중국은 소형모듈원자로 기술 개발과 상용화에 전략적으로 투자하고 있으며, 이 성과는 중국 SMR 기술의 기술 완성도 향상과 국제 시장 진출 가능성을 보여준다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMingFBVV95cUxOdXNYczVIRDZLT29mUG5zSkFrc2k4Xy0xUWNWamZTNm15OFF3U2VOcjRObkxNakxITGdJUU5wMmozbkhBRTNOTHFheml4cl9VTFVhQWZyeDFHMHdjNHhpdHVLS0NLVkVJcDFmMEtlWnNQblZPdUxiaWtnQlRoVzFrbFE4ckZfU2FSTGUwTnVHdExLS0NmNFBrVV9qR1I1dw?oc=5",
    "k": "chinese smr completes non nuclear steam start up test",
    "id": "66f6fc80"
  },
  {
    "date": "2026-01-06",
    "title": "미국과 카자흐스탄 소형모듈원자로(SMR) 배치 촉진 이니셔티브 출범",
    "summary": "미국과 카자흐스탄이 소형모듈원자로 배치를 가속화하기 위한 협력 이니셔티브를 공동으로 시작했다.",
    "summaryLong": "미국과 카자흐스탄(Kazakhstan)이 소형모듈원자로(SMR) 배치 촉진을 위한 공동 이니셔티브를 발표했다. 이는 미국 정부와 국제 에너지 기구들이 소형모듈원자로 기술의 글로벌 확산과 신흥 시장 진출을 전략적으로 추진하는 노력의 일환으로, 카자흐스탄의 에너지 수요 및 우라늄 생산 기지로서의 위상을 고려한 협력 구도다. 양국의 협력은 소형모듈원자로 인프라 구축, 규제 표준 조화, 기술 이전 및 인력 양성 등 다층적 협력을 포함할 것으로 예상되며, 중앙아시아 지역의 에너지 안보와 기후 변화 대응의 새로운 모델을 제시하는 의미를 갖는다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "u s and kazakhstan launch initiatives to facilitate smr deployment american nuclear society ans",
    "id": "fc122766"
  },
  {
    "date": "2025-12",
    "title": "미국 원자력규제위원회(NRC), 테라파워 나트륨(Natrium) 안전심사 조기 완료",
    "summary": "테라파워(TerraPower)의 나트륨(Natrium) 소형모듈원자로가 미국 원자력규제위원회(NRC) 안전심사를 일정보다 앞서 완료하였으며, 2026년 3월 건설허가(CP) 발급으로 이어질 예정이다.",
    "cat": "인허가",
    "type": "SFR",
    "dev": "TerraPower",
    "region": "US",
    "source": "ANS",
    "url": "https://www.ans.org/news/article-7818/nrc-approves-terrapower-construction-permit/",
    "summaryLong": "미국 원자력규제위원회(NRC)가 테라파워(TerraPower)의 나트륨(Natrium) 소형모듈원자로에 대한 안전심사를 예정일보다 앞서 완료하여 예산 절감 효과를 거두었다. 나트륨은 액체 금속 냉각 기술을 적용한 차세대 소형모듈원자로로, 안전심사 조기 완료는 미국의 SMR 상용화 추진 의지를 반영한다. 2026년 3월 건설허가(CP) 발급이 예정되어 있어 실제 건설 단계로의 진입이 임박한 상황이다. 이는 테라파워와 미국 에너지부(DOE)가 추진 중인 나트륨 시범사업의 경제성과 기술 신뢰도를 높이는 계기가 될 것으로 기대된다.",
    "id": "ca0f28bd"
  },
  {
    "date": "2025",
    "title": "롤스로이스(Rolls-Royce) SMR, 영국 그레이트브리티시에너지-뉴클리어(GBE-N) 우선기술 선정",
    "summary": "롤스로이스(Rolls-Royce) 소형모듈원자로가 영국 그레이트브리티시에너지-뉴클리어(GBE-N)의 우선협상 기술로 선정되어 웰파(Wylfa) 부지에 최대 3기 배치될 예정이며, 2026년 4월 부지별 계약이 체결될 것으로 예상된다.",
    "cat": "계약",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "WNN",
    "url": "https://www.world-nuclear-news.org/articles/rolls-royce-smr-progresses-to-final-step-of-uk-ass",
    "summaryLong": "영국 정부의 새로운 국영 원자력기업 그레이트브리티시에너지-뉴클리어(GBE-N)가 롤스로이스(Rolls-Royce) 소형모듈원자로를 우선 기술로 선정하였다. 웨일스 북부의 웰파(Wylfa) 부지에 최대 3기의 롤스로이스 SMR을 배치하는 방안이 검토되고 있으며, 2026년 4월 부지별 계약 체결이 예상된다. 이는 영국의 에너지 안보 강화 및 넷제로(Net Zero) 목표 달성을 위한 핵심 전략이며, 롤스로이스 SMR의 상용화 경로를 크게 앞당길 것으로 예상된다.",
    "id": "92c842f2"
  },
  {
    "date": "2025",
    "title": "롤스로이스(Rolls-Royce) SMR, 영국 포괄설계평가(GDA) 최종단계 진입",
    "summary": "롤스로이스(Rolls-Royce) 소형모듈원자로가 영국 포괄설계평가(GDA) 최종단계(Step 3)에 진입하였으며, 2026년 8월 완료가 예정되어 있다.",
    "cat": "인허가",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "Rolls-Royce SMR",
    "url": "https://www.rolls-royce-smr.com/our-progress",
    "summaryLong": "롤스로이스(Rolls-Royce) 소형모듈원자로가 영국 원자력규제기관(ONR, ESA)의 포괄설계평가(GDA) 최종단계(Step 3)에 도달하여 현재 SMR 중 가장 앞선 인허가 진행 상황을 보이고 있다. 전체 심사 기간이 약 53개월로 예상되며 2026년 8월 완료 예정이다. GDA 최종단계 통과는 설계의 기술적 안전성과 경제성 검증을 의미하며, 이후 구체적인 부지별 허가(Site Licence) 취득으로 이어질 것으로 예상된다. 롤스로이스의 GDA 완료 임박은 영국 내 SMR 프로젝트의 실질적 추진을 가능하게 할 주요 이정표가 될 것이다.",
    "id": "a135d1ef"
  },
  {
    "date": "2025-12-27",
    "title": "카자흐스탄 전력망에 소형모듈원자로 적용 검토",
    "summary": "카자흐스탄의 지리적·기술적 전력망 특성을 분석한 결과 소형모듈원자로(SMR)가 최적의 에너지 솔루션으로 평가되었다.",
    "summaryLong": "카자흐스탄의 전력 인프라 특성에 관한 분석 연구에서 소형모듈원자로(SMR)가 국가 전력망 개선을 위한 최적 기술로 평가되었다. 광대한 영토에 분산된 전력 수요와 기존 인프라의 한계를 고려할 때, SMR의 모듈식 설계와 소규모 초기 투자 특성이 카자흐스탄의 에너지 전환에 적합한 것으로 분석되었다. 이러한 평가는 중앙아시아 지역에서 SMR 도입의 타당성을 보여주며, 향후 카자흐스탄의 원자력 정책 수립에 참고될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "small modular reactors are the perfect fit for kazakhstan s power grid crude oil prices today",
    "id": "0086e9f4"
  },
  {
    "date": "2025-12-16",
    "title": "폴란드 SGE-삼성건설, 소형모듈원자로 양해각서(MOU) 체결",
    "summary": "폴란드의 SGE(Synthos Green Energy)와 한국의 삼성건설이 소형모듈원자로의 개발·배치에 관한 양해각서(MOU)를 체결하였다.",
    "summaryLong": "폴란드의 신재생에너지 기업 SGE(Synthos Green Energy)와 한국의 삼성건설이 소형모듈원자로(SMR) 개발 및 배치에 관한 양해각서(MOU)를 체결하였다. 이는 폴란드의 에너지 안보 강화 및 탄소중립 목표 달성을 위한 전략적 협력이며, 한국 건설사의 국제 원자력 프로젝트 수주 확대를 의미한다. 양사는 SMR 기술 개발, 경제성 평가, 부지 선정 등 사업화 전 단계를 협력하게 되며, 향후 폴란드 내 SMR 프로젝트의 실현 가능성을 높일 것으로 기대된다.",
    "cat": "계약",
    "type": "General",
    "dev": "삼성건설",
    "region": "KR",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "poland s sge signs small modular reactor mou with south korea s samsung c t",
    "id": "9c386d62"
  },
  {
    "date": "2025-12-11",
    "title": "지이이(GEH) BWRX-300, 영국 포괄설계평가(GDA) 2단계 통과",
    "summary": "지이이(GEH, GE Hitachi)의 BWRX-300 소형모듈원자로가 영국 포괄설계평가(GDA) 2단계를 통과하였다.",
    "summaryLong": "미국-일본 합작사 지이이(GEH, GE Hitachi)가 개발한 BWRX-300 소형모듈원자로가 영국의 포괄설계평가(GDA) 2단계를 성공적으로 통과하였다. BWRX-300은 자연순환 냉각 등 혁신적 안전 기술을 적용한 경수로형 SMR로, GDA 2단계 통과는 기본 설계의 기술적 타당성이 인정되었음을 의미한다. 향후 최종단계(Step 3)를 거쳐 2027년경 설계 승인 획득이 예상되며, 이는 지이이의 영국 내 SMR 시장 진출을 가속화할 것으로 기대된다. 롤스로이스와 함께 진행 중인 경쟁적 인허가는 영국의 SMR 상용화 및 제조 산업 육성에 긍정적 영향을 미칠 것으로 전망된다.",
    "cat": "인허가",
    "type": "BWR",
    "dev": "GEH",
    "region": "UK",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "bwrx 300 small modular reactor passes step 2 of uk s generic design assessment",
    "id": "0151cfd3"
  },
  {
    "date": "2025-12-11",
    "title": "중국, 2026년 첫 소형모듈원자로(SMR) 상용 운영 계획",
    "summary": "중국이 2026년 첫 소형모듈원자로의 상용 운영을 목표로 추진 중이다.",
    "summaryLong": "중국은 소형모듈원자로 개발 전략의 일환으로 2026년 첫 상용 운영 달성을 목표하고 있다. 이는 중국의 에너지 정책 및 기술 개발 로드맵의 중요한 이정표로 평가된다. 소형모듈원자로는 소규모 전력 공급과 산업용 열 공급에 활용될 수 있어 중국의 저탄소 에너지 전환에 기여할 것으로 예상된다. 성공 시 아시아 지역 원자력 산업에도 영향을 미칠 가능성이 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "JP",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "china to launch first small modular reactor in 2026 crude oil prices today",
    "id": "954cc7f1"
  },
  {
    "date": "2025-12-10",
    "title": "인도 마하라슈트라·안드라프라데시주 소형모듈원자로 구성 제안",
    "summary": "인도가 마하라슈트라(Maharashtra)와 안드라프라데시(Andhra Pradesh) 두 주에 소형모듈원자로(SMR)를 구성하기로 제안했다.",
    "summaryLong": "인도 정부가 마하라슈트라(Maharashtra)와 안드라프라데시(Andhra Pradesh) 주에 소형모듈원자로(SMR) 설치를 추진하기로 제안했다. 이는 인도의 첫 번째 SMR 배치 사업으로, 지역 전력 공급 및 산업용 열 응용을 위한 것으로 보인다. 이 결정은 인도의 재생에너지 다변화 및 에너지 안보 전략의 일환이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "JP",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiowFBVV95cUxOZ2pBZXZrcmpoQVZfdUpTeGJZNVFZVVNZMDcwb2wwV1FBUlBPakR6NzYtZ0o1NzlLcUZjVEhXRWVwM0VWeVlual9XcUg0RUIyaDlNbFpJUWdnTlBJenZzeWx3NlV0aGxqOXVyOUYyUVA5UGtGQm1GWVpvR2diTmZIY0NTUllOWlV6U2RYUlFGVUdSMnhSb1dGYWJ6d2FpSzZrNERj?oc=5",
    "k": "maharashtra and andhra pradesh proposed for first indian smrs",
    "id": "1a87d946"
  },
  {
    "date": "2025-12-04",
    "title": "테네시밸리청(TVA), 미국 첫 소형모듈원자로 건설 추진",
    "summary": "미국 테네시밸리청(TVA)이 테네시주에서 미국 내 첫 상용 소형모듈원자로 개발을 추진 중이다.",
    "summaryLong": "테네시밸리청(TVA)은 클린치 강(Clinch River) 부지에서 미국 첫 상용 소형모듈원자로 건설 프로젝트를 진행하고 있다. 이 프로젝트는 미국 에너지부(DOE)의 지원 대상으로 선정되어 연방 정부 자금을 확보했다. 완성될 경우 미국의 분산형 저탄소 전력 공급 체계 구축에 중요한 역할을 할 것으로 기대된다. 프로젝트의 성공은 향후 미국 내 소형모듈원자로 상용화의 모델이 될 수 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "TVA",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "tva to develop america s first small modular reactor in tennessee",
    "id": "58985587"
  },
  {
    "date": "2025-12-04",
    "title": "듀크 에너지(Duke Energy), 미국 소형모듈원자로 배치 확대 지원",
    "summary": "듀크 에너지가 미국 에너지부(DOE)의 신규 핵 투자 정책을 지지하며 소형모듈원자로 배치 확대에 협력하겠다고 밝혔다.",
    "summaryLong": "주요 전력 회사인 듀크 에너지는 미국 에너지부의 신규 핵 투자 정책을 지지하며 소형모듈원자로 배치 확대에 참여할 의사를 표명했다. 이는 에너지 회사들의 탄소 중립 에너지원으로서 소형모듈원자로에 대한 관심 증가를 보여준다. 듀크 에너지의 지원은 소형모듈원자로 상용화의 시장 확대에 긍정적 신호로 작용할 것으로 예상된다. 향후 다른 에너지 기업들의 참여 확대도 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "Duke Energy",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "duke energy applauds department of energy s new nuclear investments helps advance deployment of smrs in the u s duke energy",
    "id": "dc5f1960"
  },
  {
    "date": "2025-12-03",
    "title": "미국 에너지부(DOE), 테네시밸리청(TVA)·홀텍(Holtec) 소형모듈원자로 프로젝트 선정 지원",
    "summary": "미국 에너지부(DOE)가 테네시밸리청(TVA)과 홀텍(Holtec)의 소형모듈원자로 프로젝트를 지원 대상으로 선정했다.",
    "summaryLong": "미국 에너지부(DOE)는 소형모듈원자로 개발 촉진을 위해 테네시밸리청(TVA)과 홀텍(Holtec)이 추진하는 두 개의 프로젝트를 연방 자금 지원 대상으로 선정했다. 두 프로젝트는 각각 다른 지역과 기술 접근 방식을 기반으로 하고 있다. 이는 미국 정부의 소형모듈원자로 상용화 가속화 의지를 보여주는 정책적 결정이다. 선정된 프로젝트들의 성공은 미국의 차세대 핵 에너지 산업 발전의 기초가 될 것으로 예상된다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "TVA,Holtec",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMilgFBVV95cUxQYkVKSC1ZX0l5eE9wRUpSZU1tNS1raVk4QVozcl92blBDZEU3RXVwNXJFY2oyb3BlR0QwcFFBSlVmRFAxbnNtVE9ELXhDR2tSdnBTWWJXM0JBamszeVg0eVp2SThaLVJPNjJGcXp1UXRBR2FqQ0xqTW03Ul93MF9hOHYzZmI2MjdscjVzRnBMOS1lRFoyenc?oc=5",
    "k": "doe selects tva and holtec for smr awards american nuclear society ans",
    "id": "41122084"
  },
  {
    "date": "2025-12-03",
    "title": "소형모듈원자로(SMR)의 역할과 AI 에너지 수요 연결성",
    "summary": "인공지능 에너지 수요 증가 속에서 소형모듈원자로의 개념과 역할을 설명하는 교육 콘텐츠를 제공한다.",
    "summaryLong": "소형모듈원자로(SMR)의 기술 개념과 운영 원리를 설명하는 교육 자료가 배포되고 있다. 특히 인공지능 데이터센터 등 신규 전력 수요의 급증 속에서 소형모듈원자로의 역할이 부각되고 있다. 소형모듈원자로는 분산형 배치, 낮은 초기 투자 비용, 유연한 운영이 가능하여 AI 시대의 전력 공급 솔루션으로 주목받고 있다. 이러한 교육 콘텐츠는 일반 대중의 소형모듈원자로 이해도 증진에 기여할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "what are small modular reactors a new type of nuclear power plant sought to feed ai s energy demand",
    "id": "568cb4ec"
  },
  {
    "date": "2025-12-03",
    "title": "경제협력개발기구 핵에너지기구(NEA), 소형모듈원자로 안전평가 최근 진전과 과제 논의",
    "summary": "경제협력개발기구 핵에너지기구(NEA)가 소형모듈원자로의 안전평가 관련 최근 기술 진전과 도전 과제를 다루는 워크숍을 개최했다.",
    "summaryLong": "국제 핵에너지 전문기구인 경제협력개발기구 핵에너지기구(NEA)는 소형모듈원자로의 안전평가에 관한 국제 워크숍을 개최했다. 워크숍에서는 최근의 기술적 진전 사항과 규제 승인 과정에서의 과제들이 논의되었다. 각국의 규제 당국과 개발사들이 참여하여 안전 기준 조화와 평가 절차 개선방안을 모색했다. 이는 글로벌 소형모듈원자로 상용화 촉진을 위한 국제 협력의 중요한 사례로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "NEA",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "workshop highlights recent advances and challenges in small modular reactor safety assessment",
    "id": "1e209234"
  },
  {
    "date": "2025-12-03",
    "title": "미국 연방 자금 선정 소형모듈원자로 프로젝트 2개 발표",
    "summary": "미국 연방정부의 자금 지원을 받기로 선정된 소형모듈원자로 프로젝트 2개가 발표되었다.",
    "summaryLong": "미국 연방정부는 소형모듈원자로 상용화 촉진을 위해 2개의 프로젝트를 투자 대상으로 선정했다. 선정된 프로젝트들은 서로 다른 기술 방식과 지역을 대상으로 하고 있어 다양한 접근 방식의 검증을 가능하게 한다. 연방 자금 지원은 소형모듈원자로 개발의 기술적, 재정적 장벽을 낮추는 역할을 한다. 프로젝트들의 진행 상황은 미국의 원자력 에너지 정책 성과를 가늠하는 중요한 지표가 될 것으로 예상된다.",
    "cat": "투자",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMilwFBVV95cUxQVDBzQTFWTGNfQzlfZGVSTFF6WExxQUw2N241TEdkS3A4bVZfeno0eEo1TW1QUmFFOXZGX0l5ZmE1TE1oOG1NbUNMdjA5dG9GQnpSLW94WFdUQ291T1FYcDhGNXl4N1dERnR0cnZpdnkyZ1RfSjdrZWdENGFUTDA0NEJuZGhESWFYalVQUC1lSDBWeVhGNmpr?oc=5",
    "k": "two smr projects selected for us federal funding",
    "id": "a11e16d2"
  },
  {
    "date": "2025-12-03",
    "title": "테네시밸리오써리티(TVA) 차세대 소형모듈원자로 4억 달러 정부 지원금 획득",
    "summary": "미국 테네시밸리오써리티(TVA)가 오크릿지(Oak Ridge)의 차세대 소형모듈원자로(SMR) 사업에 4억 달러 규모 정부 지원금을 확보했다.",
    "summaryLong": "미국 테네시밸리오써리티(TVA)가 테네시 오크릿지(Oak Ridge)에서 추진하는 차세대 소형모듈원자로(SMR) 프로젝트를 위해 4억 달러 규모의 정부 지원금을 획득했다. 이는 미국 연방정부의 SMR 상용화 촉진 정책의 일환으로, TVA의 SMR 배치 계획 가속화를 지원한다. 해당 사업은 오크릿지 지역의 경제 활성화 및 청정에너지 공급에 기여할 것으로 기대된다.",
    "cat": "정책",
    "type": "General",
    "dev": "TVA",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "tva wins 400m grant for next gen small modular reactor in oak ridge",
    "id": "77636dbd"
  },
  {
    "date": "2025-12-02",
    "title": "테네시주, 클린치 강 부지 미국 최초 소형모듈원자로 건설 연방 정부 지원금 확보",
    "summary": "테네시주가 클린치 강(Clinch River) 부지에서 미국 최초의 소형모듈원자로 건설을 위한 연방정부 지원금을 확보했다.",
    "summaryLong": "테네시주는 클린치 강 부지에 미국 최초의 상용 소형모듈원자로를 건설하기 위해 미국 에너지부(DOE)로부터 연방정부 지원금을 받게 되었다. 이는 테네시밸리청(TVA)이 주도하는 프로젝트로 미국의 원자력 에너지 정책 전환을 상징한다. 클린치 강 부지는 기존 원자력 시설과의 인접성으로 인해 인프라 활용이 용이하다. 성공적 완공 시 테네시주는 저탄소 에너지 기반 경제 발전의 거점이 될 것으로 기대된다.",
    "cat": "정책",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMi-?oc=5",
    "k": "tennessee secures u s department of energy grant to develop nation s first small modular reactor at clinch river site",
    "id": "c0a23cf6"
  },
  {
    "date": "2025-12-02",
    "title": "테네시밸리오서티(TVA), SMR 개발에 연방 4억달러 지원금 획득",
    "summary": "미국 테네시밸리오서티(TVA)가 소형모듈원자로(SMR) 개발을 위해 연방정부로부터 4억달러의 지원금을 확보했다.",
    "summaryLong": "미국 테네시밸리오서티(TVA)가 소형모듈원자로 개발 사업을 위해 연방정부로부터 4억달러의 지원금을 획득했다. 이는 미국의 차세대 원자력 에너지 기술 개발을 지원하기 위한 연방정부의 정책 일환이다. 이번 지원은 SMR 상용화 및 배치 확대에 있어 중요한 자금 기반이 될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "TVA",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMimgFBVV95cUxNcnNPZkc5Q1czRkZQd0wxZXBUTHp5dFZsRmVKTFA2QWVLLUs2eFo3SHo3N282WkpGWWJkb0dzNjNNb2lTczRiNGNmcmdEMFF2ZlFFY0ZDZjh3SUtYVzJQY2xDWU1DaExqblhtejU2dG9zRHdhYk1wSF8tcFIwUjNYVGRIMXMxaXdNU01Sd0VRWmRPZ3BSNHlQLUFB?oc=5",
    "k": "tva wins 400m federal grant for small modular nuclear reactor",
    "id": "5555691a"
  },
  {
    "date": "2025-12-02",
    "title": "테네시 동부에 미국 최초 SMR 건설 계획 발표",
    "summary": "테네시주 주지사가 미국 최초의 소형모듈원자로가 동부 테네시에 개발될 것이라고 발표했다.",
    "summaryLong": "테네시주 주지사가 미국 최초의 소형모듈원자로가 동부 테네시에 건설될 계획을 공식 발표했다. 이는 미국의 에너지 정책과 원자력 산업 확대 추진 의지를 보여주는 중요한 이정표이다. 동부 테네시 지역의 에너지 공급 안정성 및 지역 경제 활성화에 기여할 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "country s first small modular reactor to be developed in east tennessee governor says",
    "id": "f1a51a61"
  },
  {
    "date": "2025-11",
    "title": "X-energy, TRISO-X 연료 조사시험 개시 및 TX-1 착공",
    "summary": "X-energy가 아이다호국립연구소(INL) 고급시험로(ATR)에서 TRISO-X 연료 조사시험을 시작하고, 테네시 오크리지(Oak Ridge)의 TX-1 연료제조시설 건설을 착수했다.",
    "cat": "기술",
    "type": "HTGR",
    "dev": "X-energy",
    "region": "US",
    "source": "X-energy TRISO-X",
    "url": "https://x-energy.com/triso-x-fuel/",
    "summaryLong": "X-energy가 아이다호국립연구소(INL) 고급시험로(ATR)에서 TRISO-X 고온가스로(HTGR) 연료의 조사시험을 시작하고, 동시에 테네시 오크리지에 TX-1 연료제조시설 건설을 착공했다. TRISO-X 연료는 차세대 원자로의 핵심 기술로서 안전성과 성능 향상에 중요한 역할을 한다. 이번 시험과 시설 건설은 상용 규모의 TRISO-X 연료 생산 기반 마련을 의미하며, SMR 산업 발전에 중요한 진전이다.",
    "id": "b2bb170b"
  },
  {
    "date": "2025-11-28",
    "title": "국제원자력기구(IAEA), SMR 규제 교육 프로그램 개시",
    "summary": "국제원자력기구(IAEA)가 소형모듈원자로 규제 전문가 양성을 위한 시범 교육 프로그램을 시작했다.",
    "summaryLong": "국제원자력기구(IAEA)가 소형모듈원자로의 규제 체계 구축을 위해 규제 전문가 양성 교육 프로그램을 개시했다. 이 프로그램은 각국의 규제당국 담당자들을 대상으로 SMR 관련 규제 기술 및 안전 기준을 교육한다. SMR 기술의 빠른 발전에 따라 국제적 규제 표준화와 안전 관리 역량 강화가 필수적이며, 이 교육은 그러한 필요성에 부응하는 의미있는 노력이다.",
    "cat": "기술",
    "type": "General",
    "dev": "IAEA",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiogFBVV95cUxOdGdqRl9lclc0UFNKRkJRQ3J1UnRCSE1QV2FPX2dMbjE5a1BVQWpRVWdmN3I1WnRLRG1hTmR5R043eVZpYmpXcGF4N1hoNW90WVEyYkdiN0JRc2h0ZnBfVld6bzQxa0FKVUV3SFBMcWo1Rl9NLU9WUV9maUtINHdNNFZVRXhKdllIb2x2VFZmOVYxZTF6cWdwRTNDUkxkb2ZLeHc?oc=5",
    "k": "iaea launches pilot school for regulating small modular reactors",
    "id": "e2092447"
  },
  {
    "date": "2025-11-28",
    "title": "뉴스케일(NuScale), 경쟁 앞서지만 수익성 불확실",
    "summary": "뉴스케일(NuScale)의 소형모듈원자로(SMR)가 경쟁사 대비 기술 선도하고 있으나 수익성 전망에는 의문이 제기된다.",
    "summaryLong": "미국의 뉴스케일(NuScale)은 현재 경쟁업체들보다 기술적으로 앞서가고 있는 것으로 평가받고 있다. 그러나 투자 분석가들은 뉴스케일의 소형모듈원자로(SMR) 사업의 장기적 수익성에 대해 의문을 제기하고 있다. 이는 SMR 산업이 기술 성숙도와 경제성 달성 사이의 간극을 여전히 안고 있음을 시사한다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "NuScale",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "nuscale small modular reactor ahead of competition profitability uncertain",
    "id": "32c3eb3e"
  },
  {
    "date": "2025-11-25",
    "title": "소형모듈원자로 개발 진전을 저해하는 요인들",
    "summary": "소형모듈원자로(SMR) 개발과 배치를 방해하는 여러 기술적, 규제적, 경제적 과제들을 분석한 보도다.",
    "summaryLong": "GIS Reports는 소형모듈원자로(SMR)의 상용화 및 배치 진행을 저해하는 주요 장애 요소들을 검토하고 있다. 소형모듈원자로는 높은 기술력과 혁신성이 기대되지만, 개별 용량 제한에 따른 경제성 문제, 규제 인증의 복잡성, 공급망 부재, 초기 자본 비용의 높음 등 다양한 현실적 난제에 직면해 있다. 이 보도는 소형모듈원자로 산업이 상용화 단계로 나아가기 위해 해결해야 할 핵심 과제들을 도출하고 업계와 정부의 관심을 환기시키는 의미를 갖는다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiUkFVX3lxTE9aQ2draXdnTWRvcXBvaHVseFNSNm82N2hEUGxzR1pmOUFrOVRGRTVoYWVXN3NwbVlBem9sbkN6MGhRM1pfYW1qOGlRc1V4R1ptRnc?oc=5",
    "k": "what is holding up progress on small modular reactors",
    "id": "d4f507d3"
  },
  {
    "date": "2025-11-20",
    "title": "소형모듈원자로(SMR) 시장 2031년 64.8억 달러 규모로 확대 전망",
    "summary": "글로벌 소형모듈원자로(SMR) 시장이 2031년까지 64.8억 달러 규모로 성장할 것으로 예측되었다.",
    "summaryLong": "데이터인텔리전스(DataM Intelligence) 분석에 따르면 소형모듈원자로(SMR) 글로벌 시장이 2031년까지 64.8억 달러 규모로 확대될 것으로 전망된다. 이는 안전성, 환경 친화성, 탄소중립 달성 강화에 따른 저탄소 전력 수요 증가 및 에너지 시스템의 모듈화 추세를 반영한다. 시장 성장은 기술 성숙도 향상, 규제 승인 확대, 건설 프로젝트 증가 등 다양한 요인에 의해 촉진될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMihwFBVV95cUxOVmR4cXNKOS13ajZSRFA4V094QlkzbW5udEZ6ODl0VmZJb3FZR0xhcEdXa2J3RzRfZW9mQTdIakNkby12OURJbEt1a1FqNXJ5MkptZlRDMHV4SFRwdEMtOG1uVHdJVHAtQ0MyaHhsQWhLQVp3clVKbDQwdUphS0pKNDRSQzdpQjg?oc=5",
    "k": "small modular reactor market to reach usd 6 48 billion by 2031 as global energy systems shift toward safe modular low carbon power according to datam intelligence",
    "id": "9aecb0f3"
  },
  {
    "date": "2025-11-19",
    "title": "태양광-SMR 하이브리드 마이크로그리드 운영 최적화 연구",
    "summary": "태양광 발전과 소형모듈원자로를 결합한 하이브리드 마이크로그리드의 운영 최적화 방안에 관한 연구 논문이 발표되었다.",
    "summaryLong": "재생에너지인 태양광 발전과 소형모듈원자로를 통합한 하이브리드 마이크로그리드의 효율적 운영 및 최적화 방법에 관한 연구 결과가 발표되었다. 이 연구는 변동성 있는 재생에너지와 안정적인 원자력 에너지의 조화로운 운영을 통해 에너지 공급 신뢰성과 경제성을 동시에 달성할 수 있는 방안을 제시한다. 이러한 통합형 에너지 시스템은 탄소중립 목표 달성과 에너지 안보 강화에 중요한 역할을 할 수 있는 미래형 전력망 모델로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiX0FVX3lxTE5DOUczakFmdzJLclJIS0pDMlRWTWNxZTctWXEzd2hWSFk1RUtnUE10YXBNWkw4bTZsbTVzcDhrMW1jQ0l4bmoyNHJuUmZaaW5aVGQ2elU1SUpSc0Y4RVJZ?oc=5",
    "k": "coordinated operation and multi layered optimization of hybrid photovoltaic small modular reactor microgrids",
    "id": "1249ae8e"
  },
  {
    "date": "2025-11-18",
    "title": "세계은행, SMR 산업의 기술 선택과 집중 강조",
    "summary": "세계은행 총재가 소형모듈원자로 산업이 최적 기술 1~2개 개발에 집중해야 한다고 강조했다.",
    "summaryLong": "세계은행 총재가 소형모듈원자로 산업의 기술 개발 전략으로 최고 수준의 기술 1~2개에 자원과 역량을 집중할 필요성을 강조했다. 현재 다양한 SMR 기술이 개발 중이지만, 분산된 투자와 개발은 산업 성숙도와 상용화를 지연시킬 수 있다는 우려를 반영한 의견이다. 이는 SMR 산업의 효율적 발전과 경제적 타당성 확보를 위한 전략적 방향성을 제시하는 것으로, 향후 글로벌 SMR 정책 및 투자 결정에 영향을 미칠 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "small modular reactor industry must focus on developing best one or two technologies says world bank head",
    "id": "330276b3"
  },
  {
    "date": "2025-11-17",
    "title": "경제협력개발기구 원자력청(NEA), SMR 디지털 대시보드 사용자 피드백 조사",
    "summary": "경제협력개발기구 원자력청(NEA)이 소형모듈원자로 디지털 대시보드에 대한 사용자 피드백을 수집하고 있다.",
    "summaryLong": "경제협력개발기구 원자력청(NEA)이 소형모듈원자로 관련 정보 제공과 산업 현황 추적을 위해 개발한 디지털 대시보드에 대한 사용자 피드백을 수집 중이다. 이 대시보드는 전 세계 SMR 프로젝트 진행 상황, 기술 동향, 규제 정보 등을 통합적으로 제공하는 플랫폼이다. 사용자 피드백을 통해 대시보드의 기능성과 유용성을 개선하고, SMR 산업의 정보 투명성과 접근성을 높이려는 노력으로 평가된다.",
    "cat": "기술",
    "type": "General",
    "dev": "NEA",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMirAFBVV95cUxPMmJoS1ozNjV4QVRoVW1YeG5FMlVIVjRrREgyTk9ONGFFeHQ2SmhZbURpbTJUcmZpclJwWkVvRTBxRmZCQXRWaUM0Nl9kQmFPbmpDTXJRRmdOcnVQdG4zM3hPYnZubVBSZ0lpV3d5Vmo3RTlsNF8yMnZ6Mk1aWUhXNVlUcGdBb0twM3ZfT1hRaXFSeWZSQklPcU5lX21uNXNqdlRwN1duekxmdWU0?oc=5",
    "k": "user feedback survey nea small modular reactor smr digital dashboard",
    "id": "aced4de5"
  },
  {
    "date": "2025-11-14",
    "title": "미국 규제당국, 애팔래치아전력의 SMR 부지비 지출 승인",
    "summary": "미국 규제당국이 애팔래치아전력(Appalachian Power)의 소형모듈원자로 개발을 위한 부지비 지출을 승인했다.",
    "summaryLong": "미국 규제당국이 애팔래치아전력의 소형모듈원자로 프로젝트 추진을 위한 부지 조성 및 개발 비용 지출을 공식 승인했다. 이는 SMR 개발 사업의 부지 확보 및 기반 조성이 진행 중임을 의미하며, 실제 건설 단계로의 진전을 나타낸다. 규제당국의 승인은 애팔래치아전력의 SMR 프로젝트 추진에 필요한 자금 조달 경로를 열어주며, 지역 에너지 공급 다변화와 미국의 SMR 상용화 가속화에 기여할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "Appalachian Power",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "regulators give appalachian power green light to incur small modular nuclear reactor site costs",
    "id": "6d4ed639"
  },
  {
    "date": "2025-11-10",
    "title": "유럽연합(EU), 소형모듈원자로(SMR) 전략 수립을 위한 의견 수렴",
    "summary": "유럽연합(EU) 집행위원회가 소형모듈원자로(SMR) 전략 수립을 위해 이해관계자들의 의견을 수렴 중이다.",
    "summaryLong": "유럽연합(EU) 집행위원회는 소형모듈원자로(SMR) 관련 향후 정책 방향을 수립하기 위해 공식적인 의견 수렴 절차를 진행하고 있다. 이는 에너지 전환과 탄소 중립 목표 달성을 위한 원자력의 역할을 강화하려는 유럽연합(EU)의 의지를 반영한다. 의견 수렴 과정에는 산업계, 연구기관, 정부 등 다양한 이해관계자들이 참여하여 소형모듈원자로(SMR)의 기술적, 경제적, 규제적 측면에 대한 의견을 제시할 예정이다. 이를 통해 유럽연합(EU)은 소형모듈원자로(SMR) 산업의 발전 가능성과 실현 방안을 구체화할 수 있을 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "EU",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "commission invites views to shape its small modular reactor strategy",
    "id": "0f30e462"
  },
  {
    "date": "2025-11-10",
    "title": "미국 원자력규제위원회(NRC), 테네시밸리공사(TVA) 소형모듈원자로(SMR) 승인 권고",
    "summary": "미국 원자력규제위원회(NRC)가 테네시밸리공사(TVA)의 소형모듈원자로(SMR) 프로젝트 승인을 권고했다.",
    "summaryLong": "미국 원자력규제위원회(NRC)는 테네시밸리공사(TVA)가 진행하는 소형모듈원자로(SMR) 개발 프로젝트에 대해 긍정적인 심사 결과를 바탕으로 승인을 권고했다. 이는 미국 내 소형모듈원자로(SMR) 상용화 진전에 있어 중요한 이정표로, 규제 당국이 기술 안전성과 구현 가능성을 인정했음을 의미한다. 테네시밸desarley공사(TVA)는 미국 남동부 지역의 전력 공급을 담당하는 주요 공사로, 이 프로젝트의 진행은 지역 에너지 안보 강화에도 기여할 것으로 보인다. 향후 정식 승인 획득 시 소형모듈원자로(SMR)의 실증화 단계로 한 걸음 더 나아갈 수 있을 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "TVA",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMinAFBVV95cUxQLUlOQjlmTXViUFJVMGNYZkVrY2FEZ0FhOThudFpwRlpXZEtwWld5LWIxZVFjWXJKY0s2cjJBTmVrdi1vVTFJSTd4TnBxNzRSaW9PNGZnVnJPYnBCZ3RvR1lyY1J0YS13U2N6VnpxdTNzMHFtLUY2S3FVWTI3RV9zN1ozZzF5emVQb3VjSi1wWmR3Sl9PMVplZHEtYjY?oc=5",
    "k": "nuclear agency recommends approval of tva small modular reactor",
    "id": "9e0d8f3f"
  },
  {
    "date": "2025-11-06",
    "title": "미국 에너지(AES), 소형모듈원자로(SMR) 발표, 퍼듀대학(Purdue University) 핵 정상회담에서 주목",
    "summary": "미국 에너지(AES)가 소형모듈원자로(SMR) 관련 발표를 하며 퍼듀대학(Purdue University) 핵 정상회담에서 주목받았다.",
    "summaryLong": "에너지 기업 미국 에너지(AES)는 퍼듀대학(Purdue University)에서 개최된 핵 정상회담에서 소형모듈원자로(SMR) 관련 기술 또는 사업 추진 현황을 발표했다. 퍼듀대학(Purdue University) 핵 정상회담은 원자력 에너지의 미래와 산업 발전 방향을 논의하는 주요 행사로, 미국 에너지(AES)의 발표는 업계의 관심을 모았다. 이를 통해 미국 에너지(AES)는 소형모듈원자로(SMR) 분야에서의 기술 역량과 사업 전략을 대외적으로 알릴 수 있었다. 이는 소형모듈원자로(SMR) 산업의 발전과 관련 투자 확대를 촉진하는 데 긍정적인 영향을 미칠 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "AES",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "aes small modular reactor announcement highlights nuclear summit at purdue university",
    "id": "e5289107"
  },
  {
    "date": "2025-11-06",
    "title": "프랑스 전력공사(EDF), 2050년까지 소형모듈원자로(SMR) 30기 생산 목표, 2026년 설계 완료 예정",
    "summary": "프랑스 전력공사(EDF)가 2050년까지 30기의 소형모듈원자로(SMR) 생산을 목표로 하며 2026년 설계 완료를 계획하고 있다.",
    "summaryLong": "프랑스 전력공사(EDF)는 야심찬 소형모듈원자로(SMR) 개발 목표를 수립했으며, 2026년까지 설계를 완료하고 2050년까지 총 30기를 생산할 계획이다. 이는 프랑스의 에너지 전환 정책과 탄소 중립 목표 달성을 위한 핵심 전략으로, 소형모듈원자로(SMR)를 미래 에너지 믹스의 중요 요소로 보고 있음을 보여준다. 프랑스 전력공사(EDF)는 유럽에서 가장 큰 에너지 회사 중 하나로, 이러한 투자는 유럽 전역의 소형모듈원자로(SMR) 산업 발전을 견인할 수 있다. 성공적인 추진 시 프랑스뿐 아니라 유럽 전체의 에너지 안보와 기후 목표 달성에 상당한 기여를 할 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "EDF",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "edf aims to produce 30 small modular reactors by 2050 design planned for 2026",
    "id": "d980b185"
  },
  {
    "date": "2025-11-01",
    "title": "국제원자력기구(IAEA), 소형모듈원자로(SMR) 플랫폼 및 응용 프로그램",
    "summary": "국제원자력기구(IAEA)가 소형모듈원자로(SMR)와 그 응용에 대한 플랫폼을 제공하고 있다.",
    "summaryLong": "국제원자력기구(IAEA)는 전 세계 소형모듈원자로(SMR) 관련 기술 정보와 응용 방안을 종합적으로 다루는 플랫폼을 운영하고 있다. 이 플랫폼은 회원국들이 소형모듈원자로(SMR)의 설계, 규제, 경제성 평가 등 다양한 분야의 정보에 접근하도록 지원하며, 국제적 표준과 모범 사례를 공유하는 역할을 한다. 국제원자력기구(IAEA)의 플랫폼은 개발도상국을 포함한 모든 회원국의 소형모듈원자로(SMR) 도입 추진을 돕는다. 이는 소형모듈원자로(SMR) 기술의 글로벌 확산과 국제 협력 강화를 통해 세계 에너지 안보 및 기후 변화 대응을 가속화하는 데 기여할 것으로 보인다.",
    "cat": "기술",
    "type": "General",
    "dev": "IAEA",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "iaea platform on small modular reactors and their applications",
    "id": "1a89ebef"
  },
  {
    "date": "2025-10-28",
    "title": "프랑스의 소형모듈원자로 도입 과제",
    "summary": "프랑스가 소형모듈원자로(SMR) 도입을 추진하면서 기술·산업·규제 측면의 주요 과제들에 직면하고 있다.",
    "summaryLong": "프랑스는 탈탄소화와 에너지 안보를 위해 소형모듈원자로(SMR) 개발·도입을 추진 중이나, 기술 실증, 산업 기반 구축, 규제 체계 정비 등 다층적 과제에 마주하고 있다. 기술 컨설팅 기업 알텐(Alten)이 지적한 바와 같이 프랑스는 기존 대형 원전 중심의 산업 생태계를 SMR에 맞게 전환해야 한다. 이는 공급망 재편, 인력 양성, 규제 인증 준비 등을 포괄한다. 프랑스의 SMR 전략은 유럽 내 에너지 정책 리더십과 산업 경쟁력 확보의 시금석이 될 것으로 보인다.",
    "cat": "기술",
    "type": "General",
    "dev": "Alten",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiggFBVV95cUxONmMyMmEzc3N2MjRBSHZ1cXhpUlJycWdCZFhwbVduSDVNWURhM1Nkel9SeWdBc2F3X0VLVi0xLUxTM3JhbnRvdTcyYUpyZUloZ0VfMzBzUWZvcVBFYnMwcFB6ZnNCOWREM1RPakhKS1p1X1pqMXBSRUdBb0dPQVJMSURR?oc=5",
    "k": "small modular reactors france faces a major challenge",
    "id": "9a505348"
  },
  {
    "date": "2025-10-27",
    "title": "아마존 지원 소형모듈원자로(SMR) 발전소 설계 시공사 선정",
    "summary": "아마존이 지원하는 소형모듈원자로(SMR) 발전소 프로젝트에 설계 및 시공사가 임명되었다.",
    "summaryLong": "아마존이 투자하거나 지원하는 소형모듈원자로(SMR) 발전소 건설 프로젝트에서 설계 및 시공을 담당할 기업이 정식으로 선정되었다. 이는 프로젝트가 구체적인 실행 단계로 진입했음을 의미하며, 아마존의 데이터센터 등 대규모 에너지 수요 충당을 위한 탄소 중립 에너지 확보 전략의 일환이다. 선정된 설계 시공사는 소형모듈원자로(SMR) 기술의 실현 가능성을 검증하고 건설을 추진하게 된다. 이 프로젝트의 성공적 완료는 기업 부문의 소형모듈원자로(SMR) 도입을 촉진하고, 데이터센터 등 산업용 에너지 공급의 새로운 모델을 제시할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "design builder appointed for amazon backed small modular reactor plant",
    "id": "9fcfd183"
  },
  {
    "date": "2025-10-25",
    "title": "미국 워싱턴주 첫 소형모듈원자로(SMR) 건설팀, 리치랜드(Richland) 근처 선정",
    "summary": "미국 워싱턴주에서 리치랜드(Richland) 근처에 소형모듈원자로(SMR)를 건설할 팀이 선정됐다.",
    "summaryLong": "미국 워싱턴주는 리치랜드(Richland) 지역 근처를 소형모듈원자로(SMR) 건설 추진을 위한 팀 선정 지역으로 결정했다. 리치랜드(Richland)는 미국 북서부의 핵 관련 시설과 기술 인프라가 집중된 지역으로, 소형모듈원자로(SMR) 건설을 위한 기반이 잘 갖춰져 있다. 워싱턴주의 첫 소형모듈원자로(SMR) 프로젝트 추진은 미국 태평양 북서부 지역의 에너지 자립성 강화와 탄소 감축을 목표로 한다. 이 프로젝트의 성공적 추진은 미국 내 소형모듈원자로(SMR) 도입 확대의 모범 사례가 되어, 다른 주의 소형모듈원자로(SMR) 도입 추진에도 긍정적인 영향을 미칠 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "wa 1st small modular nuclear reactors team picked to build near richland tri city herald",
    "id": "b62ba4b4"
  },
  {
    "date": "2025-10-22",
    "title": "경제협력개발기구 원자력기관(NEA), 소형모듈원자로(SMR) 디지털 대시보드 업데이트",
    "summary": "경제협력개발기구 원자력기관(NEA)이 소형모듈원자로(SMR) 관련 정보를 종합한 디지털 대시보드를 업데이트했다.",
    "summaryLong": "경제협력개발기구 원자력기관(NEA)은 소형모듈원자로(SMR) 분야의 최신 정보를 통합 관리하는 디지털 대시보드를 새롭게 업데이트했다. 이 대시보드는 전 세계 소형모듈원자로(SMR) 개발 현황, 프로젝트 추진 상황, 기술 규격 등 다양한 정보를 한눈에 볼 수 있도록 구성되어 있다. 경제협력개발기구 원자력기관(NEA)의 회원국들과 국제 커뮤니티는 이 대시보드를 통해 소형모듈원자로(SMR) 산업 동향을 파악하고 정책 수립에 활용할 수 있다. 정기적인 업데이트를 통해 경제협력개발기구 원자력기관(NEA)은 소형모듈원자로(SMR) 개발의 투명성을 높이고 국제 협력을 강화하는 데 기여할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "NEA",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMioAFBVV95cUxQTmtLeFZLa1NVbjR4SzEwRmN3VE1ENG9VZFlDM0NDRGdaeUZwWk9RRnVsZXcyZVFKVXl5R1I2MXpoc21qelZxcV9NTmpkcVRBMHJxVEN4U3kwckZfaWxMM09YdG1tdHZ1YXdFMVlmMTJaSFlHbnNDdzc2S1pRckNtaWdJTTJzemdrTkl1NWFpTVVZM3BuVlZfWjA1LW42aGc3?oc=5",
    "k": "open for update nea small modular reactor digital dashboard",
    "id": "4b2f150d"
  },
  {
    "date": "2025-10-16",
    "title": "워싱턴 시설에 아마존 자금 소형모듈원자로(SMR) 12기 배치",
    "summary": "아마존의 자금 지원으로 워싱턴 지역의 핵 시설에 12기의 소형모듈원자로가 배치될 예정이다.",
    "summaryLong": "아마존(Amazon)의 자금 지원을 받는 소형모듈원자로(SMR) 배치 프로젝트가 워싱턴(Washington) 시설에서 12기의 소형모듈원자로를 운영할 계획을 발표했다. 이는 대형 기술 기업의 데이터센터 전력 수요 증가 추세와 탄소중립 목표를 충족하기 위한 소형모듈원자로 도입의 현실화를 보여주는 사례다. 아마존과 같은 대규모 에너지 소비 기업들의 소형모듈원자로 투자는 SMR 산업의 경제성 입증과 시장 수요 창출에 중추적 역할을 하며, 향후 유사 기업들의 참여 확대 및 소형모듈원자로 상용화 가속을 촉진할 것으로 기대된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMinAFBVV95cUxQYU5FWW5fTVlyejhNQ29qM05CZ0lWN1F0NGlTVElsOGJPQ2xJRXVvc0hEeFZId1QxV3lLYl9aNFFwN19kLWNSdzNzMXNtRG1sc1dLNGJOeXp2U1lzazFtR2RhMXQyY2lkdmtHUjRDNm1pUnFQWWlSRHNvakdlUG9BTnFsTGsyMmNmdmxPbUo5bmpuaU1qcFNValZ2bVg?oc=5",
    "k": "washington nuclear facility will deploy 12 amazon funded smrs",
    "id": "32eea43a"
  },
  {
    "date": "2025-10-14",
    "title": "원자력 산업계가 감추는 SMR의 불편한 진실",
    "summary": "소형모듈원자로(SMR) 산업 확대의 낙관론 뒤에 숨은 기술적·경제적 문제점들을 지적하는 비판 기사이다.",
    "summaryLong": "원자력 산업계가 주장하는 소형모듈원자로(SMR)의 장점과 미래 가능성에 대해 비판적 관점에서 접근하는 기사이다. SMR 개발 및 상용화 과정에서 경제성 문제, 건설 비용, 규제 불확실성, 기술 성숙도 등 다양한 장애 요소들이 충분히 공론화되지 못하고 있음을 지적한다. 이는 낙관적인 SMR 산업 진흥 움직임에 대한 균형잡힌 평가와 현실적 과제 점검의 필요성을 제기하는 언론 비평이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "KR",
    "source": "\"SMR 원자력\" - Google 뉴스",
    "url": "https://news.google.com/rss/articles/CBMid0FVX3lxTFBONjhDU3VxTS00UHlHXzZoRVV2SUJyelBCb3g5UVlpY0lRaEZUbVlsUWZjWm1YdFFDYWV4RkJHbWtkc1hUVnRfLWx0TTBmUC1ReXplNERfV0NpRTRuOE9xSE4xUVBFdU01d2lqZ0VsaHNENWFnczZF?oc=5",
    "k": "새로운 미래 원자력 산업계가 감추는 smr의 불편한 진실",
    "id": "7f38d9a8"
  },
  {
    "date": "2025-10-10",
    "title": "우즈베키스탄 소형모듈원자로(SMR) 건설 발굴 공사 착수",
    "summary": "우즈베키스탄에서 소형모듈원자로 건설을 위한 발굴 공사가 시작됐다.",
    "summaryLong": "우즈베키스탄이 소형모듈원자로 도입을 추진하며 건설 부지에서 발굴 공사를 개시했다. 이는 중앙아시아 지역의 원자력 에너지 확대 전략의 일환이다. 소형모듈원자로는 작은 부지에서도 설치 가능한 차세대 원자력 기술로, 에너지 수급에 어려움을 겪는 국가들의 관심이 높아지고 있다. 우즈베키스탄의 프로젝트 진행 상황은 중앙아시아의 원자력 산업 발전을 보여주는 지표가 될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMinwFBVV95cUxNRXRfc1F1a2lud09JMnVyMFhrcFktSURRQS1EbUIyNU5XVmczY2RhMlZYT3labXBJZFI3OXZlVVNRQlEyamFKcFk4OUtqRkVxYjI2b3dKNGNHTjB4QUlmNVhlcTFMVUtLNTlTc0R6eFNvaWg3T25uNk1WNlA5YWY0RzZBWERHdzYyZFJLR1M2ZDJmTDdoUzVMQlNPVDFDaGs?oc=5",
    "k": "excavation works begin for uzbekistan small modular reactor",
    "id": "61dec3d7"
  },
  {
    "date": "2025-10-05",
    "title": "퍼듀 대학(Purdue University) Korsnick 강연 - SMR 핵 정책 연구",
    "summary": "퍼듀 대학(Purdue University)에서 소형모듈원자로 관련 핵 정책 강연이 개최됐다.",
    "summaryLong": "퍼듀 대학(Purdue University)이 소형모듈원자로 분야의 정책 및 전략을 주제로 한 강연을 개최했다. Korsnick 연사가 소형모듈원자로의 글로벌 확산과 관련 정책 과제를 논의했다. 이 강연은 미국의 원자력 정책 커뮤니티에서 소형모듈원자로에 대한 이해를 높이는 기회를 제공했다. 학계와 산업계의 협력을 통해 소형모듈원자로 기술의 상용화 경로를 모색하는 추세를 반영한다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMieEFVX3lxTE1aQnpLTGNGSXVadFQxVUdKNnVBR3lvamZPLTUtOU9VeTE2UWhIV1p1ZXdhSUhjaGhhYnpRR3JRNmlVMUc2MEJfYTRsOGtDZ1ZWdEU0M2dDTzlYa1J4bDg5cjZrc1RvM2ZYSjhIU295RGtfZUtEUXJsYg?oc=5",
    "k": "korsnick lecture small modular reactor smr nuclear studies",
    "id": "844eb2e6"
  },
  {
    "date": "2025-09",
    "title": "센트리카(Centrica)–X-Energy 공동개발협약, 영국 6GW 목표 하트클풀(Hartlepool) 우선 부지",
    "summary": "센트리카(Centrica)와 X-Energy가 영국 내 6GW 규모의 첨단모듈원자로 공동개발협약(JDA)을 체결, 하트클풀(Hartlepool)을 1차 배치 부지로 선정하고 2026년 본격화를 목표로 한다.",
    "cat": "계약",
    "type": "HTGR",
    "dev": "X-energy",
    "region": "UK",
    "source": "Centrica",
    "url": "https://www.centrica.com/media-centre/news/2025/centrica-and-x-energy-agree-to-deploy-uk-s-first-advanced-modular-reactors/",
    "summaryLong": "영국 에너지 기업 센트리카(Centrica)와 미국 원자로 개발사 X-Energy가 공동개발협약(JDA)을 통해 영국 최초의 첨단모듈원자로 배치 프로젝트를 추진하기로 합의했다. 이 협약은 영국 내 총 6GW 규모의 원자력 에너지 공급을 목표로 하며, 하트클풀(Hartlepool)을 1차 부지로 선정했다. 2026년 본격적인 건설 및 운영을 목표로 추진될 예정이다. 영국의 탄소 중립 목표 달성과 에너지 안보 강화에 기여할 것으로 기대된다.",
    "id": "878403ec"
  },
  {
    "date": "2025-09-25",
    "title": "X-Energy, Xe-100 고온가스로(HTGR) 캐나다 알버타주 배치 타당성 확인",
    "summary": "X-Energy가 Xe-100 고온가스로(HTGR)의 캐나다 알버타주 배치 가능성을 확인했다.",
    "summaryLong": "미국 원자로 개발사 X-Energy가 자사의 Xe-100 고온가스로(HTGR) 기술을 캐나다 알버타주에 배치하는 것의 타당성을 검증했다. 이는 캐나다의 원자력 산업 확대 정책과 알버타주의 에너지 다원화 전략과 부합한다. Xe-100은 고온 열을 활용한 산업용 응용이 가능한 차세대 소형모듈원자로이다. 향후 구체적인 프로젝트 추진 단계로 진행될 것으로 전망된다.",
    "cat": "기술",
    "type": "HTGR",
    "dev": "X-Energy",
    "region": "CA",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "x energy confirms feasibility of xe 100 advanced small modular reactor deployment in alberta x energy",
    "id": "7b584e20"
  },
  {
    "date": "2025-09-15",
    "title": "미국 콜로라도주 푸에블로, 소형모듈원자로(SMR) 도입 검토",
    "summary": "미국 콜로라도주 푸에블로가 소형모듈원자로 도입 가능성을 검토하고 있다.",
    "summaryLong": "미국 콜로라도주의 푸에블로시가 지역 에너지 공급 확대를 위해 소형모듈원자로 도입 타당성을 검토 중이다. 이는 미국 내 소형모듈원자로의 지역 수용성을 높이기 위한 움직임으로 해석된다. 푸에블로는 에너지 기반 경제 구조를 갖춘 지역으로, 원자력 에너지는 탄소 감축과 지역 경제 활성화를 동시에 달성할 수 있는 방안으로 주목받고 있다. 검토 결과에 따라 구체적인 사업 추진 여부가 결정될 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiigFBVV95cUxPamxXZ1JJWUk2MER6R2JQamQwTW1BUHdMTmdsU2hhZ2VDTHh1NUw0X29RM19abEJwZ19WYjNJdEZob0VycGotbFl6VzZ0aWM0MmVaUUdMbUhqM3NXSHcwa2psR0hCM1NBOXNBQmdHTFlRV19kak5RSDNaemwzWnJyTU9sWmRzMFhFQmc?oc=5",
    "k": "small modular nuclear reactors are having a moment will they arrive in time for pueblo",
    "id": "e5393f3b"
  },
  {
    "date": "2025-09-15",
    "title": "롤스로이스(Rolls-Royce), 영국·미국 정부의 SMR 정책 환영",
    "summary": "롤스로이스(Rolls-Royce)가 영국과 미국 정부의 원자력 에너지 정책 조치를 환영하며 'Golden Age(황금기)' 도래를 언급했다.",
    "summaryLong": "영국 방위산업 기업인 롤스로이스(Rolls-Royce)는 영국과 미국 정부가 추진 중인 원자력 에너지 정책에 대해 공개적으로 지지를 표명했다. 이는 SMR 산업 발전을 위한 정부 차원의 정책 지원이 가시화되고 있음을 시사한다. 양국 정부의 정책적 액션이 원자력 에너지의 새로운 성장 시대를 열 것으로 업계가 평가하고 있다.",
    "cat": "기술",
    "type": "PWR",
    "dev": "Rolls-Royce",
    "region": "UK",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "rolls royce welcomes action from uk and us governments to usher in new golden age of nuclear energy rolls royce plc",
    "id": "6e0e8805"
  },
  {
    "date": "2025-08",
    "title": "X-Energy, TX-1 TRISO-X 연료시설 건설사로 클락 컨스트럭션(Clark Construction) 선정",
    "summary": "X-Energy가 테네시 오크리지(Oak Ridge) TX-1 TRISO-X 연료시설 건설사로 클락 컨스트럭션(Clark Construction)을 선정했다.",
    "cat": "계약",
    "type": "HTGR",
    "dev": "X-energy",
    "region": "US",
    "source": "X-energy",
    "url": "https://x-energy.com/news-releases",
    "summaryLong": "X-Energy가 미국 테네시주 오크리지(Oak Ridge)에 건설할 TX-1 TRISO-X 연료시설의 건설사로 대형 건설사 클락 컨스트럭션(Clark Construction)을 선정했다. TRISO-X는 고온가스로용 고급 핵연료로, 안전성과 효율성이 뛰어난 제품이다. 이 시설은 미국의 첨단 원자력 연료 공급망 구축을 위한 핵심 프로젝트이다. 시설 완공을 통해 소형모듈원자로의 상용화 촉진이 기대된다.",
    "id": "497efff1"
  },
  {
    "date": "2025-08-25",
    "title": "X-Energy·아마존(Amazon)·한수원·두산에너빌리티 4자 전략적 협력",
    "summary": "X-Energy, 아마존(Amazon), 한국수력원자력(KHNP), 두산에너빌리티가 AI 인프라용 첨단원자력 확대를 위해 원자로 설계·공급망·건설·투자·운영·글로벌 공동배치 분야에서 전략적 협력을 추진하며, 한국 기업들이 공급망에 편입된다.",
    "cat": "계약",
    "type": "HTGR",
    "dev": "X-energy",
    "region": "US·KR",
    "source": "BusinessWire",
    "url": "https://www.businesswire.com/news/home/20250825430061/en/",
    "summaryLong": "X-Energy, 아마존(Amazon), 한국수력원자력(KHNP), 두산에너빌리티가 AI 데이터센터 등 대전력 수요 충당을 위한 첨단 원자력 솔루션 공급에 협력하기로 합의했다. 이 협약은 원자로 설계, 공급망 구축, 건설, 투자, 운영, 글로벌 공동배치 등 전 가치사슬을 포괄한다. 한국 기업인 한수원(KHNP)과 두산에너빌리티가 국제 협력 프로젝트의 공급망에 편입되어 한국 원자력 산업의 글로벌 경쟁력 강화를 의미한다. 이는 소형모듈원자로의 실제 시장 창출과 한국 기업의 국제 역할 확대를 보여주는 사례로 평가된다.",
    "id": "6cc4999f"
  },
  {
    "date": "2025-08-08",
    "title": "소형모듈원자로(SMR) 연구보고서 2025-2030",
    "summary": "2025-2030년 소형모듈원자로(SMR) 시장의 혁신, 효율성, 안전성 및 비용 절감에 관한 연구보고서가 발표되었다.",
    "summaryLong": "소형모듈원자로(SMR)의 에너지, 제조, 공급망 산업 혁신을 분석한 2025-2030년 전망 보고서가 발표되었다. 보고서는 SMR이 향상된 효율성, 안전성, 비용 절감을 통해 에너지 생산 및 산업 구조를 혁신할 것으로 예측한다. 이는 글로벌 에너지 전환 기조 속에서 SMR의 역할과 시장 기회를 종합적으로 평가한 자료이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMihwFBVV95cUxPanlkeTlPaGlVTE9BMVBuMDdIYlY4S0Nua2t1czBoQlF5cF9TZVEwS2V6cy00b1BORi1WcDNXQmhLcFVRZ0V5SUkzV19qT09EUmtfYkZnRDhnbXVvRmJReU1rTFV5a01Pdk9HVlA1RFFGcUFTdlBiNDdaVHNpejFLaDdQX0FGdlU?oc=5",
    "k": "small modular reactor smr research report 2025 2030 revolutionizing energy manufacturing and supply chains with enhanced efficiency safety and cost savings",
    "id": "a169bb27"
  },
  {
    "date": "2025-08-04",
    "title": "국제원자력기구(IAEA), 소형모듈원자로(SMR) 글로벌 이니셔티브 확대",
    "summary": "국제원자력기구(IAEA)가 소형모듈원자로 분야의 글로벌 역량 강화를 위한 이니셔티브를 확대했다.",
    "summaryLong": "국제원자력기구(IAEA)가 회원국들의 소형모듈원자로 기술 도입을 지원하기 위해 글로벌 이니셔티브를 확대하고 있다. 이는 개발도상국과 선진국 모두의 에너지 전환을 가속화하려는 IAEA의 전략적 노력이다. 기술 이전, 인력 양성, 규제 기준 마련 등 다각적인 지원 프로그램이 강화되고 있다. IAEA의 주도적 역할은 소형모듈원자로의 안전한 글로벌 확산을 촉진하며, 탄소 중립 목표 달성에 기여할 것으로 예상된다.",
    "cat": "기술",
    "type": "General",
    "dev": "IAEA",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "iaea expands global initiative to boost knowledge of small modular reactors",
    "id": "9e1a1583"
  },
  {
    "date": "2025-07",
    "title": "텐시밸리공사(TVA) 클린치강(Clinch River) BWRX-300, 미국원자력규제위원회(NRC) 건설허가 신청 접수(17개월 심사)",
    "summary": "미국 최초 BWRX-300 건설허가(CP) 신청이 접수되었으며, 2026년 12월까지 심사 완료를 목표로 한다.",
    "cat": "인허가",
    "type": "BWR",
    "dev": "GE-Hitachi",
    "region": "US",
    "source": "Federal Register",
    "url": "https://www.federalregister.gov/documents/2025/07/01/2025-11037/tennessee-valley-authority-clinch-river-nuclear-site-construction-permit-application",
    "summaryLong": "텐시밸리공사(TVA)가 미국에서 최초로 지이-히타치(GE-Hitachi)의 BWRX-300 소형모듈원자로 건설허가를 신청했다. 미국원자력규제위원회(NRC)는 17개월에 걸친 심사를 진행하며 2026년 12월 완료를 목표로 하고 있다. 이는 미국의 소형모듈원자로 상용화 추진의 주요 이정표이자, BWRX-300의 규제 검증이 진행되는 과정이다. 건설허가 승인 후 실제 건설 진입이 예상된다.",
    "id": "6e561787"
  },
  {
    "date": "2025-07",
    "title": "ARC-100, 캐나다 캐나다핵안전위원회(CNSC) 벤더설계심사(VDR) Phase 2 완료",
    "summary": "아크클린에너지(ARC)의 ARC-100이 근본적 인허가 장벽이 없는 것으로 판정되었으며, 뉴브런즈윅전력(NB Power)은 부지준비허가 신청을 진행 중이고 2027년 건설허가 신청을 목표로 한다.",
    "cat": "인허가",
    "type": "SFR",
    "dev": "ARC",
    "region": "CA",
    "source": "WNN",
    "url": "https://www.world-nuclear-news.org/articles/arc-100-completes-canadian-regulatory-design-review",
    "summaryLong": "아크클린에너지(ARC)의 ARC-100 소형모듈원자로가 캐나다핵안전위원회(CNSC)의 벤더설계심사(VDR) 2단계를 완료하여 '근본적 인허가 장벽이 없다'는 판정을 받았다. 뉴브런즈윅전력(NB Power)은 이를 토대로 부지준비허가 신청 절차를 진행하고 있으며, 2027년 건설허가 신청을 목표로 하고 있다. 이는 캐나다에서 소형모듈원자로 인허가의 진전을 보여주는 중요한 사례이다. 실제 건설 단계로의 진입이 임박했다.",
    "id": "e5cb9369"
  },
  {
    "date": "2025-07-22",
    "title": "국제원자력기구(NEA) 소형모듈원자로 대시보드 최신판 발표, 글로벌 배치 확대 추세 확인",
    "summary": "원자력에너지기구(NEA)가 소형모듈원자로(SMR) 대시보드 최신판을 발표하며 전 세계적 SMR 배치 확대 추세를 공표했다.",
    "summaryLong": "경제협력개발기구(OECD) 산하 원자력에너지기구(NEA)가 소형모듈원자로(SMR) 추적·분석용 대시보드의 최신판을 공개했다. 업데이트된 대시보드는 각국의 SMR 개발 및 배치 현황을 종합적으로 보여주며, 글로벌 SMR 시장의 빠른 확대 추세를 확인할 수 있다. 이는 국제사회의 SMR에 대한 높아진 관심과 상용화 가속화를 반영한다.",
    "cat": "기술",
    "type": "General",
    "dev": "NEA",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.?oc=5",
    "k": "new nea small modular reactor dashboard edition reveals global expansion of smr deployment",
    "id": "f1bc9919"
  },
  {
    "date": "2025-07-21",
    "title": "소형모듈원자로 글로벌 확산 추진 중, 실제 건설 가능성은 미지수",
    "summary": "소형모듈원자로(SMR)가 전 세계적으로 주목받고 있으나 실제 건설·운영 단계로의 전환 가능성에 대한 의문이 제기되고 있다.",
    "summaryLong": "전 세계적으로 소형모듈원자로(SMR)에 대한 관심과 투자가 증가하고 있지만, 기술 검증, 규제 승인, 경제성 확보 등 실제 상용 배치 단계로의 전환에 대한 우려가 제기되고 있다. 업계 전문가들은 SMR의 성공적인 실현을 위해서는 정부 정책 지원, 규제 표준화, 기술 성숙도 향상이 필수적이라고 지적한다. 이는 SMR 시장의 성장 가능성과 실현 과정 사이의 격차를 보여주는 현황 분석이다.",
    "cat": "기술",
    "type": "General",
    "dev": "",
    "region": "",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiqgFBVV95cUxNLWc2T29NVWFGY250cmw5dTB3UGhoUEtMMFk2Wko3VUdLcS03SkZ1ZHdCaEJhY015cld6QUI3N3dFLXc3d1pFYVQ5bjFsWFE4blBlWUVhbkREMnZEQmY4WjVaaTdiMzFjOVpQbzhzQkRNSDdVaXU1X2NNVmJydXpHdVZyNEwxRDlDMWR1a3hkMGFCMm13SW9mVDN5dFBwUU5tVGRlS1NvTFp5QQ?oc=5",
    "k": "small modular reactors are gaining steam globally will any get built",
    "id": "32311738"
  },
  {
    "date": "2025-07-16",
    "title": "경제협력개발기구원자력기관(NEA) 소형모듈원자로 디지털 대시보드",
    "summary": "경제협력개발기구원자력기관(NEA)이 소형모듈원자로의 글로벌 정보를 종합한 디지털 대시보드를 제공하고 있다.",
    "summaryLong": "경제협력개발기구원자력기관(NEA)은 전 세계 소형모듈원자로 프로젝트의 현황, 기술 규격, 인허가 진행 상황 등을 통합 관리하는 디지털 대시보드를 운영 중이다. 이 플랫폼은 국제적 정보 투명성을 높이고 각국의 소형모듈원자로 정책 수립을 지원하는 역할을 한다. 업계와 규제당국, 정책 입안자들이 글로벌 동향을 실시간으로 파악할 수 있는 핵심 자료로 활용되고 있다.",
    "cat": "기술",
    "type": "General",
    "dev": "NEA",
    "region": "EU",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMiiwFBVV95cUxQVFo4eVgyQVRldE9lX3RsSm1tYVVYRHp2LVBkUE5aOTN6b3FSbzlvd1RyN0FJMHU0Qm1Pbkpvb3UxR0JqODd3VloyV0wxUW5Ed0YySWhfRVItX2JsREw0eTh0SUdMRGVwNGJwdmFsVjhrYXBxUG5XdGs2a2N5MnpqU1luU3BFMnowUUc4?oc=5",
    "k": "nea small modular reactor digital dashboard",
    "id": "c5fab225"
  },
  {
    "date": "2025-07-11",
    "title": "미국원자력규제위원회(NRC), 텐시밸리공사(TVA) 소형모듈원자로 건설허가 신청서 공식 접수",
    "summary": "미국원자력규제위원회(NRC)가 텐시밸리공사(TVA)의 소형모듈원자로 건설허가 신청을 공식 접수했다.",
    "summaryLong": "미국원자력규제위원회(NRC)는 텐시밸리공사(TVA)로부터 제출된 소형모듈원자로 건설허가(Construction Permit, CP) 신청서를 공식적으로 접수하였다. 이는 미국 내 소형모듈원자로의 규제 심사 진입을 의미하는 중요한 절차이다. 향후 적법성 검토를 거쳐 본격적인 안전성 심사가 진행될 예정이다.",
    "cat": "인허가",
    "type": "General",
    "dev": "TVA",
    "region": "US",
    "source": "\"\"small modular reactor\"\" - Google News",
    "url": "https://news.google.com/rss/articles/CBMipwFBVV95cUxOQlB0NkVhUEd2Y3BDLVZCZlFTRUp2WEZzTjYwSi0tR0pscm1iZzBGZUZEWFBZSDJHakRJQ3ktc3RrVTJ5bzZUM0x6cGxpQ2ZyeUhXUHYzeUsyVkVGek5iN0dBMjh0cFk2X2hEMmlveWJHeC1UaXhEUkRNc0ZqQ0t5Q1RuNk5nbUZRRXhwX2Z2aHJCSXp5MTFHenZ3SWdvMnVFQUlBUVJFbw?oc=5",
    "k": "nrc dockets construction permit application for tva small modular reactor",
    "id": "db234998"
  },
  {
    "date": "2025-05",
    "title": "두산에너빌리티, 엑스에너지(X-energy) 16기분 주기기 예약 + 창원 소형모듈원자로 전용 공장 신설",
    "summary": "두산에너빌리티가 엑스에너지(X-energy)의 소형모듈원자로 16세트분 원자로냉각재펌프(RCP) 등 주요기기를 예약하고, 11GW 파이프라인 대응을 위해 창원에 소형모듈원자로 전용 제작공장을 신설하기로 약정했다.",
    "cat": "계약",
    "type": "HTGR",
    "dev": "두산에너빌리티",
    "region": "KR",
    "source": "POWER",
    "url": "https://www.powermag.com/x-energy-doosan-lock-in-16-unit-xe-100-component-reservation-as-doosan-commits-to-new-smr-factory/",
    "summaryLong": "두산에너빌리티는 미국의 엑스에너지(X-energy)와의 협력 계약으로 16기분의 원자로용기(RPV) 등 원자로냉각재계통(NSSS) 주요기기를 예약했다. 이는 미국의 대규모 소형모듈원자로 수요(약 11GW)에 대응하기 위한 전략적 선제 투자이다. 두산에너빌리티는 창원에 소형모듈원자로 전용 제조공장을 신설하여 글로벌 공급망 확보를 추진하고 있다. 이를 통해 국내 원전기기 산업의 글로벌 경쟁력 강화를 기대하고 있다.",
    "id": "5e061ba6"
  },
  {
    "date": "2025-04",
    "title": "캐나다 온타리오전력공사(OPG), BWRX-300 건설허가 발급 및 핵콘크리트 타설(북미 최초)",
    "summary": "캐나다핵안전위원회(CNSC)가 온타리오전력공사(OPG) 달링턴 부지에 BWRX-300 건설허가를 발급하였고, 세계 최초로 원자로건물 기초공사를 착수했으며 운영허가 신청도 제출했다.",
    "cat": "인허가",
    "type": "BWR",
    "dev": "GE-Hitachi",
    "region": "CA",
    "source": "GE Vernova",
    "url": "https://www.gevernova.com/news/press-releases/ge-vernova-hitachi-bwrx-300-small-modular-reactor-approved-construction-province-ontario-opg",
    "summaryLong": "캐나다핵안전위원회(CNSC)는 온타리오전력공사(OPG)의 달링턴(Darlington) 부지에서 추진 중인 BWRX-300 프로젝트에 건설허가를 정식 발급했다. 이를 바탕으로 온타리오전력공사(OPG)는 원자로건물 기초공사를 시작하여 세계 최초의 BWRX-300 상용 건설 프로젝트를 진행 중이다. 동시에 운영허가 신청도 제출하여 인허가 절차를 가속화하고 있다. 이는 지이-히타치(GE-Hitachi)의 BWRX-300이 국제 규제 검증을 통과하고 실제 건설 단계에 진입했음을 입증하는 획기적 사건이다.",
    "id": "8f101a5d"
  },
  {
    "date": "2025-04",
    "title": "시보그(Seaborg)→솔트포스(Saltfoss) 사명 변경, 초기 연료 저유농축우라늄(LEU)으로 전환",
    "summary": "덴마크의 해상 부유식 용융염로(CMSR) 개발사가 사명을 변경하고, 고유농축우라늄(HALEU) 공급 리스크를 회피하기 위해 초기 연료를 저유농축우라늄(LEU)과 흑연 감속재로 변경했다.",
    "cat": "기술",
    "type": "MSR",
    "dev": "Seaborg/Saltfoss",
    "region": "DK",
    "source": "WNN",
    "url": "https://world-nuclear-news.org/Articles/Seaborg-switches-fuel-plans-due-to-HALEU-supply-is",
    "summaryLong": "덴마크의 해상 부유식 용융염로(CMSR) 개발기업 시보그(Seaborg)가 솔트포스(Saltfoss)로 사명을 변경했다. 이와 함께 국제 공급망의 불확실성으로 인한 고유농축우라늄(HALEU) 조달 리스크를 완화하기 위해, 초기 연료를 저유농축우라늄(LEU)으로 변경하고 감속재를 흑연으로 적용하기로 기술 설계를 조정했다. 이는 현실적 상용화 경로를 모색하는 기술적·전략적 결정이다.",
    "id": "11a23e32"
  },
  {
    "date": "2025-04",
    "title": "웨스팅하우스(Westinghouse) eVinci, 미국원자력규제위원회(NRC) 주요설계기준(DCD) 토픽보고서 승인",
    "summary": "웨스팅하우스(Westinghouse)의 히트파이프 마이크로원자로 eVinci가 미국원자력규제위원회(NRC)의 주요설계기준(DCD) 토픽보고서 승인 마일스톤을 달성했으며, 2024년 12월에는 계측제어(I&C) 플랫폼 승인을 받은 첫 마이크로원자로가 되었다.",
    "cat": "기술",
    "type": "Micro",
    "dev": "Westinghouse",
    "region": "US",
    "source": "WNN",
    "url": "https://www.world-nuclear-news.org/articles/pre-licensing-milestone-for-evinci",
    "summaryLong": "웨스팅하우스(Westinghouse)의 히트파이프 마이크로원자로 eVinci가 미국원자력규제위원회(NRC)로부터 주요설계기준(DCD, Design Certification Document) 토픽보고서에 대한 승인을 받았다. 이전 2024년 12월에는 계측제어(I&C) 플랫폼 승인을 획득하여 마이크로원자로 중 최초의 성과를 달성했다. 이러한 규제 마일스톤들은 eVinci의 인허가 진전을 보여주는 중요한 지표이다. 향후 최종 설계인증(DC) 취득으로 진행될 예정이다.",
    "id": "0270ca2f"
  },
  {
    "date": "2025-03",
    "title": "다우·X-에너지(X-energy), 롱 몬트(Long Mott) Xe-100 건설허가 신청 제출",
    "summary": "시드리프트(텍사스) 부지에 Xe-100 4기 건설허가(CP) 신청 — 다우 화학공장의 공정열과 전력 공급용으로, 산업체 최초의 첨단로 건설허가 신청이다.",
    "cat": "인허가",
    "type": "HTGR",
    "dev": "X-energy",
    "region": "US",
    "source": "POWER",
    "url": "https://www.powermag.com/dow-and-x-energy-advance-landmark-nuclear-project-in-texas-with-construction-permit-filing/",
    "summaryLong": "X-에너지와 화학기업 다우가 미국 텍사스주 시드리프트 부지에 Xe-100 초고온가스로(HTGR) 4기의 건설허가(CP)를 신청했다. 이는 다우의 화학공장에 공정열과 전력을 공급하기 위한 것으로, 산업체가 첨단로 건설허가를 신청한 첫 사례다. 이 프로젝트는 산업 탈탄소화의 주요 사례로 주목받고 있으며, 미국 원자력규제위원회(NRC)의 심사를 통해 실현 가능성이 판단될 것으로 예상된다.",
    "id": "3762b014"
  },
  {
    "date": "2024",
    "title": "아마존, X-에너지에 5억 달러 투자 + 에너지 노스웨스트(Energy Northwest) 배치",
    "summary": "아마존이 시리즈 C-1 투자로 약 5억 달러를 X-에너지에 투자하고, 워싱턴주 에너지 노스웨스트와 Xe-100 배치 합의를 이루었다.",
    "cat": "투자",
    "type": "HTGR",
    "dev": "X-energy",
    "region": "US",
    "source": "X-energy",
    "url": "https://en.wikipedia.org/wiki/X-energy",
    "summaryLong": "전자상거래 기업 아마존이 X-에너지의 시리즈 C-1 펀딩 라운드에 약 5억 달러를 투자했다. 동시에 아마존은 워싱턴주 지역 전력기업 에너지 노스웨스트(Energy Northwest)와 함께 Xe-100 배치를 추진하기로 합의했다. 이는 아마존의 급증하는 데이터센터 전력 수요를 충족하기 위한 전략의 일부로, 빅테크 기업의 첨단로 기술에 대한 관심 증가를 반영한다.",
    "id": "bedb6e25"
  },
  {
    "date": "2024",
    "title": "카이로스 파워(Kairos Power)–구글, 첨단로 전력공급 계약 체결",
    "summary": "카이로스의 KP-FHR(헤르메스(Hermes) 계열) 첨단로에서 생산한 전력을 구글에 공급하는 전력공급계약(PPA)을 체결했다.",
    "cat": "계약",
    "type": "FHR",
    "dev": "Kairos Power",
    "region": "US",
    "source": "Kairos Power",
    "url": "",
    "summaryLong": "첨단로 개발사 카이로스 파워와 기술 대기업 구글이 전력공급계약(PPA)을 체결했다. 카이로스의 KP-FHR(헤르메스 계열) 용융염냉각로(MSFR)에서 생산한 전력을 구글에 장기 공급하는 계약이다. 이는 빅테크 기업이 첨단로 전력을 직접 구매하는 계약 중 주요 사례로, 데이터센터 운영의 탈탄소화 추진을 보여준다. 구글의 재정 및 기술적 지원은 카이로스의 상용화 추진에 중요한 역할을 할 것으로 전망된다.",
    "id": "811c9dc5"
  },
  {
    "date": "2024-09",
    "title": "SMART100 표준설계인가 취득(한국)",
    "summary": "한국원자력연구원(KAERI)의 SMART100이 한국 표준설계인가를 취득했으며, 사우디 수출과 현대엔지니어링 협력을 추진 중이다.",
    "cat": "인허가",
    "type": "PWR",
    "dev": "SMART/KAERI",
    "region": "KR",
    "source": "KAERI",
    "url": "https://kaeri.re.kr/board/view?linkId=12112&menuId=MENU00326",
    "summaryLong": "한국원자력연구원(KAERI)이 개발한 소형모듈로(SMR) SMART100이 국내 표준설계인가를 취득했다. SMART100은 166 MWth급 통합형 원전으로, 한국 기술의 우수성을 국제적으로 입증하는 의미를 가진다. 이를 통해 사우디아라비아 수출 및 현대엔지니어링과의 협력이 구체화될 것으로 예상되며, 향후 한국의 SMR 해외 수출 전략 추진에 중추적 역할을 할 것으로 전망된다.",
    "id": "5661c65a"
  },
  {
    "date": "2023-01",
    "title": "뉴스케일(NuScale) US600(77MWe) 설계인증 취득",
    "summary": "미국원자력규제위원회(NRC)로부터 설계인증을 받은 첫 소형모듈로(SMR)이며, 이후 출력증강형 US460의 표준설계승인(SDA) 추가 심사가 진행 중이다.",
    "cat": "인허가",
    "type": "PWR",
    "dev": "NuScale",
    "region": "US",
    "source": "Federal Register",
    "url": "https://www.federalregister.gov/documents/2023/01/19/2023-00729/nuscale-small-modular-reactor-design-certification",
    "summaryLong": "뉴스케일이 개발한 77 MWe급 소형모듈로(SMR) US600이 미국원자력규제위원회(NRC)로부터 설계인증(DC)을 획득했다. 이는 상용 SMR 중 미국 규제당국으로부터 획득한 첫 번째 설계인증으로, SMR 산업화에 있어 중요한 이정표다. 뉴스케일은 이후 출력증강형 US460 설계에 대한 표준설계승인(SDA)을 추가로 신청하여 심사를 받고 있다. 이러한 진전은 미국 내 SMR 시장 활성화 및 글로벌 확산에 기여할 것으로 예상된다.",
    "id": "17aaa070"
  }
];
window.SMR_UPDATED = "2026-06-15 13:17 KST";
